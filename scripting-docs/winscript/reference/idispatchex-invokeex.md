...
ms.locfileid: "88144413"
---
# <a name="idispatchexinvokeex"></a>IDispatchEx::InvokeEx
Fournit l’accès aux propriétés et aux méthodes exposées par un `IDispatchEx` objet.  
...
#### <a name="parameters"></a>Paramètres  
 ...
 `lcid`  
 Contexte des paramètres régionaux dans lequel interpréter les arguments. `lcid`Est passé à `InvokeEx` pour permettre à l’objet d’interpréter ses arguments spécifiques à des paramètres régionaux.  
 ...
 Indicateurs décrivant le contexte de l' `InvokeEx` appel :  
...
|DISPATCH_CONSTRUCT|Le membre est utilisé en tant que constructeur. (Il s’agit d’une nouvelle valeur définie par `IDispatchEx` ). Les valeurs autorisées pour `wFlags` sont les suivantes :<br /><br /> DISPATCH_PROPERTYGET DISPATCH_METHOD DISPATCH_PROPERTYPUT DISPATCH_PROPERTYPUTREF DISPATCH_CONSTRUCT|  
 ...
 `pdp`  
 Pointeur vers une structure qui contient un tableau d'arguments, un tableau d'arguments DISPID pour les arguments nommés et le nombre d'éléments de chaque tableau. Consultez la `IDispatch` documentation pour obtenir une description complète de la structure DISPPARAMS.  
 ...
 `pei`  
 Pointeur vers une structure qui contient les informations sur les exceptions. Cette structure doit être remplie si `DISP_E_EXCEPTION` est retourné. Peut avoir la valeur null. Consultez la `IDispatch` documentation pour obtenir une description complète de la `EXCEPINFO` structure.  
...
|`pspCaller`|`pspCaller`Permet à l’objet d’accéder aux services fournis par l’appelant. Les services spécifiques peuvent être gérés par l’appelant lui-même ou être délégué aux appelants plus loin dans la chaîne d’appel. Par exemple, si un moteur de script à l’intérieur d’un navigateur effectue un `InvokeEx` appel à un objet externe, l’objet peut suivre la `pspCaller` chaîne pour obtenir des services à partir du moteur de script ou du navigateur. (Notez que la chaîne d’appel n’est pas la même que la chaîne de création, également appelée chaîne de conteneur ou chaîne de site. La chaîne de création peut être disponible par le biais d’un autre mécanisme tel que `IObjectWithSite` .)|  
|Pointeur `this`|Lorsque DISPATCH_METHOD est défini dans `wFlags` , il peut y avoir un « paramètre nommé » pour la valeur « This ». Le DISPID sera DISPID_THIS et il doit s’agir du premier paramètre nommé.|  
 ...
 Le paramètre inutilisé `riid` dans `IDispatch::Invoke` a été supprimé.  
  
 Le `puArgArr` paramètre dans `IDispatch::Invoke` a été supprimé.  
  
 Consultez la `IDispatch::Invoke` documentation pour les exemples suivants :  
...
|DISP_E_EXCEPTION|L’application doit lever une exception. Dans ce cas, la structure passée `pei` doit être remplie.|  
