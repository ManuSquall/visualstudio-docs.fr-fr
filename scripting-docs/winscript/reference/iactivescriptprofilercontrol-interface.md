...
ms.locfileid: "72571602"
---
# <a name="iactivescriptprofilercontrol-interface"></a>IActiveScriptProfilerControl, interface
Implémenté par le moteur de script qui prend en charge le profilage. En général, un objet qui implémente le `IActiveScriptProfilerControl` implémente également l’interface [IActiveScript](../../winscript/reference/iactivescript.md) . Dans ce cas, vous pouvez obtenir un handle pour l’interface `IActiveScriptProfilerControl` en appelant la méthode `IUnknown::QueryInterface` sur l’objet. L’interface fournit les méthodes nécessaires pour arrêter et démarrer le profilage sur le moteur de script.  
...
|[IActiveScriptProfilerControl::SetProfilerEventMask](../../winscript/reference/iactivescriptprofilercontrol-setprofilereventmask.md)|Définit le masque d’événement du profileur dans le moteur de script.|  
