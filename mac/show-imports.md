...
Visual Studio pour Mac pouvez afficher tous les types disponibles, même s’ils ne sont pas importés dans votre projet, dans votre liste de saisie semi-automatique IntelliSense. Si vous sélectionnez un élément qui n’est pas importé, l' `using` instruction correcte sera ajoutée à votre fichier source.
...
Pour activer cette fonctionnalité, ouvrez **Préférences** via les préférences **Visual Studio**  >  **Preferences** et accédez à **éditeur de texte**  >  **IntelliSense**. Cochez la case **afficher les éléments à importer** pour activer des éléments supplémentaires dans IntelliSense.
...
Dans la liste IntelliSense, les espaces de noms s’affichent en regard des membres qui ne sont pas actuellement référencés par une `using` instruction. Si vous choisissez l’un de ces éléments dans la liste, le membre est ajouté à votre code _et_ l' `using` instruction est ajoutée au début du fichier. Les membres des types déjà référencés dans le code codé n’afficheront pas leur espace de noms dans IntelliSense.
