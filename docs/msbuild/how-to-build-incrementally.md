...
Quand vous générez un projet volumineux, il est important de ne pas regénérer les composants précédemment générés qui sont encore à jour. Si toutes les cibles sont générées à chaque fois, la génération de builds prend beaucoup de temps. Pour permettre des builds incrémentaux (construits dans lesquels seules les cibles qui n’ont pas été construites auparavant ou les cibles qui sont obsolètes, sont reconstruites), le moteur de construction Microsoft (MSBuild) peut comparer les délais des fichiers d’entrée avec les délais des fichiers de sortie et déterminer s’il faut sauter, construire ou reconstruire partiellement une cible. Toutefois, il doit exister un mappage un-à-un entre les entrées et les sorties. Vous pouvez utiliser des transformations pour permettre aux cibles d’identifier ce mappage direct. Pour plus d’informations sur les transformations, consultez [Transformations](../msbuild/msbuild-transforms.md).
...
MSBuild peut comparer les délais des fichiers d’entrée avec les délais des fichiers de sortie et déterminer s’il faut sauter, construire ou reconstruire partiellement une cible. Dans l’exemple suivant, si `@(CSFile)` un fichier de la liste d’éléments est plus récent que le fichier *hello.exe,* MSBuild exécutera la cible; sinon il sera ignoré:
...
Quand les entrées et les sorties sont spécifiées dans une cible, chaque sortie est mappée avec une seule entrée ou il n’existe pas de mappage direct entre les sorties et les entrées. Dans la [tâche précédente Csc](../msbuild/csc-task.md), par exemple, la sortie, *hello.exe*, ne peut pas être cartographié à une seule entrée - il dépend de chacun d’eux.

> [!NOTE]
> Une cible dans laquelle il n’y a pas de cartographie directe entre les entrées et les extrants sera toujours construite plus souvent qu’une cible dans laquelle chaque sortie peut cartographier à une seule entrée parce que MSBuild ne peut pas déterminer quelles sorties doivent être reconstruites si certains des entrées ont changé.
