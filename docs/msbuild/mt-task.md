...
Enveloppe l’outil de manifeste de Microsoft, *mt.exe*. Pour plus d’informations, voir [mt.exe](/windows/desktop/SbsCs/mt-exe).
...
> [!NOTE]
> La documentation *mt.exe* utilise**-** un trait d’union () comme préfixe pour**/** les options de ligne de commande, mais ce sujet utilise une barre oblique ( ). Les deux préfixes sont acceptés.

|Paramètre|Description|
|---------------|-----------------|
|**AdditionalManifestFiles**|Paramètre **de chaîne en** option.<br /><br /> Spécifie le nom d’un ou plusieurs fichiers manifestes.<br /><br /> Pour plus d’informations, voir l’option **/manifest** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
...
|**AssemblyIdentity**|Paramètre **de chaîne** facultatif.<br /><br /> Spécifie les valeurs d’attribut de l’élément **assemblyIdentity** du manifeste. Spécifier une liste de virgule, où le `name` premier composant est la valeur de l’attribut, suivi d’une ou plusieurs paires de noms/valeurs qui ont le formulaire, * \<attribuent *le nom><attribute_value>.<br /><br /> Pour plus d’informations, voir l’option **/identity** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
|**ComponentFileName**|Paramètre **de chaîne** facultatif.<br /><br /> Spécifie le nom de la bibliothèque de liaison dynamique que vous avez l’intention de construire à partir des fichiers *.rgs* ou *.tlb.* Ce paramètre est obligatoire si vous spécifiez les paramètres **RegistrarScriptFile** ou **TypeLibraryFile** de la tâche MT.<br /><br /> Pour plus d’informations, voir l’option **/dll** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
...
|**GenerateCatalogFiles**|Paramètre `Boolean` facultatif.<br /><br /> Si `true`, génère la définition de catalogue (*.cdf*) fichiers.<br /><br /> Pour plus d’informations, voir l’option **/makecdfs** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
...
|**RegistrarScriptFile**|Paramètre **de chaîne** facultatif.<br /><br /> Spécifie le nom du fichier script du registraire (*.rgs)* à utiliser pour le support manifeste COM sans inscription.<br /><br /> Pour plus d’informations, voir l’option **/rgs** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
|**ReplacementsFile**|Paramètre **de chaîne** facultatif.<br /><br /> Spécifie le fichier qui contient des valeurs pour les chaînes remplaçables dans le script du registraire (*.rgs*) fichier.<br /><br /> Pour plus d’informations, voir l’option **/replacements** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
...
|**récentes**|Paramètre `ITaskItem[]` facultatif.<br /><br /> Spécifie une liste des fichiers sources du manifeste, séparés par des espaces.<br /><br /> Pour plus d’informations, voir l’option **/manifest** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
...
|**TypeLibraryFile**|Paramètre **de chaîne** facultatif.<br /><br /> Spécifie le nom du fichier type bibliothèque *(.tlb).* Si vous spécifiez ce paramètre, spécifiez aussi le paramètre de tâche **ComponentFileNameMT**.<br /><br /> Pour plus d’informations, voir l’option **/tlb** de [mt.exe](/windows/desktop/SbsCs/mt-exe).|
