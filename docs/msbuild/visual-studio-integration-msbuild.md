 ...
 *MSBuild.exe* reconnaît n'importe quelle extension de nom de fichier projet correspondant au modèle *.\*proj*. Toutefois, Visual Studio ne reconnaît qu’un sous-ensemble de ces extensions de nom de fichier projet, qui déterminent le système de projet spécifique au langage qui chargera le projet. Visual Studio n’a pas de système de projet basé sur MSBuild indépendant du langage.

 Par exemple, le C# système de projet charge les fichiers *. csproj* , mais Visual Studio n’est pas en mesure de charger un fichier *. fichier xxproj* . Un fichier projet pour les fichiers sources dans un langage arbitraire doit utiliser la même extension que Visual Basic C# ou les fichiers projet à charger dans Visual Studio.
...
```
 ...
 À cet effet, Visual Studio examine les conditions sur les éléments `PropertyGroup`, `ItemGroup`, `Import`, Property et Item.
...
## <a name="build-solutions"></a>Créer des solutions
 ...
 Lors de la génération dans Visual Studio, la propriété `$(BuildingInsideVisualStudio)` est définie sur `true`. Vous pouvez l'utiliser dans vos fichiers projet ou *.targets* pour que la génération se comporte différemment.
...
## <a name="debugging"></a>Débogage
 ...
 Pour rechercher et lancer l’assembly de sortie et attacher le débogueur, Visual Studio a besoin des propriétés `OutputPath`, `AssemblyName`et `OutputType` correctement définies. Il ne sera pas possible d'attacher le débogueur si le processus de génération n'a pas donné lieu à la génération d'un fichier *.pdb* par le compilateur.
...
## <a name="intellisense-and-validation"></a>IntelliSense et validation
 ...
 Lorsque vous utilisez l’éditeur XML pour modifier des fichiers projet, IntelliSense et la validation sont pilotés par les fichiers de schéma MSBuild. Ceux-ci sont installés dans le cache des schémas, qui se trouve dans *\<Répertoire d’installation de Visual Studio>\Xml\Schemas\1033\MSBuild*.
