...
Une fois l’enregistreur d’événements compilé dans un assembly, vous devez indiquer à MSBuild d’utiliser cet enregistreur d’événements pendant les builds. Pour cela, vous devez utiliser le commutateur **-logger** avec *MSBuild.exe*. Pour plus d’informations sur les commutateurs disponibles pour *MSBuild.exe*, consultez [Informations de référence sur la ligne de commande](../msbuild/msbuild-command-line-reference.md).
