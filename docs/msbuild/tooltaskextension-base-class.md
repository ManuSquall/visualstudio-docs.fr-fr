...
|<xref:Microsoft.Build.Utilities.ToolTask.EchoOff%2A>|Paramètre `bool` facultatif.<br /><br /> Quand la valeur est `true`, cette tâche passe **\/Q** à la ligne de commande cmd.exe de sorte que la ligne de commande ne soit pas copiée dans stdout.|  
