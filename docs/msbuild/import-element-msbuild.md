...
## <a name="remarks"></a>Notes
 ...
 Si le projet importé n’a pas d’attribut `DefaultTargets` , les projets importés sont inspectés dans l’ordre dans lequel ils sont importés, et la valeur du premier attribut `DefaultTargets` découvert est utilisée. Par exemple, si ProjectA importe le projetb et le projetC (dans cet ordre) et que le projetb importe ProjectD, MSBuild commence par Rechercher les `DefaultTargets` spécifiés sur ProjectA, puis le projetb, puis Projected et enfin le projetC.
