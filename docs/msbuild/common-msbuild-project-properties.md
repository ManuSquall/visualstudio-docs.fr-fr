...
| ALToolPath | Chemin d’accès où se trouve *al. exe* . Cette propriété remplace la version actuelle de *al. exe* pour permettre l’utilisation d’une version différente. |
...
| BaseOutputPath | Spécifie le chemin d'accès de base du fichier de sortie. S’il est défini, MSBuild utilise `OutputPath = $(BaseOutputPath)\$(Configuration)\` . Exemple de syntaxe : `<BaseOutputPath>c:\xyz\bin\</BaseOutputPath>` |
| BuildInParallel | Valeur booléenne qui indique si les références de projet sont générées ou nettoyées en parallèle quand MSBuild multi-proc est utilisé. La valeur par défaut est `true`, ce qui signifie que les projets seront générés en parallèle si le système possède plusieurs cœurs ou processeurs. |
...
| CscToolPath | Le chemin d’accès de *CSC. exe*, le compilateur C#. |
...
| DebugSymbols | Valeur booléenne qui indique si les symboles sont générés par la procédure.<br /><br /> La définition de **-p :DebugSymbols = false** sur la ligne de commande désactive la génération de fichiers de symboles de base de données du programme (*. pdb*). |
...
| DisabledWarnings | Supprime les avertissements spécifiés. Seule la partie numérique de l'identificateur d'avertissement doit être spécifiée. Les différents avertissements sont séparés par des points-virgules. Ce paramètre correspond au `/nowarn` commutateur du compilateur *vbc. exe* . |
...
| FrameworkPathOverride | Spécifie l’emplacement de *mscorlib. dll* et de *Microsoft. VisualBasic. dll*. Ce paramètre est équivalent au `/sdkpath` commutateur du compilateur *vbc. exe* . |
| GenerateDocumentation | (C#, Visual Basic) Paramètre booléen qui indique si la documentation est générée par la Build. Si `true` , la génération génère des informations de documentation et les place dans un fichier *. xml* avec le nom de la bibliothèque ou du fichier exécutable créé par la tâche de génération. |
...
| NoVBRuntimeReference | Valeur booléenne qui indique si le runtime d’Visual Basic (*Microsoft. VisualBasic. dll*) doit être inclus comme référence dans le projet. |
...
| OptionCompare | Spécifie la façon dont sont effectuées les comparaisons de chaînes. Les valeurs valides sont "binary" et "text". Cette propriété est équivalente au `/optioncompare` commutateur du compilateur *vbc. exe*. |
...
| OptionStrict | Valeur booléenne qui, lorsqu'elle correspond à `true`, pousse la tâche de génération à appliquer une sémantique de type stricte pour restreindre les conversions de types implicites. Cette propriété est équivalente au `/optionstrict` commutateur du compilateur *vbc. exe* . |
...
| RemoveIntegerChecks | Valeur booléenne indiquant s'il convient de désactiver les contrôles d'erreurs de dépassement sur les entiers. La valeur par défaut est `false`. Cette propriété est équivalente au `/removeintchecks` commutateur du compilateur *vbc. exe* . |
...
| SkipInvalidConfigurations | Quand `true` la valeur est, génère un avertissement sur les combinaisons non valides de plateforme et de configuration, mais ne fait pas échouer la build ; lorsque la valeur est ou n’est pas `false` définie (valeur par défaut), génère une erreur. |
...
| UseHostCompilerIfAvailable | Paramètre booléen qui, s'il a pour valeur `true`, force la tâche de génération à utiliser l'objet de compilateur in-process, s'il est disponible. Ce paramètre est utilisé uniquement par Visual Studio. |
...
| VbcVerbosity | Spécifie le niveau de détail de la sortie du compilateur Visual Basic. Les valeurs valides sont « Quiet », « Normal » (valeur par défaut) et « Verbose ». |
