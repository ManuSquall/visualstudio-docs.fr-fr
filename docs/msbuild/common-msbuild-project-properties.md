...
ms.locfileid: "77634420"
---
# <a name="common-msbuild-project-properties"></a>Propriétés communes des projets MSBuild

Le tableau suivant répertorie fréquemment les propriétés utilisées qui sont définies dans les fichiers du projet Visual Studio ou incluses dans des fichiers *.cibles* que MSBuild fournit.

 Les fichiers projet dans Visual Studio (*.csproj*, *.vbproj*, *.vcxproj* et autres) contiennent le code XML MSBuild qui s’exécute quand vous générez un projet à l’aide de l’IDE. Les projets importent généralement un ou plusieurs fichiers *.targets* pour définir leur processus de construction. Pour plus d’informations, consultez [Fichiers .targets MSBuild](../msbuild/msbuild-dot-targets-files.md).
...
| ALToolPath | Le chemin où *AL.exe* peut être trouvé. Cette propriété remplace la version actuelle *d’AL.exe* pour permettre l’utilisation d’une version différente. |
| ApplicationIcon | Le fichier *d’icône .ico* à transmettre au compilateur pour l’incorporation comme une icône Win32. Cette propriété est équivalente au commutateur `/win32icon` du compilateur. |
| ApplicationManifest | Spécifie le chemin d'accès du fichier utilisé pour générer les informations de manifeste de Contrôle de compte d'utilisateur externe. S’applique uniquement aux projets Visual Studio ciblant Windows Vista.<br /><br /> Dans la plupart des cas, le manifeste est incorporé. Toutefois, si vous utilisez le déploiement d’Registration Free COM ou ClickOnce, le manifeste peut être un fichier externe qui est installé avec vos assemblages d’applications. Pour plus d'informations, consultez la propriété NoWin32Manifest dans cette rubrique. |
...
| BaseOutputPath | Spécifie le chemin d'accès de base du fichier de sortie. Si elle est définie, MSBuild utilisera `OutputPath = $(BaseOutputPath)\$(Configuration)\`. Exemple de syntaxe : `<BaseOutputPath>c:\xyz\bin\</BaseOutputPath>` |
| BuildInParallel | Une valeur boolean qui indique si les références de projet sont construites ou nettoyées en parallèle lorsque multi-Proc MSBuild est utilisé. La valeur par défaut est `true`, ce qui signifie que les projets seront générés en parallèle si le système possède plusieurs cœurs ou processeurs. |
| BuildProjectReferences | Une valeur boolean qui indique si les références de projet sont construites par MSBuild. Définissez `false` automatiquement si vous construisez votre projet dans l’environnement de développement intégré Visual Studio (IDE), `true` si autrement. `-p:BuildProjectReferences=false` peut être spécifié sur la ligne de commande pour éviter de vérifier si les projets référencés sont à jour. |
...
| CscToolPath | Le chemin de *csc.exe*, le compilateur C. |
...
| DebugSymbols | Valeur booléenne qui indique si les symboles sont générés par la procédure.<br /><br /> Réglage **-p:DebugSymbols-faux** sur la ligne de commande désactive la génération de fichiers de base de données de programme (*.pdb).* |
...
| DisabledWarnings | Supprime les avertissements spécifiés. Seule la partie numérique de l'identificateur d'avertissement doit être spécifiée. Les différents avertissements sont séparés par des points-virgules. Ce paramètre correspond `/nowarn` au commutateur du compilateur *vbc.exe.* |
| DisableFastUpToDateCheck | Une valeur boolean qui ne s’applique qu’à Visual Studio. Le gestionnaire de construction Visual Studio utilise un processus appelé FastUpToDateCheck pour déterminer si un projet doit être reconstruit pour être à jour. Ce processus est plus rapide que l’utilisation de MSBuild pour déterminer cela. Définir la propriété DisableFastUpToDateCheck pour vous permettre de `true` contourner le gestionnaire de construction Visual Studio et de l’obliger à utiliser MSBuild pour déterminer si le projet est à jour. |
...
| ExcludeDeploymentUrl | La [tâche GenerateDeploymentManifest](../msbuild/generatedeploymentmanifest-task.md) ajoute une étiquette de déploiementProvider au manifeste de déploiement si le fichier du projet comprend l’un des éléments suivants :<br /><br /> -   UpdateUrl<br />-   InstallUrl<br />-   PublishUrl<br /><br /> En utilisant ExcludeDeploymentUrl, toutefois, vous pouvez empêcher l’étiquette deploymentProvider d’être ajoutée au manifeste de déploiement même si l’une des URL ci-dessus est spécifiée. Pour cela, ajoutez la propriété suivante à votre fichier projet :<br /><br /> `<ExcludeDeploymentUrl>true</ExcludeDeploymentUrl>` <br /><br />**Note:**  ExcluDeploymentUrl n’est pas exposé dans l’IDE Visual Studio et ne peut être défini qu’en modifiant manuellement le fichier du projet. La mise en place de cette propriété n’affecte pas l’édition au sein de Visual Studio; c’est-à-dire que l’étiquette deploymentProvider sera toujours ajoutée à l’URL spécifiée par PublishUrl. |
...
| FrameworkPathOverride | Specifie l’emplacement de *mscorlib.dll* et *microsoft.visualbasic.dll*. Ce paramètre est `/sdkpath` équivalent à l’interrupteur du compilateur *vbc.exe.* |
| GenerateDocumentation | (C, Visual Basic) Un paramètre boolean qui indique si la documentation est générée par la construction. Si, `true`la construction génère des informations de documentation et les met dans un fichier *.xml* avec le nom du fichier exécutable ou de la bibliothèque que la tâche de construction créé. |
| GénérerFullPaths | (C) Générer des chemins complets pour les noms de fichiers dans la sortie en utilisant l’option de compilateur [-fullpaths.](/dotnet/csharp/language-reference/compiler-options/fullpaths-compiler-option) |
...
| NoVBRuntimeReference | Une valeur boolean qui indique si le temps d’exécution Visual Basic (*Microsoft.VisualBasic.dll*) doit être inclus comme une référence dans le projet. |
| NoWin32Manifest | Valeur booléenne qui indique si les informations de manifeste de Contrôle de compte d'utilisateur seront incorporées dans l'exécutable de l'application. S’applique uniquement aux projets Visual Studio ciblant Windows Vista. Dans les projets déployés à l’aide de ClickOnce et de Registration-Free COM, cet élément est ignoré. `False` (valeur par défaut) spécifie que les informations de manifeste de Contrôle de compte d'utilisateur doivent être incorporées dans le fichier exécutable de l'application. `True` spécifie que les informations de manifeste de Contrôle de compte d'utilisateur ne doivent pas être incorporées.<br /><br /> Cette propriété ne s’applique qu’aux projets Visual Studio ciblant Windows Vista. Dans les projets déployés à l’aide de ClickOnce et De l’enregistrement-Free COM, cette propriété est ignorée.<br /><br /> Vous ne devez ajouter NoWin32Manifest que si vous ne voulez pas que Visual Studio intègre des informations manifestes dans l’exécution de l’application; ce processus est appelé *virtualisation*. Pour utiliser la virtualisation, définissez `<ApplicationManifest>` conjointement à `<NoWin32Manifest>`, comme suit :<br /><br /> - Pour les projets `<ApplicationManifest>` Visual Basic, retirez le nœud. (Dans les projets Visual `<NoWin32Manifest>` `<ApplicationManifest>` Basic, est ignoré quand un nœud existe.)<br />- Pour les projets `False` `<NoWin32Manifest>` C, set `<ApplicationManifest>` to and to `True`. (Dans les projets C,) `<ApplicationManifest>` `<NoWin32Manifest>`<br /> Cette propriété est `/nowin32manifest` équivalente à l’interrupteur compilateur de *vbc.exe*. |
...
| OptionCompare | Spécifie la façon dont sont effectuées les comparaisons de chaînes. Les valeurs valides sont "binary" et "text". Cette propriété est `/optioncompare` équivalente à l’interrupteur compilateur de *vbc.exe*. |
...
| OptionStrict | Valeur booléenne qui, lorsqu'elle correspond à `true`, pousse la tâche de génération à appliquer une sémantique de type stricte pour restreindre les conversions de types implicites. Cette propriété est `/optionstrict` équivalente à l’interrupteur du compilateur *vbc.exe.* |
| OutDir OutDir | Indique l’emplacement final de sortie du projet ou de la solution. Lors de la construction d’une solution, OutDir peut être utilisé pour recueillir plusieurs sorties de projet en un seul endroit. En outre, OutDir est inclus dans AssemblySearchPaths utilisés pour résoudre les références. Par exemple, *bin-Debug*. |
| OutputPath | Spécifie le chemin du répertoire de sortie, relatif au répertoire de projet, par exemple *bin\Debug*. |
| OutputType | Spécifie le format de fichier du fichier de sortie. Ce paramètre peut prendre l'une des valeurs suivantes :<br /><br /> -   Library. Crée une bibliothèque de code. (Valeur par défaut)<br />-   Exe. Crée une application console.<br />-   Module. Crée un module.<br />-   Winexe. Crée un programme Windows.<br /><br /> Cette propriété est `/target` équivalente à l’interrupteur du compilateur *vbc.exe.* |
...
| RemoveIntegerChecks | Valeur booléenne indiquant s'il convient de désactiver les contrôles d'erreurs de dépassement sur les entiers. La valeur par défaut est `false`. Cette propriété est `/removeintchecks` équivalente à l’interrupteur du compilateur *vbc.exe.* |
...
| UseHostCompilerIfAvailable | Paramètre booléen qui, s'il a pour valeur `true`, force la tâche de génération à utiliser l'objet de compilateur in-process, s'il est disponible. Ce paramètre n’est utilisé que par Visual Studio. |
...
| VbcVerbosity | Spécifie la verbosité de la sortie du compilateur Visual Basic. Les valeurs valides sont « Quiet », « Normal » (valeur par défaut) et « Verbose ». |
