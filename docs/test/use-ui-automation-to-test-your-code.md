...
Vous pouvez créer des tests automatisés qui vérifient l’interface utilisateur (IU) de votre application. Ces tests sont appelés *tests codés de l’interface utilisateur*. Ils permettent d’effectuer des tests fonctionnels des contrôles d’interface utilisateur et de vérifier que l'application entière, y compris son interface utilisateur, fonctionne correctement. Les tests codés de l'interface utilisateur sont particulièrement utiles quand il y a une validation ou autre logique dans l'interface utilisateur, par exemple dans une page web. On les utilise aussi fréquemment pour automatiser un test manuel existant.  
 ...
 ![Test durant le développement d’applications](~/docs/test/media/cuit_overview.png "CUIT_Overview")  
...
2.  **Ajouter un fichier de test codé de l’interface utilisateur.**  
     ...
     ![Créer un test codé de l’interface utilisateur](~/docs/test/media/codedui_create.png "CodedUI_Create")  
     ...
     ![Sélectionner des actions d’enregistrement](~/docs/test/media/codedui_codegendialogb.png "CodedUI_CodeGenDialogB")  
  
     Le Générateur de test codé de l'interface utilisateur apparaît et Visual Studio est réduit.  
  
     ![Générateur de test codé de l’interface utilisateur](~/docs/test/media/codedui_testbuilder.png "CodedUI_TestBuilder")  
...
4.  **Vérifier les valeurs dans les champs d’IU tels que les zones de texte**.  
     ...
     ![Élément de ciblage du test codé de l’interface utilisateur](~/docs/test/media/codedui_1.png "CodedUI_1")  
...
7.  **Modifier les détails des actions et assertions du test**.  
     ...
     ![Éditeur de test codé de l’interface utilisateur](~/docs/test/media/cuit_editor_edit.png "CUIT_Editor_edit")  
...
-   ![Prérequis](~/docs/test/media/prereq.png "Prérequis") Si vous ne souhaitez pas enregistrer les actions de démarrage de votre application durant le test, vous devez la démarrer avant de choisir l’icône **Enregistrer**.  
  
-   ![Prérequis](~/docs/test/media/prereq.png "Prérequis")À la fin du test, le processus dans lequel le test s’exécute est arrêté. Si vous avez démarré votre application dans le test, l'application se ferme généralement à l'issue du test.  Si vous ne voulez pas que votre application soit fermée à la fin du test, vous devez ajouter un fichier .runsettings à votre solution et utiliser l'option `KeepExecutorAliveAfterLegacyRun`. Pour plus d’informations, consultez [Configurer des tests unitaires à l’aide d’un fichier .runsettings](../test/configure-unit-tests-by-using-a-dot-runsettings-file.md).  
  
-   ![Prérequis](~/docs/test/media/prereq.png "Prérequis") Vous pouvez ajouter une méthode d’initialisation de test, identifiée par un attribut [TestInitialize], qui exécute du code au début de chaque méthode de test. Vous pouvez par exemple démarrer l'application à partir de la méthode TestInitialize.  
  
-   ![Prérequis](~/docs/test/media/prereq.png "Prérequis") Vous pouvez ajouter une méthode de nettoyage de test, identifiée par un attribut [TestCleanup], qui exécute du code à la fin de chaque méthode de test. La méthode de fermeture de l'application pourrait par exemple être appelée à partir de la méthode TestCleanup.  
...
###  <a name="VerifyingCodeUsingCUITGenerateAssertions"></a> Validation des propriétés des contrôles d’IU  
 ...
 ![Propriétés du test codé de l’interface utilisateur](~/docs/test/media/codedui_2.png "CodedUI_2")  
...
-   *Je ne vois aucune propriété quand je sélectionne un contrôle dans mon application, ou je ne vois pas le contrôle dans le Mappage de contrôle d’interface utilisateur.*  
 ...
 ![Assertions du test codé de l’interface utilisateur](~/docs/test/media/codedui_3.png "CodedUI_3")  
...
-   Ajoute une méthode nommée `AssertForAddTwoNumbers` à votre mappage d'interface utilisateur (UIMap.uitest). Ce fichier s'ouvre dans l'Éditeur de test codé de l'interface utilisateur, où vous pouvez modifier les assertions.  
     ...
     ![Modifier l’assertion à l’aide de l’éditeur de test codé de l’interface utilisateur](~/docs/test/media/cuit_editor_assert.png "CUIT_Editor_assert")  
