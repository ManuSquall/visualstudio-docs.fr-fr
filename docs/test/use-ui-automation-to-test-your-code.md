...
## <a name="creating-coded-ui-tests"></a>Création de tests codés de l'interface utilisateur
...
### <a name="starting-and-stopping-the-application-under-test"></a>Démarrage et arrêt de l'application testée
...
-   ![Prérequis](../test/media/prereq.png "Prérequis") Si vous ne souhaitez pas enregistrer les actions de démarrage de votre application durant le test, vous devez la démarrer avant de cliquer sur l’icône **Enregistrer**.  
  
-   ![Prérequis](../test/media/prereq.png "Prérequis") À la fin du test, le processus dans lequel s’exécute le test est arrêté. Si vous avez démarré votre application dans le test, l'application se ferme généralement à l'issue du test.  Si vous ne voulez pas que votre application soit fermée à la fin du test, vous devez ajouter un fichier .runsettings à votre solution et utiliser l'option `KeepExecutorAliveAfterLegacyRun`. Pour plus d’informations, consultez [Configurer des tests unitaires à l’aide d’un fichier .runsettings](../test/configure-unit-tests-by-using-a-dot-runsettings-file.md).  
...
### <a name="validating-the-properties-of-ui-controls"></a>Validation des propriétés des contrôles d'interface utilisateur
...
## <a name="customizing-your-coded-ui-test"></a>Personnalisation de votre test codé de l'interface utilisateur
...
### <a name="coding-ui-control-actions-and-properties"></a>Codage des actions et propriétés du contrôle d'interface utilisateur
...
[FAQ concernant les tests codés de l’interface utilisateur](http://go.microsoft.com/fwlink/?LinkID=230576)
  
## <a name="see-also"></a>Voir aussi

<xref:Microsoft.VisualStudio.TestTools.UITest.Common.UIMap.UIMap> <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert> [Améliorer la qualité du code](../test/improve-code-quality.md) 
