...
Les tests unitaires dans Visual Studio peuvent être configurés à l’aide d’un fichier \*.runsettings. (Le nom du fichier n’a pas d’importance, à condition d’utiliser l’extension « .runsettings ».) Par exemple, vous pouvez modifier la version du .NET Framework sur laquelle les tests sont exécutés, le répertoire dans lequel les résultats des tests sont remis et les données recueillies pendant une série de tests.

Si vous n’avez pas à appliquer de configuration spéciale, vous n’avez pas besoin d’un fichier \*.runsettings. L’usage le plus courant d’un fichier \*.runsettings consiste à personnaliser la [couverture du code](../test/customizing-code-coverage-analysis.md).
