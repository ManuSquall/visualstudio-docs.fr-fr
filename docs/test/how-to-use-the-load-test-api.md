...
Vous pouvez également créer des plug-ins pour les tests de performances web. Pour plus d’informations, voir [Comment : Créer un plug-in de test de performance Web](../test/how-to-create-a-web-performance-test-plug-in.md) et comment : créer un [plug-in au niveau de la demande.](../test/how-to-create-a-request-level-plug-in.md)
...
8. Ajoutez le nouveau plug-in de test de charge à l'aide de l'éditeur de test de charge :
    ...
    3. Dans le volet **Propriétés du plug-in sélectionné**, définissez les valeurs initiales du plug-in à utiliser au moment de l’exécution.
        ...
        > [!NOTE]
        > Vous pouvez exposer autant de propriétés que vous voulez de vos plug-ins. Il suffit de les rendre publics, settable, et d’un type de base comme Integer, Boolean, ou String. Vous pourrez modifier ultérieurement les propriétés du plug-in du test de charge dans la fenêtre **Propriétés**.

9. Exécutez votre test de charge.

     Pour une <xref:Microsoft.VisualStudio.TestTools.LoadTesting.ILoadTestPlugin>mise en œuvre de , voir [Comment: Créer un plug-in test de charge](../test/how-to-create-a-load-test-plug-in.md).
...
- [Comment: Utilisez le test de performance web API](../test/how-to-use-the-web-performance-test-api.md)
