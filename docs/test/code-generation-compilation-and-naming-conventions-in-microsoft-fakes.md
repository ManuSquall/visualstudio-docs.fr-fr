...
Par exemple, le fichier *. substituts* suivant génère des stubs pour les types sous les espaces de noms System et System.IO, mais exclut tout type contenant « handle » dans System :
...
1. Créez une bibliothèque de classes avec la version du runtime .NET. correspondant à vos projets de test. Appelons-la Fakes.Prebuild. Supprimez le fichier *Class1.cs* du projet, qui n’est pas nécessaire.
...
> - Le **type de retour** est ignoré à moins qu’il y ait une ambiguïté de surcharge. Dans le cas d’une ambiguïté de surcharge, le type de retour est ajouté à la fin du nom.
...
- [Isolation du code testé avec les substituts Microsoft](../test/isolating-code-under-test-with-microsoft-fakes.md)
