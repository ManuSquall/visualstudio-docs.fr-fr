...
1. Insérez du code de test dans **TestMethod1** dans le fichier *UnitTest.cs* :
   ...
   La classe <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert> fournit plusieurs méthodes statiques que vous pouvez utiliser pour vérifier les résultats dans les méthodes de test.
...
2. Ajoutez une instruction `using` au fichier *UnitTest.cs* :
...
La méthode **rangetest a échoué** code en dur le dénominateur de la variable de *tolérance* qui est transmise à la méthode <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert>. Si vous envisagez d’ajouter des tests supplémentaires qui utilisent le même calcul de tolérance, l’utilisation d’une valeur codée en dur dans plusieurs emplacements rend le code plus difficile à gérer.
...
> [!TIP]
> Si vous ajoutez une méthode d’assistance à une classe de test et que vous ne souhaitez pas qu’elle apparaisse dans l' **Explorateur de tests**, n’ajoutez pas l’attribut <xref:Microsoft.VisualStudio.TestTools.UnitTesting.TestMethodAttribute> à la méthode.
