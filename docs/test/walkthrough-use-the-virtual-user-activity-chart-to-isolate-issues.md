...
2. Ouvrez la solution **ColorWebApp** qui contient *LoadTest1.loadtest*. Ce test de charge résulte des trois procédures pas à pas répertoriées au début de cette rubrique dans la section des conditions requises.
     ...
     Les étapes restantes dans cette procédure pas à pas supposent une application web nommée ColorWebApp, un test de performance web nommé *ColorWebAppTest.webtest* et un test de charge nommé *LoadTest1.loadtest*.
...
- Dans **l’éditeur de test de charge**, choisissez le bouton **Run** sur la barre d’outils. L'exécution de LoadTest1 démarre.
...
2. Sur le graphique **Temps de réponse de la page**, cliquez avec le bouton droit sur l’une des icônes de violation de seuil et sélectionnez **Accéder au détail de l’utilisateur**.
    ...
    > [!NOTE]
    > Vous pouvez utiliser le bouton **Détails** dans la barre d’outils de **l’Éditeur de test de charge** pour ouvrir également le graphique d’activités des utilisateurs. Toutefois, si vous utilisez **l’option De détail Go to user,** le **graphique d’activité utilisateur virtuel** effectuera automatiquement un zoom avant sur la partie du test que vous avez cliqué sur le graphique.
...
5. Dans le volet **Résultats du filtre**, décochez les cases **Afficher les résultats réussis** et **Erreur HTTP**, mais laissez la case **Erreur de règle de validation** cochée.
     ...
     Le **graphique d’activité des utilisateurs virtuels** affiche uniquement les utilisateurs virtuels qui ont passé plus de 3 secondes sur la page *Red.aspx* comme spécifié par la violation de seuil configurée dans la procédure pas à pas précédente.
...
9. Le test de performance Web ColorWebTest qui est associé au journal s’ouvre dans le **Web Performance Test Results Viewer**. Il permet d'identifier l'endroit où les violations de seuil se sont produites.
