...
ms.topic: "article"
helpviewer_keywords: 
  - "tests codés de l'interface utilisateur, meilleures pratiques"
...
-   Si vous créez des assertions en codant avec l'API, créez une méthode pour chaque assertion dans le cadre de la classe <xref:Microsoft.VisualStudio.TestTools.UITest.Common.UIMap.UIMap> qui se trouve dans le fichier `UIMap.cs`.  Appelez cette méthode depuis votre méthode de test pour exécuter l'assertion.  
...
-   Trouvez la méthode enregistrée qui référence ce contrôle et utiliser le **Générateur de test codé de l'interface utilisateur** pour réenregistrer les actions de cette méthode.  Vous pouvez utiliser le même nom pour la méthode pour écraser les actions existantes.  
...
## Que faire si un processus en arrière\-plan doit être terminé pour que le test puisse continuer ?  
