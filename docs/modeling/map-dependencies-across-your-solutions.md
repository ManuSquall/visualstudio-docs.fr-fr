...
Vous pouvez mapper les dépendances pour le code dans les langages suivants :

- Visual C# ou Visual Basic dans une solution ou des assemblys ( *. dll* ou *. exe*)

- Code C C++ ou code natif ou managé dans C++ des projets visuels, des fichiers d’en `#include`-tête ( *. h* ou) ou des fichiers binaires
...
1. Dans la barre d’outils **Explorateur de solutions** , choisissez **afficher sur la carte** ![de code créer](../modeling/media/createnewgraphfromselectedbutton.gif)un graphique à partir des nœuds sélectionnés. Ou bien, ouvrez le menu contextuel d’un ou d’un groupe d’éléments et choisissez **afficher sur la carte de code**.
...
5. Continuez à développer les éléments qui vous intéressent, juste jusqu’au niveau de la classe et du membre, si nécessaire.
   ...
   Pour afficher les membres qui figurent dans le code, mais qui n’apparaissent pas sur la carte, cliquez sur ![l’icône récupérer à](../modeling/media/dependencygraph_deletednodesicon.png) l’icône des **enfants** récupérer les enfants dans le coin supérieur gauche d’un groupe.
...
7. La carte affiche les relations. Dans cet exemple, la carte affiche les méthodes appelées par `Find` la méthode et leur emplacement dans la solution ou en externe.
