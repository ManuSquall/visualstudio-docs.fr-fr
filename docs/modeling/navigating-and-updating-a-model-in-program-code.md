...
Par défaut, chaque relation d’incorporation a `PropagatesDelete` la valeur true au niveau du rôle cible. La `henry` suppression ne supprime pas `familyTree`le, `familyTree.Delete()` mais supprime tout le `Persons`.
