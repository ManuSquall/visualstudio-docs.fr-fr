...
Par défaut, chaque relation d’incorporation a `PropagatesDelete` true au rôle cible. La suppression de `henry` ne supprime pas le `familyTree`, mais `familyTree.Delete()` supprime toutes les `Persons`.
