...
5. Sélectionner tous les nœuds sur la carte de Code (utilisez _Ctrl_ + _A_, ou utiliser la sélection élastique en appuyant sur la _MAJ_ avant que vous cliquez sur, faites glisser et libérez de la clé.
...
7. Cela montre l’architecture d’application actuel. Déterminer l’architecture à ce que vous souhaitez et modifier le diagramme de dépendances en conséquence.
...
|Créer une nouvelle couche qui contient deux ou plusieurs couches existantes|Sélectionnez les couches, ouvrez le menu contextuel pour votre sélection, puis choisissez **groupe**.|
