...
Lorsque vous mettez à jour un modèle, enserrez les modifications dans un `ILinkedUndoTransaction`, qui regroupe vos modifications dans une transaction. Si les modifications échoue, la transaction entière est restaurée. Si l’utilisateur annule une modification, toutes les modifications sont annulées ensemble.
...
Créer un commentaire en appelant `CreateComment()` sur le conteneur approprié.
