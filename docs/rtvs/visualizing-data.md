...
La génération d’un tracé utilise la fenêtre de tracés « active », et enregistre tout tracé précédent dans l’historique de tracés (voir [Historique de tracés](#plot-history)). Par exemple, entrez `plot(100:1)` et vous verrez que le tracé ci-dessus est remplacé par une ligne vers le bas.
...
Le redimensionnement d’une fenêtre de tracé provoque toujours le réaffichage du tracé, afin de fournir la meilleure qualité d’image. Cette opération s’effectue généralement avant d’exporter le tracé vers un fichier ou vers le Presse-papiers à l’aide des commandes décrites dans la section suivante.
...
Étant donné que les scientifiques des données travaillent souvent avec de nombreux tracés provenant de différents jeux de données, RTVS vous permet de créer autant de fenêtres indépendantes de tracés que vous le souhaitez, qui peuvent être organisées comme vous le souhaitez dans le cadre de Visual Studio ou en dehors de ce cadre. (Pour obtenir des informations générales sur l’ancrage et le redimensionnement des fenêtres, consultez [Personnalisation des dispositions de fenêtres dans Visual Studio](../ide/customizing-window-layouts-in-visual-studio.md).)

Vous pouvez créer une fenêtre de tracé à l’aide de la barre d’outils ou en cliquant sur **Outils R > Tracés > Nouvelle fenêtre de tracés**. La nouvelle fenêtre de tracés devient la fenêtre *active*, qui est l’emplacement où les nouveaux tracés seront affichés. Pour changer la fenêtre active, basculez vers elle et sélectionnez le bouton de barre d’outils Activer la fenêtre de tracés ou cliquez sur **Outils R > Tracés > Activer la fenêtre de tracés**.
