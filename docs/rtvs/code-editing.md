...
De nombreux développeurs aiment écrire du code dans l’éditeur, puis l’envoyer dans la [fenêtre interactive](interactive-repl.md) pour le tester immédiatement (il s’agit de la boucle REPL : read-evaluate-print loop). En appuyant sur Ctrl+Entrée dans l’éditeur R, vous envoyez la ligne de code actuelle à la fenêtre interactive, puis placez le curseur sur la ligne suivante. En appuyant sur Ctrl+Entrée, donc, vous pouvez efficacement parcourir votre code à partir de l’éditeur.
