...
Pour une manipulation et une analyse approfondies, il est souvent utile d’exporter des variables de session au format CSV, ce que vous pouvez faire avec la petite icône Excel (![icône d’exportation Excel](media/variable-explorer-excel-icon.png)) en regard de chaque nœud dans l’Explorateur de variables, ou en double-cliquant sur un élément et en sélectionnant **Ouvrir dans l’application CSV**. Quand vous sélectionnez l’icône, les données sont écrites dans un nouveau fichier CSV dans le dossier `%userprofile%\Documents\RTVS_CSV_Exports`, et ce fichier s’ouvre dans l’application associée à l’extension `.csv`.
