...
* **[Installez Visual Studio](#install-visual-studio)**: Ces tables comprennent les URL de domaine à ajouter à une liste d’autorisations afin que vous ayez accès à tous les composants et charges de travail que vous voulez.

* **[Utilisez Visual Studio et Azure Services](#use-visual-studio-and-azure-services)**: Cette table inclut les URL de domaine à ajouter à une liste d’autorisations et les ports et protocoles à ouvrir afin que vous ayez accès à toutes les fonctionnalités et services que vous souhaitez.
...
| xamarin-downloads.azureedge.net | Android SDK paquets télécharger l’emplacement de la liste |
...
| developerservices2.apple.com | Fourniture Xamarin.iOS |
| developer.apple.com | Fourniture Xamarin.iOS |
| appstoreconnect.apple.com | Fourniture Xamarin.iOS |
| idmsa.apple.com | Fourniture Xamarin.iOS |
| | |
...
Pour vous assurer d’avoir accès à tout ce que vous voulez lorsque vous utilisez Visual Studio ou Azure Services derrière un pare-feu ou un serveur proxy, voici les URL que vous devez ajouter à une liste d’autorisations et les ports et protocoles que vous pourriez vouloir ouvrir.
...
| Active Directory <br>Graph | graph.windows.net | https/443 | Permet de configurer les nouvelles applications Azure Active Directory. Également utilisé par le fournisseur de service connecté Office 365 MSGraph- |
...
| Cloud Explorer | 1. &#60;clusterendpoint&#62; <br>Service Fabric <br>2. &#60;point de terminaison de gestion&#62;<br>Exp Cloud général <br>3. &#60;point de terminaison de graphique&#62;<br>Exp Cloud général<br>4. &#60;point de terminaison du compte de stockage&#62;<br>Nœuds de stockage <br>5. &#60;URL du portail Azure&#62;<br>Exp Cloud général <br>6. &#60;points de terminaison du coffre de clés&#62; <br>Nœuds de machine virtuelle Azure Resource Manager<br>7. &#60;PublicIPAddressOfCluster&#62;<br>Débogage distant et traces ETW Service Fabric | <br>1.https/19080<br>2. https/443<br>3. https/443<br>4. https/443<br>5. https/443<br>6. https/443<br>7.tcp/dynamique | 1. Exemple : test12.eastus.cloudapp.com<br>2. Récupère les abonnements et récupère/gère les ressources Azure<br>3. Récupère les abonnements Azure Stack<br>4. Gère les ressources de stockage (exemple : mystorageaccount.blob.core.windows.net)<br>5. Option de menu contextuelle « Open in Portal » (ouvre une ressource dans le portail Azure)<br>6. Crée et utilise des voûtes clés pour le débogage VM (Exemple : myvault.vault.azure.net) <br><br>7. Alloue dynamiquement le bloc de ports en fonction du nombre de nœuds dans le cluster et les ports disponibles. <br><br>Un bloc de ports tentera d’obtenir trois fois le nombre de nœuds avec un minimum de 10 ports.<br><br>Pour les traces de diffusion en continu, une tentative est effectuée pour obtenir le bloc de ports à partir de 810. Si un de ces blocs de ports est déjà utilisé, une tentative est alors effectuée pour obtenir le bloc suivant et ainsi de suite. (Si l’équilibreur de charge est vide, les ports à partir de 810 sont très probablement utilisés) <br><br>De même pour le débogage, quatre jeux de ces blocs de ports sont réservés : <br>- connectorPort : 30398, <br>- forwarderPort : 31398, <br>- forwarderPortx86 : 31399,<br>- fileUploadPort : 32398<br> |
| Cloud Services | 1. RDP<br><br>2. core.windows.net <br><br>3.  management.azure.com<br> management.core.windows.net <br><br>4. &#42;.blob.core.windows.net <br>&#42;.queue.core.windows.net<br>&#42;.table.core.windows.net <br><br>5. portal.azure.com <br><br>6. &#60;service de cloud utilisateur &#62;.cloudapp.net <br> &#60;Machine virtuelle de l’utilisateur&#62;.&#60;région&#62;.azure.com | 1. rdp/3389 <br><br> 2. https/443 <br><br> 3. https/443 <br><br> 4. https/443 <br><br> 5. https/443 <br><br>6. tcp <br>a) 30398 <br>b) 30400 <br>c) 31398 <br>d) 31400 <br>e) 32398 <br>f) 32400 | 1. Remote Desktop to Cloud Services VM <br><br> 2. Composant de compte de stockage de la configuration de diagnostic privé <br><br> 3. Portail Azure <br><br> 4. Server Explorer - Azure Storage &#42; est nommé compte de stockage du client  <br><br> 5. Liens pour ouvrir le portail &#47; Téléchargez le certificat d’abonnement &#47; Publier le fichier paramètres <br><br>6. a)  Port local du connecteur pour le débogage à distance pour le service de cloud et la machine virtuelle<br> 6. b)  Port public du connecteur pour le débogage à distance pour le service de cloud et la machine virtuelle <br> 6. c)  Port local redirecteur pour le débogage à distance pour le service de cloud et la machine virtuelle <br> 6. d) Port public redirecteur pour le débogage à distance pour le service de cloud et la machine virtuelle  <br> 6. e) Port local chargeur de fichiers pour le débogage à distance pour le service de cloud et la machine virtuelle <br> 6. f) Port public chargeur de fichiers pour le débogage à distance pour le service de cloud et la machine virtuelle |
| Service Fabric | 1. <br>ocs.Microsoft.com<br>aka.ms <br>go.microsoft.com <br><br>2. <br>vssftools.blob.core.windows.net <br>Vault.azure.com <br>Portal.azure.com <br><br> 3. &#42; vault.azure.net<br><br> 4. <br>app.vsaex.visualstudio.com<br>&#42; .vsspsext.visualstudio.com<br>clouds.vsrm.visualstudio.com <br>clouds.visualstudio.com<br>app.vssps.visualstudio.com <br>&#42; .visualstudio.com | https/443 | 1. Documentation <br><br> 2. Créer la fonction Cluster <br><br>3. Le &#42; est le nom azuréen de voûte de clé (Exemple: - test11220180112110108.vault.azure.net  <br><br>  4. Le &#42; est dynamique (Exemple : vsspsextprodch1su1.vsspsext.visualstudio.com) |
| Instantané <br>Débogueur | 1. go.microsoft.com <br>2. management.azure.com <br> 3. &#42;.azurewebsites.net <br> 4. &#42;.scm.azurewebsites.net<br>5. api.nuget.org/v3/index.json <br>6. Adresse IP service/serveurs à distance/FQDN | 1. https/443 <br>2. https/443  <br>3. http/80 <br>4. https/443 <br>5. https/443 <br>6. Concord/<br> 4022 (Visual Studio version dépendante) | 1. Fichier Query .json pour le service d’application taille SKU <br>2. Divers appels Azure RM <br>3. Appel d’échauffement du site via  <br>4. Point de terminaison ciblée du service d’application Kudu du client <br>5. Version d’extension du site de requête publiée en nuget.org <br>6. [Débogage à distance](../debugger/remote-debugging.md) |
...
| Service d’empaquetage | [compte].visualstudio.com <br/> [compte].\*.visualstudio.com <br/> \*.blob.core.windows.net <br/> registry.npmjs.org </br> nodejs.org <br/> dist.nuget.org <br/> nuget.org | https/443 | Les \*.npmjs.org, \*.nuget.org et \*.nodejs.org ne sont nécessaires que pour certains scénarios de tâches de construction (par exemple: NuGet Tool Install, Node Tool Install) ou si vous avez l’intention d’utiliser le public en amont avec vos flux. Les trois autres domaines sont requis pour les fonctionnalités de base du service de création de packages. |
...
| Communauté des développeurs | sendvsfeedback2.azurewebsites.net/api | https/443 | Utilisé pour appeler les API outil de rétroaction communautaire développeur (mes problèmes, recherche, vote, commentaire, soumettre, télécharger, CV) |
| Intellicode (en) | \*.intellicode.vsengsaas.visualstudio.com | https/443 | Utilisé pour appeler intellicode API |
| Live Share | \*.liveshare.vsengsaas.visualstudio.com| https/443 | Utilisé pour appeler live Share API |
| Visual Studio Online | \*.online.visualstudio.com | https/443 | Utilisé pour appeler Visual Studio Online API |
| Acquisition de type automatique JavaScript | registry.npmjs.org | https/443 | Utilisé pour installer des définitions de type TypeScript pour fournir Intellisense pour les bibliothèques JavaScript populaires |
| Service de licence d’abonnements Visual Studio | app.vssps.visualstudio.com/apis/<br/>Licences/ClientsRights | https/443 | Licence pour l’activation en ligne |
| Débogueur | 1. <br>vsdebugger.blob.core.windows.net <br>vsdebugger.azureedge.net <br><br>2. <br>download.visualstudio.com/\*/<br/>onecore.msvsmon. \*.zip<br><br> 3. referencesource.microsoft.com/symbols <br><br> 4. <br>symbols.nuget.org/download/symbols<br><br> 5. visualstudio.com<br><br>6. msdl.microsoft.com/download/symbols | https/443 | 1. <br>Utilisé pour télécharger des bits de débogage pour .NET Core débogage sur Unix / macOS sur SSH <br><br>2. <br>Utilisé pour télécharger des bits de débbugger pour la débugging conteneur Windows Docker à distance<br><br> 3. Utilisé pour .NET étape de source de cadre <br><br> 4. <br>(Si l’utilisateur opte) Utilisé pour télécharger des symboles publiés sur nuget.org serveur de symbole.<br><br> 5. (Si l’utilisateur opts-in) Utilisé pour télécharger des symboles et des binaires MS, pourrait également être nécessaire pour déboguer le code géré dans les décharges |
| Visual Studio Online| \*.online.visualstudio.com | https/443 | Utilisé pour appeler Visual Studio Online API |
| Xamarin Android App Publishing | \*.googleapis.com <br/> play.google.com <br/>accounts.google.com | https/443 | Utilisé pour interagir avec le service Google Play Store pour publier/télécharger Xamarin Android Applications directement à partir de Visual Studio. |
| Azure Container Registry | azurecr.io | https/443 | Registres de conteneurs d’accès hébergés sur Azure, pour la configuration des pipelines CICD |
| | | | |
