...
### <a name="excel-add-in-programming-model"></a>Modèle de programmation de compléments Excel  
 ...
 Pour plus d’informations sur la `ThisAddIn` classe et autres fonctionnalités de Visual Studio que vous pouvez utiliser dans un complément VSTO, consultez [Compléments VSTO de programme](../vsto/programming-vsto-add-ins.md).  
...
|[Contrôles Windows Forms sur une vue d’ensemble des documents Office](../vsto/windows-forms-controls-on-office-documents-overview.md)|Décrit comment ajouter des contrôles Windows Forms aux feuilles de calcul Excel.|  
...
|[Procédure : Ajouter des contrôles à une feuille de calcul lors de l’exécution dans un projet de complément VSTO](../vsto/walkthrough-adding-controls-to-a-worksheet-at-run-time-in-vsto-add-in-project.md)|Montre comment ajouter un bouton Windows Forms, un <xref:Microsoft.Office.Tools.Excel.NamedRange>et une <xref:Microsoft.Office.Tools.Excel.ListObject> à une feuille de calcul lors de l’exécution en utilisant un composant logiciel complément VSTO.|
|[Comprendre les compléments et collaboration](./understanding-coauthoring-and-addins.md)|Décrit les ajustements, que vous devrez peut-être apporter à vos solutions pour prendre en charge la co-création.|  
