...
### <a name="make-the-custom-task-pane-visible"></a>Rendre le volet des tâches personnalisé visible
 Par défaut, le volet de tâches n’est pas visible. Pour afficher le volet de tâches, vous devez affecter à <xref:Microsoft.Office.Tools.CustomTaskPane.Visible%2A> la propriété la **valeur true**.
...
## <a name="Outlook"></a>Programme
 ...
 Pour associer le volet de tâches à un explorateur ou un inspecteur spécifique, <xref:Microsoft.Office.Tools.CustomTaskPaneCollection.Add%2A> utilisez la méthode pour créer le volet de tâches et <xref:Microsoft.Office.Interop.Outlook.Explorer> transmettez l’objet ou <xref:Microsoft.Office.Interop.Outlook.Inspector> au paramètre de *fenêtre* . Pour plus d’informations sur la création de volets de tâches personnalisés, consultez [vue d’ensemble des volets de tâches personnalisés](../vsto/custom-task-panes.md).
...
## <a name="WordAndInfoPath"></a>Word, InfoPath et PowerPoint
 ...
 Pour associer le volet de tâches à une fenêtre de document spécifique, <xref:Microsoft.Office.Tools.CustomTaskPaneCollection.Add%2A> utilisez la méthode pour créer le volet de tâches et <xref:Microsoft.Office.Interop.Word.Window> transmettez un (pour <xref:Microsoft.Office.Interop.InfoPath.WindowObject> Word), (pour InfoPath) ou [DocumentWindow](/previous-versions/office/developer/office-2010/ff762047(v=office.14)) (pour PowerPoint) au paramètre de *fenêtre* . .
