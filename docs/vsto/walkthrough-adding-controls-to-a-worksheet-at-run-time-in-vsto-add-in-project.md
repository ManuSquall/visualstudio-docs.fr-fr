...
---
# <a name="walkthrough-add-controls-to-a-worksheet-at-run-time-in-vsto-add-in-project"></a>Procédure pas à pas : Ajouter des contrôles à une feuille de calcul au moment de l’exécution dans un projet de complément VSTO
...
1. Dans [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], créez un projet de complément VSTO Excel nommé **ExcelDynamicControls**. Pour plus d'informations, voir [Procédure : Créer des projets Office dans Visual](../vsto/how-to-create-office-projects-in-visual-studio.md)Studio.

2. Ajoutez une référence à l’assembly **Microsoft. Office. Tools. Excel. v 4.0. Utilities. dll** . Cette référence est obligatoire pour ajouter par programmation un contrôle Windows Forms à une feuille de calcul, plus loin dans cette procédure pas à pas.
...
3. À partir de l’onglet **contrôles de ruban Office** de la **boîte à outils**, faites glisser un contrôle CheckBox sur **Group1**.
...
4. En C#, vous devez créer un gestionnaire d'événements pour l'événement <xref:Microsoft.Office.Interop.Excel.AppEvents_Event.WorkbookBeforeSave>. Vous pouvez placer ce code dans la méthode `ThisAddIn_Startup`. Pour plus d’informations sur la création de gestionnaires d' [événements, consultez Procédure : Créer des gestionnaires d’événements dans les](../vsto/how-to-create-event-handlers-in-office-projects.md)projets Office. Remplacez la méthode `ThisAddIn_Startup` par le code suivant.
