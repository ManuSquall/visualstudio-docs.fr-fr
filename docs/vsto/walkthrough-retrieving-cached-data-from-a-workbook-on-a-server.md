...
helpviewer_keywords: 
  ...
  - "classeurs (développement Office dans Visual Studio), récupérer des données"
...
---
# Proc&#233;dure pas &#224; pas&#160;: r&#233;cup&#233;ration des donn&#233;es mises en cache d&#39;un classeur sur un serveur
  Cette procédure pas à pas montre comment récupérer des données d'un groupe de données mis en cache dans un classeur Microsoft Office Excel sans démarrer Excel à l'aide de la classe <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>.  
...
-   Lecture des données du groupe de données mis en cache dans le groupe de données de l'application console, sans démarrer Excel.  
...
5.  Cliquez sur **OK**.  
     ...
     [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] ajoute le projet **DataReader** à l'**Explorateur de solutions** et ouvre le fichier de code **Program.cs** ou **Module1.vb**.  
  
## Récupération des données du groupe de données mis en cache à l'aide de l'application console  
 Utilisez la classe <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> de l'application console pour lire les données dans un objet `AdventureWorksLTDataSet` local.  Pour confirmer que le groupe de données local a été initialisé avec les données du groupe mis en cache, l'application affiche le nombre de lignes du groupe de données local.  
  
#### Pour récupérer des données dans le groupe de données mis en cache  
  
1.  Dans l'**Explorateur de solutions**, cliquez avec le bouton droit sur le projet **DataReader**, puis sélectionnez **Ajouter une référence**.  
  
2.  Sous l'onglet de **.NET**, sélectionnez Microsoft.VisualStudio.Tools.Applications.ServerDocument.  
  
3.  Cliquez sur **OK**.  
  
4.  Dans l'**Explorateur de solutions**, cliquez avec le bouton droit sur le projet **DataReader**, puis sélectionnez **Ajouter une référence**.  
...
8.  Ajoutez le code suivant à la méthode `Main`.  Ce code déclare les objets suivants :  
    ...
    -   un objet <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> à utiliser pour accéder au cache de données du classeur.  
        ...
        > [!NOTE]  
        >  Le code suivant suppose que le classeur est enregistré avec l'extension .xlsx.  Si le classeur de votre projet possède une autre extension, modifiez le chemin d'accès de la manière appropriée.  
...
9. Ajoutez le code suivant à la méthode `Main` après le code inséré à l'étape précédente.  Ce code exécute les tâches suivantes :  
    ...
    -   Il affiche le nombre de lignes du groupe de données local, afin de vérifier qu'il comporte des données.  
...
## Test du projet  
 Lorsque vous exécutez l'application console, elle affiche le nombre de lignes du groupe de données local.  
  
#### Pour tester le classeur  
  
1.  Dans l'**Explorateur de solutions**, cliquez avec le bouton droit sur le projet **DataReader**, pointez sur **Déboguer**, puis cliquez sur **Démarrer une nouvelle instance**.  
  
     Vérifiez que l'application indique que le groupe de données local contient 295 lignes.  
