---
title: Limitations de programmation des éléments hôtes et contrôles hôtes
ms.date: 02/02/2017
ms.topic: conceptual
dev_langs:
- VB
...
ms.locfileid: "63447075"
---
# <a name="programmatic-limitations-of-host-items-and-host-controls"></a>Limitations de programmation des éléments hôtes et contrôles hôtes
 ...
 Pour obtenir des informations générales sur les éléments hôtes et contrôles hôtes, consultez [éléments hôtes et héberger de vue d’ensemble des contrôles](../vsto/host-items-and-host-controls-overview.md).
...
## <a name="programmatically-create-host-items"></a>Créer par programmation des éléments hôtes
 ...
 Dans les projets de complément VSTO, vous pouvez créer <xref:Microsoft.Office.Tools.Word.Document>, <xref:Microsoft.Office.Tools.Excel.Workbook>, ou <xref:Microsoft.Office.Tools.Excel.Worksheet> héberger les éléments lors de l’exécution. Pour plus d’informations, consultez [documents Word d’étendre et classeurs Excel dans des Compléments VSTO lors de l’exécution](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md).
...
### <a name="return-types-of-native-office-methods-and-properties"></a>Retourne les types de propriétés et méthodes Office natives
...
### <a name="access-collections-of-host-controls"></a>Collections d’accès de contrôles hôtes
 Le [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] ne fournit pas de collections individuelles pour chaque type de contrôle hôte. Au lieu de cela, utilisez la propriété de contrôles d’un élément hôte pour effectuer une itération dans tous les contrôles managés (contrôles hôtes et contrôles Windows Forms) sur le document ou la feuille de calcul et puis rechercher les éléments qui correspondent au type du contrôle hôte qui que vous intéresse. L’exemple de code suivant permet d’examiner chaque contrôle d’un document Word et de déterminer si le contrôle correspond à <xref:Microsoft.Office.Tools.Word.Bookmark>.
