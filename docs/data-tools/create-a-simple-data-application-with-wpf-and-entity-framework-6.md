...
Cet exemple utilise SQL Server Express LocalDB et la base de données Northwind. Si le fournisseur de données ADO.NET pour ce produit prend en charge Entity Framework, il doit également fonctionner avec d’autres produits de base de données SQL.
...
2.  Ensuite, nous ajoutez le package NuGet pour Entity Framework 6. Dans l’Explorateur de solutions, sélectionnez le nœud du projet. Dans le menu principal, choisissez **projet**, **gérer les Packages NuGet...**
...
3.  Dans le Gestionnaire de NuGet Package, cliquez sur le **Parcourir** lien. Entity Framework est probablement le paquet supérieur dans la liste. Cliquez sur **installer** dans le volet droit et suivez les invites. La fenêtre Sortie indique quand l’installation est terminée.
     ...
     ![Package de NuGet Entity Framework](../data-tools/media/raddata_vs2015_nuget_ef.png)
...
1.  Cliquez avec le bouton droit sur le nœud de projet dans l’Explorateur de solutions et choisissez **ajouter**, **un nouvel élément...** . Dans le volet gauche, sous le nœud C#, choisissez **données** et dans le volet central, sélectionnez **ADO.NET Entity Data Model**.
  ...
  2.  Appelez le modèle `Northwind_model` et cliquez sur OK. Le **Assistant Entity Data Model** s’ouvre. Choisissez **EF Designer à partir de la base de données** puis cliquez sur **suivant**.
...
4.  Dans la page suivante de l’Assistant, nous choisissons les tables, les procédures stockées et autres objets de base de données à inclure dans le modèle Entity Framework. Développez le nœud dbo dans l’arborescence et choisissez Customers, Orders et Order Details. Conservez les valeurs par défaut activées et cliquez sur **Terminer**.
     ...
     ![Choisissez des objets de base de données pour le modèle](../data-tools/media/raddata-choose-ef-objects.png)

5.  L’Assistant génère les classes c# qui représentent le modèle Entity Framework. Les classes sont brut ancien classes c# et qui nous sont databind à l’interface utilisateur WPF. Le fichier .edmx décrit les relations et autres métadonnées qui associe les classes d’objets dans la base de données. Les fichiers .tt sont les modèles T4 qui génèrent le code qui fonctionne sur le modèle et enregistrement les modifications dans la base de données. Vous pouvez voir ces fichiers dans l’Explorateur de solutions sous le nœud Northwind_model :
...
2.  Sélectionnez le client. (Les sources pour les commandes sont automatiquement générés à partir de la propriété de navigation des commandes client.)
...
La disposition par défaut générée par Visual Studio n’est pas idéale pour notre application, nous allons apporter des modifications manuellement dans le code XAML. Nous devons également certaines « forms » (qui sont en fait des grilles) pour permettre à l’utilisateur Ajouter un nouveau client ou une commande. Pour être en mesure d’ajouter un nouveau client et l’ordre, nous avons besoin d’un ensemble de zones de texte qui ne sont pas liés aux données à le `CollectionViewSource`. Nous allons contrôler quels grille l’utilisateur voit à tout moment en définissant la propriété Visible dans les méthodes du gestionnaire. Enfin, nous ajouter un bouton Supprimer pour chaque ligne dans la grille de commandes pour permettre à l’utilisateur supprimer une commande individuelle.
...
### <a name="add-commands-bindings-and-buttons-in-xaml"></a>Ajouter des liaisons, des boutons et des commandes en XAML
