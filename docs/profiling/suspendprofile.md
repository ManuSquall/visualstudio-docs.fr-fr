...
La méthode `SuspendProfile` incrémente le compteur de Suspend\/Resume pour le niveau de profilage spécifié.  
...
## Exemple  
 L'exemple suivant illustre la méthode SuspendProfile.  Cet exemple suppose qu'un appel antérieur à StartProfile a été effectué pour le processus ou le thread identifié par [PROFILE\_CURRENTID](../profiling/profile-currentid.md).  
