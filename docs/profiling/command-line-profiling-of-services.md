...
| **Collecter les données de mémoire .NET** : utilisez la méthode d’échantillonnage ou d’instrumentation pour collecter des données d’allocation de mémoire .NET indiquant le nombre d’objets alloués, ainsi que leur taille. Vous pouvez également collecter des données de durée de vie des objets qui indiquent le nombre et la taille des objets qui sont récupérés dans chaque génération de garbage collection. | -   [Recueillir des données de mémoire .NET](../profiling/collecting-memory-data-from-dotnet-framework-services-by-using-the-profiler-command-line.md) |
| **Recueillir des données de concurrence :** Utilisez la méthode de concurrence pour recueillir des données de contention de ressources et des données d’activité de thread qui vous montre l’utilisation du processeur, la contention de thread, la migration de thread, les retards de synchronisation, les zones d’OIE superposées, et d’autres événements du système. | -   [Recueillir des données de concurrence](../profiling/collecting-concurrency-data-for-a-service-by-using-the-profiler-command-line.md) |
...
|**Profiler des applications ASP.NET**|-   [Profil ASP.NET applications Web](../profiling/command-line-profiling-of-aspnet-web-applications.md)|
