...
dev_langs: 
  ...
  - "C++"
helpviewer_keywords: 
  - "dominateurs, analyseur de mémoire (JavaScript)"
  - "fuites de mémoire (JavaScript)"
  - "mémoire de tas, JavaScript"
  - "fuites, mémoire (JavaScript)"
  - "instantanés, analyseur de mémoire (JavaScript)"
  - "analyseur de mémoire JavaScript"
  - "analyse de mémoire, JavaScript"
  - "analyseur de mémoire, JavaScript"
...
-   Réduire les informations ne pouvant pas être exploitées dans les données de tas JavaScript.  
 ...
 Pour vous guider dans le processus d'identification d'une fuite de mémoire dans une application, consultez [Procédure pas à pas : rechercher une fuite de mémoire \(JavaScript\)](../profiling/walkthrough-find-a-memory-leak-javascript.md).  
  
 Dans cette rubrique :  
  
 [Exécuter l'analyseur de mémoire JavaScript](#Run)   
 [Vérifier l'utilisation de la mémoire](#Check)   
 [Isoler une fuite de mémoire](#Isolate)   
 [Visualiser le résumé dynamique d'utilisation de la mémoire](#LiveMemory)   
 [Visualiser un résumé de l'instantané](#SnapshotSummary)   
 ...
 [Associer le code source aux données d'utilisation de la mémoire](#JSConsoleCommands)   
...
#### Pour exécuter l'analyseur de mémoire  
...
3.  Dans la barre de menus, choisissez **Déboguer**, **Démarrer les diagnostics sans débogage**.  
...
7.  Pour afficher les données que l'analyseur de mémoire rassemble, sélectionnez **Prendre un instantané du tas**. Consultez [Visualiser un résumé de l'instantané](#SnapshotSummary) plus loin dans cette rubrique.  
  
##  <a name="Check"></a> Vérifier l'utilisation de la mémoire  
 Vous pouvez essayer d'identifier les fuites de mémoire grâce à différentes vues de l'analyseur de mémoire JavaScript. Si vous pensez que votre application rencontre des fuites de mémoire, consultez [Isoler une fuite de mémoire](#Isolate) pour une suggestion de flux de travail.  
...
-   [Visualiser un résumé de l'instantané](#SnapshotSummary). Vous pouvez consulter les informations résumées relatives aux instantanés pendant ou après une session de profilage de mémoire. Utilisez les résumés des instantanés pour les lier aux détails d'un instantané et aux vues comparées des instantanés.  
...
##  <a name="Isolate"></a> Isoler une fuite de mémoire  
...
6.  Prenez un instantané du tas de base en sélectionnant **Prendre un instantané du tas**.  
     ...
     ![Instantané de ligne de base](../profiling/media/js_mem_leak_workflow_baseline.png "JS\_Mem\_Leak\_Workflow\_Baseline")  
  
    > [!TIP]
    >  Pour exercer un contrôle plus précis de la synchronisation des instantanés, utilisez la commande [Associer le code source aux données d'utilisation de la mémoire](#JSConsoleCommands) dans votre code.  
...
10. Basculez vers Visual Studio et prenez un troisième instantané.  
     ...
     ![Deuxième et troisième instantané](../profiling/media/js_mem_leak_workflow.png "JS\_Mem\_Leak\_Workflow")  
...
12. Choisissez l'une des options suivantes relatives au filtre **Portée** pour faciliter l'identification des problèmes d'utilisation de la mémoire :  
    ...
    >  Utilisez la vue filtrée des objets restants du précédent instantané pour analyser les fuites de mémoire. Par exemple, si le nombre différentiel d'objets est 205 \/ \-195, cette vue montre les 10 objets restants. Ces derniers peuvent correspondre à des fuites de mémoire.  
     ...
     ![Vue de la comparaison des instantanés affichant les types](../profiling/media/js_mem_snapshot_diff.png "JS\_Mem\_Snapshot\_Diff")  
...
##  <a name="LiveMemory"></a> Visualiser le résumé dynamique d'utilisation de la mémoire  
 ...
 ![Graphique de mémoire Analyseur de mémoire JavaScript](../profiling/media/js_mem_memory_graph.png "JS\_Mem\_Memory\_Graph")  
  
 Si vous avez ajouté des marques utilisateur au code de l'application \(consultez [Associer le code source aux données d'utilisation de la mémoire](#JSConsoleCommands)\), un triangle inversé apparaît dans le graphique d'utilisation de la mémoire pour indiquer quand cette section de code est atteinte.  
...
##  <a name="SnapshotSummary"></a> Visualiser un résumé de l'instantané  
...
>  L'analyseur de mémoire JavaScript force un garbage collection avant chaque instantané. Cela permet de garantir une meilleure cohérence des résultats entre les différentes exécutions.  
 ...
 ![Résumé d'instantané](../profiling/media/js_mem_snapshot_summary.png "JS\_Mem\_Snapshot\_Summary")  
...
##  <a name="SnapshotDetails"></a> Visualiser les détails de l'instantané  
 ...
 ![Vue des détails de l'instantané affichant des problèmes potentiels](../profiling/media/js_mem_snapshot_details.png "JS\_Mem\_Snapshot\_Details")  
...
##  <a name="FoldObjects"></a> Afficher les objets par dominateur  
 ...
 Pour basculer la vue des objets par dominateur, choisisse le bouton **Plier les objets par dominateur**.![Plier les objets dans leurs dominateurs](../profiling/media/js_mem_fold_objects.png "JS\_Mem\_Fold\_Objects")  
...
##  <a name="BuiltInValues"></a> Afficher les objets intégrés  
 ...
 Pour afficher ces objets, choisissez **Afficher les éléments intégrés** dans la liste des paramètres ![Liste déroulante des paramètres dans l'analyseur de mémoire](../profiling/media/js_mem_settings.png "JS\_Mem\_Settings") située dans l’angle supérieur droit du volet.  
...
##  <a name="JSConsoleCommands"></a> Associer le code source aux données d'utilisation de la mémoire  
...
-   `console.takeHeapSnapshot` prend un instantané du tas qui s'affiche dans l'analyseur de mémoire JavaScript. Cette commande est l'une des [Commandes de la console JavaScript](../debugger/javascript-console-commands.md).  
...
```  
 ...
 ![Utilisation d'une marque de profil](../profiling/media/js_mem_performance_marks.png "JS\_Mem\_Performance\_Marks")  
  
##  <a name="Tips"></a> Conseils pour identifier les problèmes de mémoire  
  
-   Suivez le flux de travail dans [Isoler une fuite de mémoire](#Isolate) et utilisez le filtre **Objets créés à partir de l’instantané \#\<numéro\>** dans une vue différentielle pour identifier les objets qui peuvent présenter des fuites de mémoire.  
...
-   Recherchez les objets qui sont conservés par inadvertance dans la mémoire après que l'utilisateur a accédé à une nouvelle page. Il s'agit d'une cause fréquente de problèmes de mémoire. Par exemple :  
    ...
    -   Vous devrez peut\-être supprimer un ou plusieurs détecteurs d'événements. Pour plus d'informations, consultez [Afficher les écouteurs d'événements DOM](../debugger/view-dom-event-listeners.md).  
...
-   Envisagez de modifier temporairement le code pour isoler les problèmes. Vous pouvez, par exemple, décider d'effectuer les opérations suivantes :  
    ...
    -   Utilisez les commandes pour l'analyseur de mémoire, `console.takeSnapshot` et `performance.mark`. \(Consultez [Associer le code source aux données d'utilisation de la mémoire](#JSConsoleCommands).\)  
