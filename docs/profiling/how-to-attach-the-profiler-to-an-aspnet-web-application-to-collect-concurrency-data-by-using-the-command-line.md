...
1. Démarrez le profileur en tapant la commande suivante :
   ...
   | Option | Description |
   | - | - |
   | [/utilisateur](../profiling/user-vsperfcmd.md) **:**:`Domain\`[ ]`UserName` | Spécifie les noms de domaine et d’utilisateur facultatifs du compte qui doit obtenir l’accès au profileur. |
   | [/croix](../profiling/crosssession.md) | Active le profilage des processus dans d’autres sessions ouvertes. |
...
3. Attachez le profileur au processus de travailleur ASP.NET en tapant la commande suivante :**VSPerfCmd/attach:** `PID` [**/targetclr:**`Version`]
   ...
   - [/targetclr](../profiling/targetclr.md) **:** `Version` spécifie la version de l’heure d’exécution de langue commune (CLR) au profil lorsque plus d’une version du temps d’exécution est chargée dans une application. Ce paramètre est facultatif.
...
- [Profilage rapide de site Web avec VSPerfASPNETCmd](../profiling/rapid-web-site-profiling-with-vsperfaspnetcmd.md)
