...
Les outils de profilage [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] prennent en charge la collecte des données d’allocation de mémoire .NET et de durée de vie des objets, ce qui permet de détecter les problèmes de performances liés à la mémoire de votre application.  
...
-   Dans l’ **Explorateur de performances**, cliquez avec le bouton droit sur le nom de la session de performance, puis cliquez sur **Propriétés**.  
 ...
 Les tâches du tableau suivant décrivent les options que vous pouvez spécifier dans la boîte de dialogue *Session de performance***Pages de propriétés** quand vous collectez des données de mémoire .NET.  
