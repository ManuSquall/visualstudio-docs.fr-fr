---
title: 'Ligne de commande Profiler : Composant .NET du client d’instrument, obtenez des données de temps'
ms.date: 11/04/2016
ms.topic: conceptual
...
Ce sujet décrit comment [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] utiliser les outils de ligne de commande Profiling Tools pour instrumenter un composant cadre .NET tel qu’un . *exe* ou . *dll* fichier, et de recueillir des données détaillées de synchronisation.
...
4. Démarrez le profileur. Tapez :
   ...
   | [/croix](../profiling/crosssession.md) | Active le profilage des processus dans d’autres sessions. Cette option est nécessaire si l’application ASP.NET s’exécute dans une autre session. L’idenitifier de session est répertorié dans la colonne **d’identification de session** sur l’onglet **Processus** de Windows Task Manager. **/CS** peut être spécifié comme abréviation de **/crosssession**. |
