...
1. Dans la boîte de dialogue **Nouveau projet** , développez le nœud **Visual C#** ou **Visual Basic** et choisissez le nœud **Windows** , puis développez le nœud **Windows** et choisissez le nœud **Bureau classique** .

1. Dans la liste de modèles, choisissez le modèle **Application WPF** .

1. Dans la boîte de dialogue **Nom** , entrez `ExpenseIt`, puis choisissez le bouton **OK** .
...
1. Si vous utilisez C#, recherchez la ligne `<Window x:Class="ExpenseIt.MainWindow"` et remplacez-la par `<NavigationWindow x:Class="ExpenseIt.MainWindow"`.
     ...
     Notez que lorsque vous remplacez la balise `<Window` par `<NavigationWindow`, IntelliSense remplace automatiquement la balise de fermeture par `</NavigationWindow>`.
...
1. Choisissez les balises `<Grid>` et `</Grid>` et supprimez-les.
...
1. Recherchez la ligne `public partial class MainWindow : Window` et remplacez-la par `public partial class MainWindow : NavigationWindow`.
...
1. Dans la boîte de dialogue **Ajouter un nouvel élément** , choisissez la zone de texte **Nom** et entrez `ExpenseItHome`, puis choisissez le bouton **Ajouter** .
...
1. Dans le concepteur, choisissez l’onglet **MainWindow.xaml** .

1. Recherchez l’élément `Title="ExpenseIt" Height="375" Width="500">` de ligne et ajoutez une propriété `Source="ExpenseItHome.xaml"` .
...
1. Dans la boîte de dialogue **Ajouter un nouvel élément** , choisissez la zone de texte **Nom** et entrez `ExpenseReportPage`, puis choisissez le bouton **Ajouter** .
...
1. Dans la barre de menus, choisissez **Déboguer** > **Démarrer le débogage** (ou appuyez sur **F5**) pour exécuter l’application.
...
1. Fermez l’application pour repasser en mode de conception.
...
1. Dans la boîte de dialogue **Propriétés** , développez le nœud de catégorie **Disposition** , puis affectez à **Marge** les valeurs `10`, `10`, `0`et `10`(celles-ci correspondent aux marges gauche, droite, haut et bas).
...
1. Ajoutez le code XAML suivant à l’élément `Grid` pour créer les définitions de ligne et de colonne :
...
1. Ajoutez le code XAML suivant juste au-dessus de la balise `</Grid>` pour créer les contrôles `Border`, `ListBox` et `Button` :
...
1. Générez et exécutez l’application. L’illustration suivante montre l’apparence au moment de l’exécution des contrôles créés par le code XAML dans cette procédure.
...
1. Dans **l’Explorateur de solutions**, ouvrez le menu contextuel du nœud **ExpenseIt**, puis choisissez **Ajouter** > **Élément existant**.

1. Dans la boîte de dialogue **Ajouter un élément existant** , recherchez l’image **watermark.png** que vous venez d’ajouter, sélectionnez-la, puis choisissez le bouton **Ajouter** .

    > [!NOTE]
    >  Vous devrez peut-être développer la liste **Types de fichiers** et choisir **Fichiers image**.

1. Ouvrez le fichier **ExpenseItHome.xaml** et ajoutez le code XAML suivant juste au-dessus de la balise `</Grid>` pour créer une image d’arrière-plan :
...
1. Recherchez la ligne `<Grid.ColumnDefinitions>` et ajoutez ce qui suit juste en dessous :
...
1. Recherchez la ligne `<Grid.RowDefinitions>` et ajoutez ce qui suit juste en dessous :
...
1. Déplacez les contrôles vers la deuxième colonne en affectant à `Grid.Column` la valeur 1. Déplacez chaque contrôle d’une ligne vers le bas en augmentant chaque valeur `Grid.Row` d’une unité.
    ...
    1. Recherchez la ligne `<ListBox Name="peopleListBox" Grid.Column="0" Grid.Row="1"`. Remplacez `Grid.Column="0"` par `Grid.Column="1"` et `Grid.Row="1"` par `Grid.Row="2"`.
...
1. Si vous générez et exécutez l’application à ce stade, elle doit ressembler à l’illustration suivante :
...
1. Choisissez l’élément `Button` et ajoutez le code XAML suivant immédiatement après l’élément `HorizontalAlignment="Right"` : `Click="Button_Click"`.
...
1. Ouvrez le fichier **ExpenseItHome.xaml.cs** ou **ExpenseItHome.xaml.vb** .

1. Ajoutez le code suivant à la classe `ExpenseItHome` :
...
1. Ajoutez le code XAML suivant entre les balises `<Grid>` et `</Grid>` :
...
1. Ajoutez le code XAML suivant entre les balises `<Application.Resources>` et `</Application.Resources>` :
...
1. Ouvrez **ExpenseItHome.xaml** et remplacez tout ce qui se trouve entre les éléments `<Grid>` et `</Grid>` par le code XAML suivant :
...
1. Ouvrez **ExpenseReportPage.xaml** et remplacez tout ce qui se trouve entre l’élément `<Grid>` et l’élément `</Grid>` final par le code XAML suivant :
...
1. Ajoutez le code XAML suivant :
...
1. À l’intérieur de l’élément `<Grid.Resources>` , ajoutez le code XAML suivant :
...
1. Remplacez l’élément `<ListBox>` existant par le code XAML suivant :
...
1. En C#, ajoutez le constructeur suivant à la classe **ExpenseReportPage** . En Visual Basic, remplacez la classe existante par la suivante :
...
1. Ouvrez **ExpenseItHome.xaml.vb** ou **ExpenseItHome.xaml.cs**.

1. Remplacez le code du gestionnaire d’événements `Click` par le code suivant :
...
1. Remplacez le code XAML pour les éléments **Nom** et **et**`<StackPanel` par le code suivant :
...
1. Ajoutez le code XAML suivant à l’intérieur de l’élément `<Grid>` :
...
1. Remplacez l’élément `<DataGrid>` par le suivant :
...
1. Choisissez une personne, puis le bouton **View** .
...
-   [Vue d’ensemble des médias, de l’animation et du graphisme WPF](/dotnet/framework/wpf/graphics-multimedia/index)
