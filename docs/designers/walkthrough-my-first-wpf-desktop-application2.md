...
4.  Dans la zone de texte **Nom**, entrez `ExpenseIt`, puis choisissez le bouton **OK**.  
...
#### Pour modifier la fenêtre principale  
...
4.  Dans la fenêtre **Propriétés**, développez le nœud de catégorie **Commun** et choisissez la propriété **Titre**, puis entrez `ExpenseIt` et appuyez sur la touche **Entrée**.  
  
     Notez que l’élément **Titre** de la fenêtre XAML est remplacé par la nouvelle valeur. Vous pouvez modifier des propriétés XAML dans la fenêtre XAML ou la fenêtre **Propriétés**, les modifications étant synchronisées.  
  
5.  Dans la fenêtre XAML, affectez à l’élément **Hauteur** la valeur `375` et à la propriété **Largeur** la valeur `500`.  
...
#### Pour modifier le fichier code\-behind \(C\#\)  
...
#### Pour ajouter une fenêtre de détails  
...
2.  Dans la fenêtre **Propriétés**, développez le nœud de la catégorie **Disposition**, puis affectez à **Marge** les valeurs `10`, `10`, `0` et `10` \(celles\-ci correspondent aux marges gauche, droite, haut et bas\).  
...
#### Pour ajouter du code au bouton  
...
#### Pour créer l’interface utilisateur de la page de note de frais  
...
#### Pour appliquer un style aux contrôles  
...
#### Pour lier les données à un contrôle  
...
#### Pour connecter les données aux contrôles  
...
#### Pour mettre à jour l’interface utilisateur avec les modèles de données  
  
1.  Ouvrez **ExpenseReportPage.xaml**.  
  
2.  Remplacez le code XAML pour les éléments `<StackPanel`**Name** et **Department** par le code suivant :  
...
## Voir aussi  
 [Procédure pas à pas : création d’une application de bureau WPF connectée à un service mobile Azure](../Topic/Walkthrough:%20Create%20a%20WPF%20Desktop%20Application%20connected%20to%20an%20Azure%20Mobile%20Service.md)   
