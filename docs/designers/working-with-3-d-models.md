...
description: En savoir plus sur la création de modèles 3D à l’aide de l’éditeur de modèle dans Visual Studio, que vous pouvez utiliser dans votre jeu ou application basé sur DirectX.
...
Les modèles 3D définissent la forme des objets tels qu’ils existent dans une scène 3D. Les modèles peuvent être des objets solitaires de base, des objets complexes qui sont formés à partir de hiérarchies d’objets de base, ou même des scènes 3D intégrales. Un objet 3D est constitué de points dans l’espace 3D (appelés *sommets* ), d’index qui définissent des triangles, des lignes ou d’autres primitives constituées de ces points, ainsi que des attributs qui peuvent s’appliquer à une base par vertex ou par primitive, par exemple, les normales de surface. En outre, certaines informations peuvent s’appliquer par objet, par exemple quel nuanceur et quelles textures donneront à l’objet son apparence unique.
