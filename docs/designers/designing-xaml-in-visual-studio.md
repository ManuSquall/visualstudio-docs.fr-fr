...
-   **Fonctionnalités de débogage de base :** vous pouvez maintenant déboguer dans Blend, notamment en définissant des points d'arrêt dans votre code afin de déboguer votre application en cours d'exécution. Pour maintenir une expérience de débogage cohérente avec Visual Studio, Blend pour Visual Studio inclut la plupart des fenêtres de débogage et barres d'outils de Visual Studio. Certaines fonctionnalités de débogage avancées, telles que les diagnostics et l'analyse du code, sont disponibles uniquement dans Visual Studio. Consultez [Débogage dans Visual Studio](../debugger/debugging-in-visual-studio.md).  
...
|À|Visual Studio|Blend pour Visual Studio|En savoir plus|  
|--------|-------------------|-----------------------------|----------------------|  
