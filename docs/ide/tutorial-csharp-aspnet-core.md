...
### <a name="tour-your-solution"></a>Visite guidée de votre solution
 ...
 2. Ouvrez le fichier *HomeController.cs* dans le dossier **Controllers**.
 ...
 3. Affichez le fichier *HomeController.cs*.
 ...
 6. La solution contient également un dossier *wwwroot*, qui correspond à la racine de votre site web. Vous pouvez placer le contenu de site statique, tel que les bibliothèques JavaScript, les feuilles CSS et les images, directement aux emplacements où vous souhaiteriez qu’il soit lors du déploiement du site.
...
2. Visual Studio lance une fenêtre de navigateur. Sélectionnez **About**.
...
3. Laissez la fenêtre du navigateur ouverte et revenez dans Visual Studio. Ouvrez *Controllers/HomeController.cs* s’il n’est pas déjà ouvert.
...
4. Définissez un point d’arrêt sur la première ligne de la méthode **About**. Pour ce faire, cliquez dans la marge ou placez le curseur sur la ligne et appuyez sur **F9**.
...
5. Revenez dans le navigateur et actualisez la page **About**. Cette opération déclenche le point d’arrêt dans Visual Studio.

6. Dans Visual Studio, placez la souris sur le membre **ViewData** pour afficher ses données.
...
7. Supprimez le point d’arrêt de l’application en appliquant la même méthode que celle utilisée pour l’ajouter.

8. Ouvrez *Views/Home/About.cshtml*.

   ![Sélectionnez About.cshtml dans l’Explorateur de solutions](../ide/media/csharp-aspnet-solution-explorer-view-about.png)

9. Remplacez le texte **« additional »** par **« changed »** et enregistrez le fichier.

   ![Remplacez le texte « additional » par « changed »](../ide/media/csharp-aspnet-about-cshtml-code-change.png)

10. Revenez à la fenêtre du navigateur pour voir le texte mis à jour. (Si vous ne voyez pas le texte que vous avez modifié, actualisez le navigateur.)
...
11. Choisissez le bouton **Arrêter le débogage** dans la barre d’outils pour arrêter le débogage. (Vous pouvez également appuyer sur **Maj**+**F5**, ou choisir **Déboguer** > **Arrêter le débogage** dans la barre de menus.)

   ![Sélectionnez le bouton Arrêter le débogage dans la barre d’outils](../ide/media/csharp-aspnet-stop-debugging.png)

## <a name="next-steps"></a>Étapes suivantes

Félicitations ! Vous avez terminé ce didacticiel. Nous espérons que vous en avez appris un peu plus sur C#, ASP.NET Core et l’IDE Visual Studio. Pour voir l’application en cours d’exécution sur un serveur public, sélectionnez le bouton suivant.

> [!div class="nextstepaction"]
> [Déployer l’application sur Azure App Service](..//deployment/quickstart-deploy-to-azure.md)

Vous pouvez également découvrir comment utiliser le framework MVC (Model-View-Controller) dans ASP.NET Core en suivant le tutoriel [Bien démarrer avec ASP.NET Core MVC et Visual Studio](/aspnet/core/tutorials/first-mvc-app/start-mvc?tabs=aspnetcore2x).
