...
1. Pour ajouter des références aux contrôles Label dans votre formulaire, utilisez le code suivant.
     ...
     Lorsqu’une variable ne suit pas un objet, elle est réglée `null` à une `Nothing` valeur réservée spéciale : en C et en Visual Basic. Au démarrage du programme, `firstClicked` et `secondClicked` ont donc la valeur `null` ou `Nothing`, ce qui signifie que les variables n'effectuent aucun suivi.
...
4. Choisissez le contrôle d'étiquette suivant, et notez que rien ne se passe. Le programme est déjà garder une trace de `firstClicked` la première étiquette `null` que le `Nothing` joueur a choisi, n’est donc pas égal à en C ou dans Visual Basic. Lorsque votre instruction `if` vérifie si `firstClicked` est égal à `null` ou `Nothing`, elle découvre que ce n'est pas le cas et n'exécute pas les instructions dans l'instruction `if`. Ainsi, seule la première icône choisie devient noire, et les autres icônes sont invisibles, comme le montre l’image suivante.

     ![Jeu de combinaisons affichant une icône](../ide/media/express_tut4step5.png)<br/>
***Jeu correspondant*** *affichant une icône*

     Vous allez résoudre cette situation à l'étape suivante du didacticiel en ajoutant un contrôle **Minuterie**.

## <a name="to-continue-or-review"></a>Pour continuer ou examiner

- Pour passer à l’étape suivante tutoriel, voir **[Étape 6: Ajouter une minuterie](../ide/step-6-add-a-timer.md)**.
