...
---
# Guide de l&#39;administrateur Help Viewer
...
La visionneuse d’aide vous permet de gérer les installations locales d’aide pour les environnements réseau avec ou sans accès à Internet.  Le contenu d’aide locale est configuré sur la base de l’ordinateur.  Par défaut, les utilisateurs doivent disposer des droits d’administrateur pour mettre à jour leur installation d’aide locale.  
 ...
 \<*chemin d’accès*\>\\HlpCtntmgr.exe \/operation \<*argument*\> \/catalogname \<*nom*\> \/locale \<*paramètres régionaux*\> \/sourceuri \<*chemin d’accès .msha ou URL*\>  
...
## Déploiement de contenu d’aide locale à partir d’Internet  
 ...
 \\\<*chemin d’accès*\>\\v2.2\\HlpCtntmgr.exe \/operation \<*nom*\> \/catalogname \<*nom du catalogue*\> \/locale \<*paramètres régionaux*\>  
...
-   La source par défaut de l’aide sera toujours en ligne.  
    ...
    > [!TIP]
    >  Vous pouvez modifier la source par défaut de l’aide en modifiant la clé de Registre HKEY\_LOCAL\_MACHINE\\Software\\Microsoft\\VisualStudio\\14.0\\help\\UseOnlineHelp.  Pour plus d’informations, consultez [Substitutions Help Content Manager](../ide/help-content-manager-overrides.md).  
  
-   Les clients seront toujours invités à installer le contenu d’aide de base lors du premier démarrage de Visual Studio.  Vous pouvez désactiver cette invite en modifiant la clé de Registre HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Wow6432Node\\Microsoft\\VisualStudio\\14.0\\Help\\DisableFirstRunHelpSelection.  
  
### Exemple  
 L’exemple suivant installe le contenu en anglais pour Visual Studio sur un ordinateur client.  
  
##### Pour installer le contenu en anglais à partir d’Internet  
...
## Déploiement du contenu d’aide locale préinstallé sur les ordinateurs clients  
...
-   Les utilisateurs doivent disposer des droits d’administrateur pour mettre à jour, ajouter ou supprimer le contenu d’aide locale une fois celui\-ci installé.  
    ...
    > [!TIP]
    >  Si les utilisateurs ne disposent pas des droits d’administrateur, nous vous recommandons de désactiver l’onglet Gérer le contenu de la visionneuse d’aide.  Pour plus d’informations, consultez [Substitutions Help Content Manager](../ide/help-content-manager-overrides.md).  
...
-   Les clients seront toujours invités à installer le contenu d’aide de base lors du premier démarrage de Visual Studio.  Pour plus d’informations, consultez [Substitutions Help Content Manager](../ide/help-content-manager-overrides.md).  
  
### Créer l’ensemble de contenu  
...
##### Pour désinstaller l’aide locale  
  
1.  Dans la visionneuse d’aide, choisissez l’onglet **Gérer le contenu**.  
  
2.  Sous **Documentation disponible**, accédez à l’ensemble de documents de Visual Studio  
  
3.  Sélectionnez **Supprimer** en regard de chaque sous\-élément.  
  
4.  Sélectionnez **Démarrer** pour désinstaller.  
  
5.  Accédez à *n* :\\ProgramData\\Microsoft\\HelpLibrary2\\Catalogs\\VisualStudio12 et vérifiez que le dossier ne contienne que le fichier catalogType.xml.  
...
##### Pour créer un package du contenu  
...
4.  Tapez le texte suivant :  
     ...
     Xcopy %SYSTEMDRIVE%\\ProgramData\\Microsoft\\HelpLibrary2 \<*nom du dossier*\>\\ \/y \/e \/k \/o  
...
1.  Créez un partage réseau et copiez le contenu d’aide à cet emplacement.  
     ...
     Par exemple, copiez le contenu du dossier c:\\VS12Help vers \\\\myserver\\VS12Help.  
