...
### <a name="BKMK_Override_call_stack_filtering"></a>Remplacer le filtrage de pile des appels  
...
## <a name="BKMK__NET_Framework_Just_My_Code"></a>.NET Framework Uniquement mon code  
...
### <a name="BKMK_NET_Stepping_behavior"></a>Comportement pas à pas  
...
### <a name="BKMK_NET_Breakpoint_behavior"></a>Comportement du point d’arrêt  
...
### <a name="BKMK_NET_Exception_behavior"></a>Comportement de l’exception  
...
### <a name="BKMK_CPP_Stepping_behavior"></a>Comportement pas à pas  
...
### <a name="BKMK_CPP_Exception_behavior"></a>Comportement de l’exception  
...
|`Name`|Obligatoire. Une expression régulière mise en forme selon ECMA-262 spécifiant le nom complet de la fonction concernée. Par exemple :<br /><br /> `<Name>MyNS::MyClass.*</Name>`<br /><br /> indique au débogueur que toutes les méthodes de `MyNS::MyClass` doivent être considérées comme du code non-utilisateur. La correspondance tient compte de la casse.|  
|`Module`|Ce paramètre est facultatif. Une expression régulière mise en forme selon ECMA-262 spécifiant le chemin d'accès complet au module contenant la fonction. La recherche de correspondance ne respecte pas la casse.|  
|`Action`|Obligatoire. Une des valeurs suivantes (respectant la casse) :<br /><br /> -   `NoStepInto` : indique au débogueur d’effectuer un pas à pas principal dans la fonction correspondante.<br />-   `StepInto` : indique au débogueur d’effectuer un pas à pas détaillé dans les fonctions correspondantes, en substituant tout autre `NoStepInto` pour les fonctions mises en correspondance.|  
...
|`Company`|Ce paramètre est facultatif. Le nom de la société qui publie le module incorporé dans le fichier exécutable. Vous pouvez utiliser cet attribut pour lever l'ambiguïté entre les modules.|  
...
|`Module`|Ce paramètre est facultatif. Le nom ou le chemin d'accès complet au module qui contient la fonction. Vous pouvez utiliser cet attribut pour lever l'ambiguïté entre des fonctions du même nom.|  
...
- Le script qui est exécuté en passant une chaîne à la fonction `eval` fournie par l’hôte est classé comme **MyCode**.  
  
- Le script qui est exécuté en passant une chaîne au constructeur `Function` est classé comme **LibraryCode**.  
...
- Le script exécuté en passant une chaîne aux fonctions `setTimeout`, `setImmediate`ou `setInterval` est classé comme **UnrelatedCode**.  
...
### <a name="BKMK_JS_Stepping_behavior"></a>Comportement pas à pas  
...
### <a name="BKMK_JS_Breakpoint_behavior"></a>Comportement du point d’arrêt  
...
### <a name="BKMK_JS_Exception_behavior"></a>Comportement de l’exception  
