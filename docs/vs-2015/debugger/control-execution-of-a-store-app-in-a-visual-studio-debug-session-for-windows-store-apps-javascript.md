---
title: Contrôler l’exécution d’une application de Store dans la session de débogage pour les applications du Windows Store (JavaScript) | Microsoft Docs
ms.date: 11/15/2016
...
3. **Remplacer le contenu de la page par l’exemple de code.** Supprimez tout le contenu du fichier `default.js` . Suivez ce lien : [Exemple de navigation code (JavaScript) débogueur](../debugger/debugger-navigation-sample-code-javascript.md), puis copiez le code répertorié dans la section JavaScript dans le Presse-papiers. (Choisissez **retour** dans le navigateur ou la visionneuse d’aide pour revenir à cette page de démarrage rapide.) Dans l'éditeur Visual Studio, collez le code dans le fichier `default.js` maintenant vide. Appuyez sur **Ctrl+S** pour enregistrer le fichier.
...
##  <a name="BKMK_Set_and_run_to_a_breakpoint__step_into_a_function__and_examine_program_data"></a> Définir et exécuter jusqu'à un point d'arrêt, effectuer un pas à pas détaillé dans une fonction et examiner les données de programme
 La méthode la plus courante pour démarrer une session de débogage consiste à choisir **démarrer le débogage** à partir de la **déboguer** menu (clavier : F5). L’application démarre et continue de s’exécuter jusqu’à ce qu’un point d’arrêt soit atteint, que vous suspendiez manuellement l’exécution, qu’une exception se produise ou que l’application se termine.
...
1. **Définir un point d'arrêt.** Définissez un point d'arrêt au niveau de l'instruction `callTrack = "module function";` qui se produit juste après l'appel à `app.start()`. Sélectionnez la ligne dans la marge grisée de l’éditeur de code source (clavier : Positionnez le curseur sur la ligne et choisissez le **F9** clé).
...
2. **Lancer l'exécution jusqu'au point d'arrêt.** Démarrez la session de débogage en choisissant **démarrer le débogage** sur le **déboguer** menu (clavier : F5).
...
3. **Effectuer un pas à pas détaillé dans la fonction.** Sur le **déboguer** menu, choisissez **pas à pas détaillé** (clavier : **F11**).
...
7. **Arrêter le débogage.** Sur le **déboguer** menu, choisissez **arrêter le débogage** (clavier : **MAJ + F5**). La session de débogage se termine.
...
2.  **Lancer l'exécution jusqu'au point d'arrêt.** Démarrez la session de débogage en choisissant **démarrer le débogage** sur le **déboguer** menu (clavier : F5). Le débogueur interrompt l'exécution au point d'arrêt.

3.  **Effectuer un survol de la ligne de code.** Sur le **déboguer** menu, choisissez **pas à pas principal** (clavier : F10). Le débogueur exécute l'instruction `var callTrack = "module function"` de la même manière que l'exécution pas-à-pas.
...
6.  **Sortie de code d'une fonction.** Appuyez sur la touche **F11** pour effectuer un pas à pas détaillé dans la fonction `example2_b` . Notez que `example2_b` n'est pas très différent de `example2_a`. Pour sortir de la fonction, choisissez **pas à pas sortant** sur le **déboguer** menu (clavier : **MAJ + F11**). Notez que la variable `callTrack` indique que `example2_b` a été exécutée et que le débogueur est revenu au point où `example2` reprend.
...
2.  **Lancer l'exécution jusqu'au point d'arrêt.** Démarrez la session de débogage en choisissant **démarrer le débogage** sur le **déboguer** menu (clavier : **F5**). Le débogueur suspend l’exécution au point d’arrêt dans la fonction `module` .

3.  **Effectuer un pas à pas détaillé dans la fonction example3.** Choisissez **pas à pas détaillé** sur le **déboguer** menu (clavier : **F11**) pour déplacer vers le point d’entrée de la `example3` (fonction). Continuez le pas à pas détaillé dans la fonction jusqu'à ce que vous ayez itéré une ou deux boucles du bloc `for` . Notez que l'exécution pas-à-pas de toutes les 1 000 itérations peut prendre du temps.
...
7.  **Lancer l'exécution jusqu'au curseur.** Sélectionnez la ligne `callTrack += "->example3";` , puis **exécuter jusqu’au curseur** dans le menu contextuel (clavier : **Ctrl+F10**). Le débogueur exécute les itérations de boucle, puis suspend l'exécution au niveau de la ligne.

8.  **Arrêter le débogage.** Sur le **déboguer** menu, choisissez **arrêter le débogage** (clavier : **MAJ + F5**). La session de débogage se termine.
...
2.  **Lancer l'exécution jusqu'au point d'arrêt.** Démarrez la session de débogage en choisissant **démarrer le débogage** sur le **déboguer** menu (clavier : **F5**). Le débogueur interrompt l'exécution au point d'arrêt. Effectuez un pas à pas détaillé jusqu’à la ligne.
...
7.  **MultiplyByA et examiner ses local et des variables de portée** le reste de `example4_a` et dans la ligne `var x = multiplyByA(b);`.
...
9. **Arrêter le débogage.** Sur le **déboguer** menu, choisissez **arrêter le débogage** (clavier : **MAJ + F5**). La session de débogage se termine.
...
3.  **Ouvrir la fenêtre Pile des appels.** Sur le **déboguer** menu, choisissez **Windows**, puis choisissez **pile des appels** (clavier : Alt+7). Notez que la fenêtre Pile des appels montre deux fonctions :
...
## <a name="see-also"></a>Voir aussi
 [Démarrer une session de débogage (JavaScript)](../debugger/start-a-debugging-session-for-store-apps-in-visual-studio-javascript.md) [Guide de démarrage rapide : Navigation (JavaScript) dans le débogueur](../debugger/control-execution-of-a-store-app-in-a-visual-studio-debug-session-for-windows-store-apps-javascript.md) [Guide de démarrage rapide : Déboguer du code HTML et CSS](../debugger/quickstart-debug-html-and-css.md) [déclencheur suspendre, reprendre, événements et d’arrière-plan pour le Windows Store)](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md) [déboguer des applications dans Visual Studio](../debugger/debug-store-apps-in-visual-studio.md)
