...
|[Lier la tâche](../msbuild/link-task.md)|Encapsule l’outil Éditeur de liens Visual C++ (link.exe).|  
...
|[SetEnv, tâche](../msbuild/setenv-task.md)|Définit ou supprime la valeur d’une variable d’environnement spécifiée.|  
|[Tâche VCMessage,](../msbuild/vcmessage-task.md)|Enregistre les messages d’avertissement et les messages d’erreur lors de la génération.|  
...
|[Tâche XSD](../msbuild/xsd-task.md)|Encapsule l’outil Définition du schéma XML (xsd.exe), qui génère des fichiers de schéma ou de classe à partir d’une source.|  
