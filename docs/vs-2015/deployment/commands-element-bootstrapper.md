...
L' `Commands` élément implémente les tests décrits par les éléments situés sous l' `InstallChecks` élément, et déclare le package que le [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] programme d’amorçage doit installer en cas d’échec du test.  
