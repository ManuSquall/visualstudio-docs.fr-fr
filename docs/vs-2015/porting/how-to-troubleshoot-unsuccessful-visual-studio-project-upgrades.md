...
2. Recherchez les chemins d’accès aux fichiers qui peuvent être incorrects, en particulier ceux qui contiennent un [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] numéro de version.
...
## <a name="the-project-does-not-build-because-references-are-not-valid"></a>Le projet n’est pas généré, car les références ne sont pas valides
 Lorsque vous effectuez une mise à niveau [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] , vous pouvez également mettre à niveau la [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] version. Si votre projet contient des références qui sont supprimées dans la version plus récente [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] , elles risquent de ne pas être résolues correctement. Cela est particulièrement probable pour les références qui incluent des numéros de version, par exemple, `Microsoft.VisualStudio.Shell.Interop.8.0` .

 Si votre code contient de nombreuses références non valides, la solution la plus simple consiste à utiliser la fonctionnalité de multi-ciblage de [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] pour cibler une version antérieure du [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] .
...
3. Sélectionnez la valeur **Framework cible** correcte. Vous pouvez également modifier la valeur de l' `<TargetFrameworkVersion>` élément directement dans le fichier projet.

   Si vous souhaitez que votre projet s’exécute dans la version mise à niveau [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] , vous devez mettre à jour les références pour le projet, ainsi que toutes les `Imports` `Using` instructions ou qui appellent les références. Si votre projet est chargé dans l’IDE, vous pouvez mettre à jour les références à l’aide de **Explorateur de solutions** ou de la boîte de dialogue **Gestionnaire de références** .
