...
- Réduire les informations ne pouvant pas être exploitées dans les données de tas JavaScript.  
  ...
  [Afficher le résumé de l’utilisation de la mémoire dynamique](#LiveMemory)   
  [Afficher un résumé des instantanés](#SnapshotSummary)   
  [Afficher les détails de l’instantané](#SnapshotDetails)   
  [Afficher une comparaison d’instantanés](#SnapshotDiff)   
  [Afficher les objets par dominateur](#FoldObjects)   
  [Filtrer les données par identificateur](#Filter)   
  [Rechercher un objet dans l’arborescence d’objets](#ShowInRootsView)   
  [Afficher les références d’objets partagés](#References)   
  [Afficher les objets intégrés](#BuiltInValues)   
  [Enregistrer les fichiers de session de diagnostic](#Save)   
...
## <a name="run-the-javascript-memory-analyzer"></a><a name="Run"></a> Exécuter l’analyseur de mémoire JavaScript  
...
## <a name="check-memory-usage"></a><a name="Check"></a> Vérifier l'utilisation de la mémoire  
...
- [Affichez un résumé des instantanés](#SnapshotSummary). Vous pouvez consulter les informations résumées relatives aux instantanés pendant ou après une session de profilage de mémoire. Utilisez les résumés des instantanés pour les lier aux détails d'un instantané et aux vues comparées des instantanés.  
  
    > [!TIP]
    > En général, les vues comparées des instantanés fournissent les informations les plus utiles sur les fuites de mémoire.  
  
- [Affichez les détails](#SnapshotDetails)de l’instantané. Affiche des données détaillées relatives à l'utilisation de la mémoire pour un instantané unique.  
  
- [Affichez une comparaison d’instantanés](#SnapshotDiff). Affiche les valeurs comparées pour différents instantanés. Ces vues affichent les différences de taille d'objets et de nombre d'objets.  
  
## <a name="isolate-a-memory-leak"></a><a name="Isolate"></a> Isolate a memory leak  
...
6. Prenez un instantané du tas de base en sélectionnant **Prendre un instantané du tas**.  
    ...
    ![Instantané de ligne de base](../profiling/media/js-mem-leak-workflow-baseline.png "JS_Mem_Leak_Workflow_Baseline")  
...
## <a name="view-live-memory-usage-summary"></a><a name="LiveMemory"></a> Afficher le résumé de l’utilisation de la mémoire dynamique  
 ...
 ![Graphe de mémoire Analyseur de mémoire JavaScript](../profiling/media/js-mem-memory-graph.png "JS_Mem_Memory_Graph")  
...
## <a name="view-a-snapshot-summary"></a><a name="SnapshotSummary"></a> Afficher un résumé des instantanés  
...
## <a name="view-snapshot-details"></a><a name="SnapshotDetails"></a> Visualiser les détails de l'instantané  
...
## <a name="view-a-snapshot-diff"></a><a name="SnapshotDiff"></a> Visualiser une comparaison d'instantanés  
...
## <a name="view-objects-by-dominator"></a><a name="FoldObjects"></a> Afficher les objets par dominateur  
 ...
 Pour basculer la vue des objets par dominateur, choisisse le bouton **Plier les objets par dominateur** . ![Plier les objets dans leurs dominateurs](../profiling/media/js-mem-fold-objects.png "JS_Mem_Fold_Objects")  
  
 Pour plus d'informations sur les dominateurs, consultez [Visualiser les détails de l'instantané](#SnapshotDetails).  
  
## <a name="filter-data-by-identifier"></a><a name="Filter"></a> Filtrer les données par identificateur  
...
## <a name="find-an-object-in-the-object-tree"></a><a name="ShowInRootsView"></a> Rechercher un objet dans l’arborescence d’objets  
...
## <a name="view-shared-object-references"></a><a name="References"></a> Afficher les références d’objets partagés  
...
## <a name="show-built-in-objects"></a><a name="BuiltInValues"></a> Afficher les objets intégrés  
 ...
 Pour afficher ces objets, choisissez **afficher les composants intégrés** dans la liste ![déroulante](../profiling/media/js-mem-settings.png "JS_Mem_Settings") paramètres de la liste de paramètres&#45;liste déroulante dans l’analyseur de mémoire, dans le coin supérieur droit du volet.  
  
## <a name="save-diagnostic-session-files"></a><a name="Save"></a> Enregistrer les fichiers de diagnostic de session  
...
## <a name="associate-source-code-with-memory-usage-data"></a><a name="JSConsoleCommands"></a> Associer le code source aux données d’utilisation de la mémoire  
...
## <a name="tips-for-identifying-memory-issues"></a><a name="Tips"></a> Conseils pour identifier les problèmes de mémoire  
  
- Suivez le flux de travail décrit dans [isoler une fuite de mémoire](#Isolate) et utilisez les **objets restants à partir de l’instantané # \<number> ** Filter dans une vue diff pour identifier les candidats potentiels aux fuites de mémoire.  
...
- Recherchez les objets qui sont conservés par inadvertance dans la mémoire après que l'utilisateur a accédé à une nouvelle page. Il s'agit d'une cause fréquente de problèmes de mémoire. Par exemple :  
  ...
  - l'utilisation incorrecte de la fonction [URL.CreateObjectUrl](https://msdn.microsoft.com/library/windows/apps/hh453196.aspx) peut entraîner ce problème.  

  - Certains objets peuvent fournir une méthode `dispose` et des recommandations d'utilisation. Par exemple, appelez `dispose` sur [WinJS.Binding.List](https://msdn.microsoft.com/library/windows/apps/Hh700774.aspx) si vous appelez la méthode `createFiltered` de la liste, avant de quitter une page.  
...
- Consultez [Gestion de la mémoire dans les applications du Windows Store](https://msdn.microsoft.com/magazine/jj651575.aspx).  
