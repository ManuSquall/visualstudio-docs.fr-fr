...
## <a name="package-manifest-schema"></a>Schéma de manifeste du package
 L’élément racine du fichier XML manifeste est `<PackageManifest>` , avec un seul attribut `Version` , qui est la version du format du manifeste. Si des modifications majeures sont apportées au format, le format de version sera modifié. Cette rubrique décrit le format du manifeste version 2,0, qui est spécifié dans le manifeste en affectant `Version` à l’attribut la valeur version = "2.0".
