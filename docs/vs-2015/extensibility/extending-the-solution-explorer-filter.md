...
1. Créez un projet VSIX nommé `FileFilter` . Ajoutez un modèle d’élément de commande personnalisé nommé **FileFilter**. Pour plus d’informations, consultez [création d’une extension à l’aide d’une commande de menu](../extensibility/creating-an-extension-with-a-menu-command.md).  
...
4. Dans FileFilter.cs, supprimez le placement de commande et le code de gestion du constructeur FileFilter. Le résultat doit ressembler à ceci :  
    ...
    ```  
     ...
     Supprimez également la méthode méthode ShowMessageBox ().  
  
5. Dans FileFilterPackage, CS, remplacez le code dans la méthode Initialize () par ce qui suit :  
...
4. Lorsque vous cliquez sur le bouton, tous les fichiers doivent être éliminés par filtrage et vous devriez voir que « tous les éléments ont été filtrés à partir de l’affichage ». dans le Explorateur de solutions.
