...
## <a name="accessing-physical-views"></a>Accès aux vues physiques  
 ...
 Pour permettre à votre éditeur d’accéder à toutes les vues physiques disponibles, vous devez créer une chaîne de vue physique unique pour chaque type d’objet de vue de document que votre fabrique d’éditeur peut créer. Par exemple, la [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] fabrique d’éditeur peut créer des objets de vue de document pour une fenêtre de code et une fenêtre de concepteur de formulaires.  
