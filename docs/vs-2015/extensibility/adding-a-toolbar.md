...
## <a name="creating-an-extension-with-a-toolbar"></a>Création d’une extension avec une barre d’outils  
 Créez un projet VSIX nommé `IDEToolbar` . Ajoutez un modèle d’élément de commande de menu nommé **ToolbarTestCommand**. Pour plus d’informations sur la façon de procéder, consultez [création d’une extension à l’aide d’une commande de menu](../extensibility/creating-an-extension-with-a-menu-command.md).  
