...
2. Lorsque le projet s’ouvre, ajoutez un modèle d’élément de commande personnalisé et nommez-le **dynamicMenu**. Pour plus d’informations, consultez [création d’une extension à l’aide d’une commande de menu](../extensibility/creating-an-extension-with-a-menu-command.md).  
...
## <a name="implementing-the-handlers"></a>Implémentation des gestionnaires  
...
## <a name="implementing-the-command-id-match-predicate"></a>Implémentation du prédicat de correspondance d’ID de commande  
  
1. Implémentez maintenant le prédicat match. Nous devons déterminer deux choses : tout d’abord, si l’ID de commande est valide (il est supérieur ou égal à l’ID de commande déclaré) et Deuxièmement, s’il spécifie un projet possible (il est inférieur au nombre de projets dans la solution).  
...
## <a name="setting-the-vspackage-to-load-only-when-a-solution-has-multiple-projects"></a>Définition du VSPackage à charger uniquement quand une solution contient plusieurs projets  
...
## <a name="testing-the-set-startup-project-command"></a>Test de la commande Set Startup Project  
