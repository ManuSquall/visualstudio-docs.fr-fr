...
1. Créez un projet VSIX nommé `TestToolWindowSearch` à l’aide d’une fenêtre outil nommée **TestSearch**. Si vous avez besoin d’aide pour cette opération, consultez [Creating an Extension with a Tool Window](../extensibility/creating-an-extension-with-a-tool-window.md).  
...
3. Dans le fichier TestSearchControl.xaml.cs, ajoutez l’instruction using suivante :  
...
1. Dans le fichier TestSearch.cs, ajoutez les `using` instructions suivantes :  
...
5. Testez les nouveaux paramètres en reconstruisant la solution et en commençant à debugb.  
...
2. Dans la `TestSearchTask` classe, supprimez les marques de commentaire de la ligne MatchCase dans la `OnStartSearch` méthode :  
