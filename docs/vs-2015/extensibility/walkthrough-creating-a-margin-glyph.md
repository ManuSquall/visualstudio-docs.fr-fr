...
2. Ajoutez un élément de projet classifieur de l’éditeur. Pour plus d’informations, consultez [création d’une extension avec un modèle d’élément d’éditeur](../extensibility/creating-an-extension-with-an-editor-item-template.md).  
  
3. Supprimez les fichiers de classe existants.  
  
## <a name="defining-the-glyph"></a>Définition du glyphe  
 Définissez un glyphe en implémentant l' <xref:Microsoft.VisualStudio.Text.Editor.IGlyphFactory> interface.  
...
## <a name="defining-a-todo-tag-and-tagger"></a>Définition d’une balise TODO et d’une balise  
 Définissez la relation entre l’élément d’interface utilisateur que vous avez défini dans les étapes précédentes et la marge des indicateurs en créant un type et une balise tag, puis en l’exportant à l’aide d’un fournisseur de balises.  
...
5. Un cercle bleu clair qui a un contour bleu foncé doit apparaître dans la marge de l’indicateur à gauche de la fenêtre de code.
