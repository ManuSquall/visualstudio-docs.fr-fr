...
- Un kit SDK est une collection de fichiers que Visual Studio traite comme un seul élément de référence. La boîte de dialogue **Gestionnaire de références** répertorie tous les kits de développement logiciel (SDK) pertinents pour le projet qui est ouvert lorsque vous Affichez cette boîte de dialogue. Quand vous ajoutez un kit SDK à un projet, vous pouvez accéder à tout son contenu dans IntelliSense, la **Boîte à outils**, les concepteurs, l’**Explorateur d’objets**, MSBuild, le déploiement, le débogage et l’empaquetage. Pour plus d’informations sur les kits SDK, consultez [Création d’un Kit de développement logiciel](../extensibility/creating-a-software-development-kit.md).
