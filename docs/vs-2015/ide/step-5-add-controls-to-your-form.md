...
6. Modifiez la propriété **ColumnSpan** du PictureBox pour qu'il s'étende sur deux colonnes. Choisissez le contrôle PictureBox et affectez à sa propriété **ColumnSpan** la valeur **2**. Vous voulez également que le PictureBox affiche un cadre vide lorsqu'il ne contient aucun élément. Affectez la valeur **Fixed3D** à sa propriété **BorderStyle**.
    ...
    > [!NOTE]
    > Si vous ne voyez pas de propriété **ColumnSpan** pour votre contrôle PictureBox, il est probable que le contrôle PictureBox ait été ajouté au formulaire et non au contrôle TableLayoutPanel. Pour résoudre ce problème, choisissez le contrôle PictureBox, supprimez-le, choisissez le contrôle TableLayoutPanel, puis ajoutez un nouveau contrôle PictureBox.
