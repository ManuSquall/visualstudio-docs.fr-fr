...
[Afficher des détails de build supplémentaires dans le Fenêtre Sortie](../ide/walkthrough-building-an-application.md#BKMK_outputdetails)
...
3. Nommez la solution `IntroWPFcsharp` pour Visual C#.
     ...
     ![Boîte de dialogue Nouveau projet, Exemple Visual Basic](../ide/media/buildwalk-newprojectdlgintrotowpfsamplevb.png "BuildWalk_NewProjectdlgIntrotoWPFsampleVB")

4. Choisissez le bouton **OK**.

## <a name="create-a-custom-build-configuration"></a><a name="BKMK_CreateBuildConfig"></a> Créer une configuration de build personnalisée
...
3. Dans la boîte de dialogue **Nouvelle configuration de solution**, nommez la nouvelle configuration `Test`, copiez les paramètres de la configuration Debug existante, puis cliquez sur le bouton **OK**.
...
6. Choisissez le bouton **OK**.
   ...
   ![Gestionnaire de configurations avec configuration de test](../ide/media/buildwalk-configmanagertestconfig.png "BuildWalk_ConfigManagerTestconfig")
   ...
   ![Barre d'outils Standard option Configuration de solution](../ide/media/buildwalk-standardtoolbarsolutioncongfig.png "BuildWalk_StandardToolbarSolutionCongfig")

## <a name="build-the-application"></a><a name="BKMK_building"></a> Générer l’application
...
## <a name="hide-compiler-warnings"></a><a name="BKMK_hidewarning"></a> Masquer les avertissements du compilateur
...
4. Générez la solution.
     ...
     ![Fenêtre Sortie, Visual C&#35; les avertissements de génération](../ide/media/buildwalk-visualcsharpbuildwarnings.png "BuildWalk_VisualCsharpBuildWarnings")
...
4. Générez la solution.
   ...
   ![Fenêtre Sortie, Avertissement sur la génération Visual Basic](../ide/media/buildwalk-visualbasicbuildwarnings.png "BuildWalk_VisualBasicBuildWarnings")
...
## <a name="display-additional-build-details-in-the-output-window"></a><a name="BKMK_outputdetails"></a> Afficher des détails de build supplémentaires dans le Fenêtre Sortie

Vous pouvez modifier la quantité d’informations relatives au processus de génération qui s’affichent dans la fenêtre **Sortie**. Le niveau de détail de la build est généralement défini sur minimal, ce qui signifie que la fenêtre **sortie** affiche uniquement un résumé du processus de génération, ainsi que tous les avertissements ou erreurs de priorité élevée. Vous pouvez afficher plus d’informations sur la build à l’aide de la [boîte de dialogue Options, projets et solutions, générer et exécuter](../ide/reference/options-dialog-box-projects-and-solutions-build-and-run.md).
...
5. Générez la solution, puis passez en revue les informations contenues dans la fenêtre **Sortie**.
    ...
    Dans la build Visual Basic, [/nowarn](https://msdn.microsoft.com/library/7ebf2106-0652-4fdc-bf60-70fc86465d83) n’inclut pas les avertissements spécifiques à exclure, donc aucun avertissement ne s’affiche.

   > [!TIP]
   > Vous pouvez effectuer une recherche dans le contenu de la fenêtre **sortie** si vous affichez la boîte de dialogue **Rechercher** en appuyant sur les touches Ctrl + F.
...
1. Ouvrez le **Concepteur de projet**.
     ...
     ![Menu Afficher, commande Pages de propriété](../ide/media/buildwalk-viewpropertypages.png "BuildWalk_ViewPropertyPages")
...
5. Dans la zone de texte **Chemin de sortie de la génération**, spécifiez un chemin réseau.
    ...
    > [!IMPORTANT]
    > Une boîte de message peut s’afficher, indiquant que le partage réseau que vous avez spécifié n’est pas un emplacement approuvé. Si vous faites confiance à l’emplacement que vous avez spécifié, cliquez sur le bouton **OK** dans la boîte de message.
...
- [Vue d’ensemble de la précompilation d’un projet d’application Web ASP.NET](https://msdn.microsoft.com/b940abbd-178d-4570-b441-52914fa7b887)
