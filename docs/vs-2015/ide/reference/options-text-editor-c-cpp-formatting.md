 ...
 **Afficher les blocs de code inactifs** Le code qui est inactif en raison de déclarations de `#ifdef` est coloriée différemment pour vous aider à l’identifier.
 ...
 **Mettre en retrait les accolades** Vous pouvez configurer la façon dont les accolades sont alignées lorsque vous appuyez sur entrée après avoir commencé un bloc de code, par exemple une fonction ou une boucle de `for`. Les accolades peuvent être alignées avec le premier caractère du bloc de code ou mises en retrait.
