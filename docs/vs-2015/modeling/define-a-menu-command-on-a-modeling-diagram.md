...
Vous trouverez la dernière version de cette rubrique dans [définir une commande de menu sur un diagramme de modélisation](https://docs.microsoft.com/visualstudio/modeling/define-a-menu-command-on-a-modeling-diagram).  
...
### <a name="navigating-and-updating-the-model"></a>Navigation et mise à jour du modèle  
 Les éléments du modèle UML sont tous disponibles via l’API. À partir de la sélection actuelle ou de la racine du modèle, vous pouvez accéder à tous les autres éléments. Pour plus d’informations, consultez [naviguer dans le modèle UML](../modeling/navigate-the-uml-model.md) et [Programming with the UML API](../modeling/programming-with-the-uml-api.md).  
