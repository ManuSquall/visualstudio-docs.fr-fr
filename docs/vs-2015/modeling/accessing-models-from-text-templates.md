...
- Bien que le langage dans lequel les fragments de code sont écrits est C#, vous pouvez générer du texte de n’importe quel type. Vous pouvez également écrire le code dans [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] en ajoutant la propriété `language="VB"` à la `template` directive.

- Pour déboguer le modèle, ajoutez `debug="true"` à la `template` directive. Le modèle s’ouvre dans une autre instance de [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] si une exception se produit. Si vous souhaitez vous arrêter dans le débogueur à un point spécifique dans le code, insérez l’instruction `System.Diagnostics.Debugger.Break();`
