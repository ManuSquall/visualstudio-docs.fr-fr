...
### <a name="simple-control-flows"></a>Flux de contrôle simples
 ...
 ![Un workflow de contrôle simple](../modeling/media/uml-actovsimple.png "UML_ActOvSimple")
...
|8|**Commentaireaire**|Fournit des informations supplémentaires sur les éléments auxquels il est lié.|
|9|**Action appeler un comportement**|Action qui est définie plus en détail sur un autre diagramme d'activités.<br /><br /> -   **IsSynchronous** -si la valeur est true, l’action attend jusqu’à ce que l’activité se termine.<br />**comportement** de -    : activité appelée.|
...
### <a name="concurrent-flows"></a>Flux simultanés
 ...
 ![Diagramme d’activités montrant un flot simultané](../modeling/media/uml-actovconcurrent.png "UML_ActovConcurrent")
...
### <a name="DataFlow"></a>Flux de données
 ...
 ![Diagramme d’activités montrant le workflow](../modeling/media/uml-actovdata.png "UML_ActOvData")

||||
|-|-|-|
|**Automatiques**|**Élément**|**Description**|
|15|**Nœud d’objet**|Représente des données qui passent le long d'un flux.<br /><br /> **ordre** de -    : la manière dont plusieurs jetons sont stockés.<br />-   **Selection** : appelle un processus, qui peut être défini dans un autre diagramme, qui filtre les données.<br />-    limite**supérieure** -0 indique que les données doivent passer directement le long du fluide.  \* indique que les données peuvent être stockées dans le Flow.<br />**type** de -    : type des objets stockés et transmis.|
|16|**Broche d’entrée**|Représente des données qu'une action peut recevoir quand elle s'exécute.<br /><br /> **type** de -   -le type des objets transmis.|
|17|**Broche de sortie**|Représente des données qu'une action produit quand elle s'exécute.<br /><br /> **type** de -   -le type des objets transmis.|
|18|**Nœud de paramètre d’activité**|Nœud d'objet via lequel des données peuvent être reçues ou  produites par l'activité.<br /><br /> Utilisé quand l'activité représentée par le diagramme est appelée à partir d'une autre activité ou quand le diagramme décrit une opération ou une fonction.<br /><br /> **type** de -   -le type des objets transmis.|
|(non affiché)|**Workflow d’objet**|Connecteur qui montre le flux de données entre des actions et des nœuds d'objets.<br /><br /> Pour créer un workflow d’objet, utilisez l’outil **connecteur** pour lier une broche d’entrée ou de sortie ou un nœud d’objet à un autre élément.<br /><br /> -   **Selection** : appelle un processus, qui peut être défini dans un autre diagramme, qui filtre les données.<br />**transformation** de -    : appelle un processus, qui peut être défini dans un autre diagramme, qui transforme les données.<br />-   **IsMulticast** : indique qu’il peut y avoir plusieurs objets ou composants destinataires.<br />-   **IsMultireceive** : indique que des entrées peuvent être reçues à partir de plusieurs objets ou composants.|
