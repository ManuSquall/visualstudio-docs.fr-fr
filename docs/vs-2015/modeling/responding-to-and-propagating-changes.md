---
title: Réponse et propagation des modifications | Microsoft Docs
ms.date: 11/15/2016
...
|Règles de sélection|Les règles de sélection limitent spécifiquement ce que l’utilisateur peut sélectionner.|[Guide pratique pour accéder à et contraindre la sélection actuelle](../modeling/how-to-access-and-constrain-the-current-selection.md)|
...
## <a name="comparing-rules-and-store-events"></a>**Comparaison des règles et des événements de magasin**
...
- **Abonnement à des événements** Avant de pouvoir vous abonner à un événement, créez un gestionnaire d’événements et un délégué. Utilisez ensuite la <xref:Microsoft.VisualStudio.Modeling.Store.EventManagerDirectory%2A>property pour vous abonner à l’événement. Pour plus d’informations, consultez les [gestionnaires d’événements propagent les modifications en dehors du modèle](../modeling/event-handlers-propagate-changes-outside-the-model.md).

- **Annuler des modifications** Lorsque vous annulez une transaction, les événements sont déclenchés, mais les règles ne sont pas appliquées. Si une règle modifie une valeur et que vous annulez cette modification, la valeur est réinitialisée à la valeur d’origine au cours de l’opération d’annulation. Quand un événement est déclenché, vous devez rétablir manuellement la valeur d’origine. Pour en savoir plus sur les conversions et les annulations, consultez [Comment : utiliser des transactions pour mettre à jour le modèle](../modeling/how-to-use-transactions-to-update-the-model.md).

- **Passage d’arguments d’événement à des règles et des événements** Un paramètre `EventArgs` qui contient des informations sur la façon dont le modèle a changé est passé aux deux événements et règles.

## <a name="see-also"></a>Voir aussi
 [Comment : intercepter un clic sur une forme ou un décorateur](../modeling/how-to-intercept-a-click-on-a-shape-or-decorator.md) [écriture de code pour personnaliser un langage spécifique à un domaine](../modeling/writing-code-to-customise-a-domain-specific-language.md)
