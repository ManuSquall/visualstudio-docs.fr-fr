---
title: Comment... avec les modèles de texte | Microsoft Docs
ms.date: 11/15/2016
...
### <a name="read-another-project-file-from-a-template"></a>Lire un autre fichier projet à partir d’un modèle
 Pour lire un fichier du même [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] projet que le modèle :
...
### <a name="invoke-methods-from-a-template"></a>Appeler des méthodes à partir d’un modèle
 Si les méthodes existent déjà, par exemple dans les classes de [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] standard :
...
- Envisagez de générer du code à partir d’un modèle UML. Le code ne doit pas nécessairement refléter le UML directement. Par exemple, vous n’êtes pas obligé de générer une classe pour chaque classe dans le modèle UML. Au lieu de cela, vous pouvez utiliser le diagramme de classes UML pour représenter un site Web et générer une page Web à partir de chaque classe UML. Choisissez le type de diagramme le plus proche de vos besoins. Par exemple, choisissez diagrammes d’activités pour représenter tout type de Workflow. Vous pouvez définir des stéréotypes pour ajouter des informations appropriées à votre application pour chaque type d’élément.

     La génération à partir d’un modèle UML vous permet de dessiner et de modifier le modèle dans un formulaire schématique, mais sans avoir à concevoir votre propre type de diagramme, comme vous le feriez avec un DSL.
...
### <a name="get-data-from-includevsprvsincludesvsprvs-mdmd"></a>Récupérer des données à partir d' [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]
 Pour utiliser les services fournis dans [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], définissez l’attribut `hostSpecific` et chargez l’assembly `EnvDTE`. Exemple :
...
- Autre nom pour les fonctionnalités de modèle de texte [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] décrites ici. La version précédente, qui n’a pas été publiée, était une abréviation pour « transformation de modèle de texte ».
