...
### <a name="read-another-project-file-from-a-template"></a>Lire un autre fichier projet à partir d’un modèle
 Pour lire un fichier à partir du même [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] projet que le modèle :
...
### <a name="invoke-methods-from-a-template"></a>Appeler des méthodes à partir d’un modèle
 Si les méthodes existent déjà, par exemple dans les [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] classes standard :
...
### <a name="get-data-from-vsprvs"></a>Récupérer des données à partir de [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]
 Pour utiliser les services fournis dans [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] , définissez l' `hostSpecific` attribut et chargez l' `EnvDTE` assembly. Par exemple :
...
- Autre nom pour les [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] fonctionnalités de modèle de texte décrites ici. La version précédente, qui n’a pas été publiée, était une abréviation pour « transformation de modèle de texte ».
