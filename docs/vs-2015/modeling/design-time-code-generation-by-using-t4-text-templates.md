---
title: Génération de code au moment du design à l’aide de modèles de texte T4 | Microsoft Docs
ms.date: 11/15/2016
...
Les modèles de texte T4 au moment du design vous permettent de générer du code de programme et d’autres fichiers dans votre projet [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. En général, vous écrivez les modèles afin qu’ils fassent varier le code qu’ils génèrent en fonction des données d’un *modèle*. Un modèle est un fichier ou une base de données qui contient des informations clés concernant les spécifications de votre application.
...
1. Créez un projet de [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ou ouvrez-en un existant.
...
- Cliquez sur **transformer tous les modèles** dans le menu **générer** . Cette action transforme tous les modèles dans la solution [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
...
- Dans **Explorateur de solutions**, dans le menu contextuel de n’importe quel fichier, choisissez **exécuter un outil personnalisé**. Appliquez cette méthode pour transformer un sous-ensemble de modèles sélectionné.
  ...
  Vous pouvez aussi configurer un projet [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] pour que les modèles soient exécutés quand les fichiers de données qu'ils lisent ont changé. Pour plus d’informations, consultez [régénération automatique du code](#Regenerating).
...
## <a name="Converting"></a>Conversion d’un fichier existant en modèle
 L'un des avantages des modèles, c'est que leur apparence se rapproche des fichiers qu'ils génèrent, avec en plus du code de programme inséré. Cela nous suggère une méthode utile pour créer un modèle. Commencez par créer un fichier ordinaire en tant que prototype, tel qu’un fichier [!INCLUDE[csprcs](../includes/csprcs-md.md)], puis introduisez progressivement du code de génération qui fait varier le fichier résultant.
...
5. Insérez les lignes suivantes au début du fichier :
    ...
    ```
     ...
     Si vous souhaitez écrire le code génération de votre modèle en [!INCLUDE[vbprvb](../includes/vbprvb-md.md)], affectez la valeur `language` au lieu de `"VB"` à l'attribut `"C#"`.
