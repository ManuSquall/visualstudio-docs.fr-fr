...
À partir d'un modèle UML, vous pouvez générer du code de programme, des schémas, des documents, des ressources et autres artefacts de tout genre. Une méthode pratique pour générer des fichiers texte à partir d’un modèle UML consiste à utiliser des [modèles de texte](../modeling/code-generation-and-t4-text-templates.md). Ces derniers vous permettent d'intégrer du code de programme dans le texte que vous souhaitez générer.

 Il existe trois scénarios principaux :

- [Génération de fichiers à partir d’une commande de menu ou d’un](#Command) mouvement. Vous définissez une commande [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] qui est disponible sur les modèles UML.
...
```
 ...
 Pour créer ce fichier, cliquez avec le bouton droit sur le projet dans Explorateur de solutions, pointez sur **Ajouter**, puis cliquez sur **nouvel élément**. Sélectionnez **modèle de texte prétraité**. Le nom de fichier de cet exemple doit être **VdmGen.TT**. La propriété de l' **outil personnalisé** du fichier doit être **valeur TextTemplatingFilePreprocessor**. Pour plus d’informations sur les modèles de texte prétraités, consultez [génération de texte au moment de l’exécution avec des modèles de texte T4](../modeling/run-time-text-generation-with-t4-text-templates.md).
...
- La propriété outils personnalisés du fichier de modèle doit être **TextTemplatingFileGenerator**et l’extension de nom de fichier doit être. TT.
...
- Le modèle est exécuté lorsque vous l’enregistrez et lorsque vous cliquez sur **transformer tous les modèles** dans la barre d’outils Explorateur de solutions.
...
- Vous pouvez également définir un processeur de directive spécialisé qui vous permet de fournir des paramètres au processus de génération de texte. Pour plus d’informations, consultez Personnalisation de la [transformation de texte T4](../modeling/customizing-t4-text-transformation.md).
...
6. Examinez le code dans le fichier auxiliaire. Il doit contenir une classe pour chaque classe UML dans le modèle.
   ...
   1. Dans un projet de Visual Basic, cliquez sur **Afficher tous les fichiers** dans la barre d’outils Explorateur de solutions.
...
#### <a name="content-of-the-text-template-file"></a>Contenu du fichier de modèle de texte
 Le texte suivant est placé dans le fichier **. TT** . Cet exemple génère des classes dans un fichier C# à partir des classes UML dans le modèle. Toutefois, vous pouvez générer des fichiers de n'importe quel type. Le langage du fichier généré n'est pas lié à celui dans lequel le code de modèle de texte est écrit.
