...
|Outils rémanents : l’utilisateur peut double-cliquer sur n’importe quel outil pour créer plusieurs formes ou connecteurs successivement.|Dans l’Explorateur DSL, sélectionnez le nœud `Editor`. Dans le Fenêtre Propriétés, Set **utilise des éléments de boîte à outils rémanents**.|
