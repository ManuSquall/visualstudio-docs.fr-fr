...
---
# <a name="switchlttgt-activity-designer"></a>&lt;Concepteur d' &gt; activités Switch T
L'activité <xref:System.Activities.Statements.Switch%601> évalue une expression spécifiée et exécute l'activité à partir d'une collection d'activités dont la clé associée correspond à la valeur obtenue de l'évaluation.
 ...
 Le concepteur d’activités **Switch \<T> ** est utilisé pour créer et configurer une <xref:System.Activities.Statements.Switch%601> activité dans le [!INCLUDE[wfd1](../includes/wfd1-md.md)] .
...
## <a name="how-to-use-the-switcht-activity-designer"></a>Comment utiliser le concepteur d' \<T> activités Switch
 Le concepteur d’activités **Switch \<T> ** se trouve dans la catégorie **Flow Control** de la **boîte à outils**, accessible en cliquant sur l’onglet **boîte à outils** dans [!INCLUDE[wfd2](../includes/wfd2-md.md)] (ou en sélectionnant **barre d’outils** dans le menu **affichage** , ou encore en appuyant sur CTRL + ALT + X). Après la suppression dans le [!INCLUDE[wfd2](../includes/wfd2-md.md)] , il affiche la boîte de dialogue **Sélectionner les types** pour permettre à l’utilisateur de spécifier le type générique *T* utilisé dans l' <xref:System.Activities.Statements.Switch%601> activité. La valeur par défaut est **Int32**. Une fois que le type générique *T* a été sélectionné, un concepteur de **commutateur \<T> ** est ajouté au concepteur de Workflow.

 Voici les propriétés du concepteur **de \<T> commutateur** . Toutes ces propriétés peuvent être modifiées dans la grille des propriétés. Certaines peuvent également être modifiées dans l'aire du concepteur.
