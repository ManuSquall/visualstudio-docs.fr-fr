...
## <a name="using-the-statemachine-activity-designer"></a>Utilisation du concepteur d'activités StateMachine
 Pour ajouter une <xref:System.Activities.Statements.StateMachine> activité, faites glisser le concepteur d’activités **StateMachine** de la section **machine à États** de la **boîte à outils** et déposez-le sur l’aire de conception [!INCLUDE[wfd1](../includes/wfd1-md.md)] . Pour ajouter un État enfant à cette <xref:System.Activities.Statements.StateMachine> activité, faites glisser un <xref:System.Activities.Statements.State> ou <xref:System.Activities.Core.Presentation.FinalState> depuis la **boîte à outils** et déposez-le sur le **StateMachine**.
