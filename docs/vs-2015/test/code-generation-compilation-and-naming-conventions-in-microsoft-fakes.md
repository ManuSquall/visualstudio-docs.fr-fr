...
- [Assistance](#BKMK_Guidance)

## <a name="code-generation-and-compilation"></a><a name="BKMK_Code_generation_and_compilation"></a> Génération et compilation de code

### <a name="configuring-code-generation-of-stubs"></a><a name="BKMK_Configuring_code_generation_of_stubs"></a> Configuration de la génération du code des stubs
...
### <a name="type-filtering"></a><a name="BKMK_Type_filtering"></a> Filtrage de type
...
### <a name="stubbing-concrete-classes-and-virtual-methods"></a><a name="BKMK_Stubbing_concrete_classes_and_virtual_methods"></a> Classes concrètes et méthodes virtuelles de stub
...
### <a name="internal-types"></a><a name="BKMK_Internal_types"></a> Types internes
...
- Vous devez ajouter les clés publiques de l’assembly de test et Fakes aux attributs **InternalsVisibleToAttribute** des assemblys ayant fait l’objet d’un shim. Voici l'apparence des exemples d'attributs dans le code d'assembly ayant fait l'objet d'un shim quand cet assembly porte un nom fort : 
  ...
  Si l'assembly ayant fait l'objet d'un shim porte un nom fort, le framework Fakes signe automatiquement fortement l'assembly Fakes généré. Vous devez signer avec un nom fort l'assembly de test. Consultez [Création et utilisation d’assemblys avec nom fort](https://msdn.microsoft.com/library/ffbf6d9e-4a88-4a8a-9645-4ce0ee1ee5f9).
...
```
 ...
 Vous pouvez spécifier une autre clé publique pour l’assembly de simulation, telle qu’une clé que vous avez créée pour l’assembly corrigé, en spécifiant le chemin d’accès complet au fichier **. snk** qui contient l’autre clé comme `KeyFile` valeur d’attribut dans l' `Fakes` \\ `Compilation` élément du fichier **. resubstitut** . Par exemple :
...
### <a name="optimizing-build-times"></a><a name="BKMK_Optimizing_build_times"></a> Optimisation de la durée de génération
...
### <a name="avoiding-assembly-name-clashing"></a><a name="BKMK_Avoiding_assembly_name_clashing"></a> Prévention du conflit de nom d’assembly
...
## <a name="fakes-naming-conventions"></a><a name="BKMK_Fakes_naming_conventions"></a> Simuler des conventions d’affectation de noms

### <a name="shim-type-and-stub-type-naming-conventions"></a><a name="BKMK_Shim_type_and_stub_type_naming_conventions"></a> Conventions d’affectation des noms des types shim et stub
 **Espaces de noms**
...
### <a name="shim-delegate-property-or-stub-delegate-field-naming-conventions"></a><a name="BKMK_Shim_delegate_property_or_stub_delegate_field_naming_conventions"></a> Conventions de nommage de la propriété déléguée shim ou du champ délégué stub
...
- Si la méthode est générique, `Of`*n* est ajouté, où *n* est le nombre d’arguments de méthode générique.
  ...
  Les **noms de méthode spéciaux** , tels que les accesseurs get ou Set de propriété, sont traités comme décrit dans le tableau suivant.
...
### <a name="parameter-type-naming-conventions"></a><a name="BKMK_Parameter_type_naming_conventions"></a> Conventions d’affectation des noms de type de paramètre
...
|**Type tableau**`T[]`|`TArray`|
|Un type **tableau multidimensionnel**`T[ , , ]`|`T3`|
|Un type **pointeur**`T*`|`TPtr`|
|**Type générique**`T<R1, …>`|`TOfR1`|
|**Argument de type générique** `!i` de type`C<TType>`|`Ti`|
|**Argument de méthode générique** `!!i` de méthode`M<MMethod>`|`Mi`|
|**Type imbriqué**`N.T`|`N` est ajouté, puis `T`|

### <a name="recursive-rules"></a><a name="BKMK_Recursive_rules"></a> Règles récursives
