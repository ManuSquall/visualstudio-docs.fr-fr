 ...
 **Les tests unitaires utilisent uniquement les fonctions qui sont exportées à partir du code testé :** Ajoutez un projet de test distinct. Dans ce projet de test, ajoutez une référence au projet testé.

 Passez à la procédure [Pour référencer des fonctions exportées depuis le projet de test](#projectRef).

 **Le code testé est généré sous la forme d’un fichier. exe :** Ajoutez un projet de test distinct. Liez-le au fichier objet de sortie.

 Passez à la procédure [Pour lier les tests aux fichiers objets ou bibliothèques](#objectRef).

 **Les tests unitaires doivent utiliser des fonctions et des données privées, et le code testé peut être généré sous la forme d’une bibliothèque statique :** Modifiez le projet testé pour qu’il soit compilé en un fichier. lib. Ajoutez un projet de test distinct qui référence le projet testé.
 ...
 **Les tests unitaires doivent utiliser des fonctions et des données privées, et le code doit être généré sous la forme d’une bibliothèque de liens dynamiques (dll) :** Ajoutez des tests unitaires dans le même projet que le code du produit.
