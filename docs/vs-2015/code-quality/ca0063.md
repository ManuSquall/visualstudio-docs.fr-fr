...
Examinez la valeur de l’attribut **path** pour tous les éléments **include** . Les chemins d’accès include peuvent utiliser des chemins d’accès relatifs au fichier de l’ensemble de règles parent/actuel, aux variables d’environnement et aux chemins absolus. Exemple :
