...
- Types qui ne peuvent pas être instanciés et qui définissent uniquement des méthodes `static` (`Shared` dans Visual Basic).

  Si vous appliquez <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute?displayProperty=fullName> à l’assembly en cours d’analyse, cette règle ne se produira pas sur les constructeurs marqués comme `internal`, car vous ne pouvez pas déterminer si un champ est utilisé par un autre assembly de `friend`.

  Même si vous ne parvenez pas à contourner cette limitation dans [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] l’analyse du code, le FxCop autonome externe se produira sur les constructeurs internes si chaque assembly `friend` est présent dans l’analyse.
...
- Constructeur privé pour les types qui ciblent les versions de [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 1,0 et 1,1.

- Modificateur `static` (`Shared` dans Visual Basic) pour les types qui ciblent [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)].
