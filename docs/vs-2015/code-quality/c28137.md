---
title: C28137 | Microsoft Docs
ms.date: 11/15/2016
ms.prod: visual-studio-dev14
ms.technology: vs-ide-code-analysis
ms.topic: reference
f1_keywords:
- C28137
helpviewer_keywords:
- C28137
ms.assetid: 34420007-6a73-4f09-bdf7-8d923eef9654
caps.latest.revision: 6
author: corob-msft
ms.author: corob
manager: jillfra
ms.openlocfilehash: 6da097e90e77554b3d428c34a478a81fee1549ae
ms.sourcegitcommit: 68f893f6e472df46f323db34a13a7034dccad25a
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 02/15/2020
ms.locfileid: "77265712"
---
# <a name="c28137"></a>C28137
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

AVERTISSEMENT C28137 : l’argument de la variable doit plutôt être une constante (littérale)  
  
 Cet avertissement est émis quand une constante (littéral) requise fait défaut dans un appel de fonction. Consultez la documentation de la fonction.  
  
## <a name="example"></a>Exemple  
 Par exemple, la routine [ExAcquireResourceExclusiveLite](https://msdn.microsoft.com/library/windows/hardware/ff544351.aspx) requiert une valeur true ou false pour le paramètre `Wait`. L'exemple de code suivant génère cet avertissement :  
  
```  
ExAcquireResourceExclusiveLite(Resource, Wait);  
```  
  
 L'exemple de code suivant évite cet avertissement :  
  
```  
ExAcquireResourceExclusiveLite(Resource, TRUE);  
```
