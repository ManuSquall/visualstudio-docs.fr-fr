---
title: C6054 | Microsoft Docs
ms.date: 11/15/2016
ms.prod: visual-studio-dev14
ms.technology: vs-ide-code-analysis
ms.topic: reference
f1_keywords:
- C6054
helpviewer_keywords:
- C6054
ms.assetid: d573a5c1-7e74-402b-92e6-8085f967aa50
caps.latest.revision: 28
author: corob-msft
ms.author: corob
manager: jillfra
ms.openlocfilehash: 1f3840baea43969679cd04bf30e43823bc05f2c9
ms.sourcegitcommit: 6cfffa72af599a9d667249caaaa411bb28ea69fd
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/02/2020
ms.locfileid: "77277441"
---
# <a name="c6054"></a>C6054
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

AVERTISSEMENT C6054 : la chaîne \<variable> ne peut pas se terminer par zéro  
  
 Cet avertissement indique qu’une chaîne se terminant par zéro a été passée à une fonction qui requiert une chaîne se terminant par zéro. Une fonction qui attend une chaîne se terminant par zéro va au-delà de la fin de la mémoire tampon pour rechercher la valeur zéro. Ce défaut peut provoquer une erreur de dépassement de mémoire tampon ou un blocage exploitable. Le programme doit s’assurer que la chaîne se termine par un zéro.  
  
## <a name="example"></a>Exemple  
 Le code suivant génère cet avertissement :  
  
```cpp  
  
#include <sal.h>  
  
void func( _In_z_ wchar_t* wszStr );  
  
void g ( )  
{  
    wchar_t wcArray[200];  
    func(wcArray); // Warning C6054  
}  
```  
  
 Pour corriger cet avertissement, terminez la valeur null `wcArray` avant d’appeler `func()` la fonction comme indiqué dans l’exemple de code suivant :  
  
```cpp  
  
#include <sal.h>  
  
void func( _In_z_ wchar_t* wszStr );  
  
void g( )  
{  
    wchar_t wcArray[200];   
    wcArray[0]= '\0';  
    func(wcArray);  
}  
```  
  
## <a name="see-also"></a>Voir aussi  
 [C6053](../code-quality/c6053.md)   
 [Vue d’ensemble des annotations](https://msdn.microsoft.com/2345380e-2eeb-4107-907f-6e8b809c2643)   
 [NullTerminated](https://msdn.microsoft.com/86cbc668-e134-44fa-978e-9a0d57134056)
