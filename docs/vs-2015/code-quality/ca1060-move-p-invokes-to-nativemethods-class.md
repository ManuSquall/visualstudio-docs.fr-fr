...
## <a name="rule-description"></a>Description de la règle
 Les méthodes d’appel de code non managé, telles que celles qui sont marquées à l’aide de l’attribut <xref:System.Runtime.InteropServices.DllImportAttribute?displayProperty=fullName>, ou les méthodes définies à l’aide du mot clé `Declare` dans [!INCLUDE[vbprvb](../includes/vbprvb-md.md)], accèdent au code non managé. Ces méthodes doivent être dans l’une des classes suivantes :
