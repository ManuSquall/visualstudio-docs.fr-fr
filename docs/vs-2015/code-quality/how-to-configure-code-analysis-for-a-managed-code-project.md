...
4. Pour exécuter l’analyse du code chaque fois que le projet est généré à l’aide de la configuration sélectionnée, activez la case à cocher **activer l’analyse du code sur la build (définit la constante CODE_ANALYSIS)** . Vous pouvez également exécuter l’analyse du code manuellement en ouvrant le menu **analyser** et en cliquant sur **exécuter l’analyse du code sur** *ProjectName*.
...
6. Dans la liste **exécuter cet ensemble de règles** , effectuez l’une des opérations suivantes :
    ...
    - Cliquez sur **\<Browse... >** pour spécifier un ensemble de règles personnalisé existant qui ne figure pas dans la liste.
