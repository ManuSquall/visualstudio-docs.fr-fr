...
### Utilisation de l'Assistant  
 ...
 Après avoir mis en place le flux de travail, vous êtes libre de modifier ses propriétés dans la fenêtre Propriétés.  Bien que la plupart des propriétés de flux de travail soient modifiables directement dans la fenêtre Propriétés, certaines d'entre elles exigent que vous cliquiez sur un bouton de sélection \(![Bouton de sélection du concepteur ASP.NET mobile](~/docs/sharepoint/media/mwellipsis.gif "Bouton de sélection du concepteur ASP.NET mobile")\) pour modifier leurs valeurs.  Ce bouton a pour effet de redémarrer l'**Assistant Personnalisation de SharePoint**.  Après avoir changé les valeurs des propriétés, cliquez sur le bouton **Terminer** pour les valider.  
