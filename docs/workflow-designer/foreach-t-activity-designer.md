...
|<xref:System.Activities.Statements.ForEach%601.Values%2A>|True|Collection d’éléments à itérer. Pour définir le <xref:System.Activities.Statements.ForEach%601.Values%2A>, tapez une expression Visual Basic dans la **valeurs** zone sur le **ForEach < T\>**  activité concepteur ou dans la grille des propriétés.|
