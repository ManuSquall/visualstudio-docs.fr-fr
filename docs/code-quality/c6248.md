...
```
 ...
 Pour voir un exemple complet sur la création du descripteur de sécurité, consultez [création d’un descripteur de sécurité pour un nouvel objet dans C++](http://msdn.microsoft.com/library/aa446595.aspx). Pour plus d’informations, consultez [création d’une liste DACL](/windows/desktop/SecBP/creating-a-dacl).