...
Vous pouvez empêcher le débogueur de développer des propriétés de façon autodéveloppé en appliquant <xref:System.Diagnostics.DebuggerBrowsableAttribute?displayProperty=fullName>. L’exemple suivant montre que cet attribut est appliqué à une propriété d’instance.
