...
|`dotnet_code_quality.CA1712.enum_values_prefix_trigger = Heuristic` | La règle est déclenchée à l’aide de l’heuristique FxCop par défaut (autrement dit, quand au moins 75% des valeurs enum commencent par le nom de type enum).

Vous pouvez configurer toutes ces options uniquement pour cette règle, pour toutes les règles ou pour toutes les règles de cette catégorie (affectation de noms). Pour plus d’informations, consultez [configurer les analyseurs FxCop](configure-fxcop-analyzers.md).
