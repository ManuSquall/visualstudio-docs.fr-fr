...
Lorsque des instances d’un type peuvent être combinées à l’aide d’opérations telles que l’addition et la soustraction, vous devez presque toujours définir l’égalité pour retourner `true` pour deux instances ayant les mêmes valeurs constitutives.
...
L’exemple suivant définit un type (`BadAddableType`) qui enfreint cette règle. Ce type doit implémenter l’opérateur d’égalité pour faire en sorte que les deux instances qui ont les mêmes valeurs de champ testent `true` l’égalité. Le `GoodAddableType` de type affiche l’implémentation corrigée. Notez que ce type implémente également l’opérateur d’inégalité et remplace <xref:System.Object.Equals%2A> pour satisfaire d’autres règles. Une implémentation complète implémenterait également <xref:System.Object.GetHashCode%2A>.
