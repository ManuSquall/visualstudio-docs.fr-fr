---
title: 'Ca3010 : passez en revue le code pour les vulnérabilités d’injection XAML'
ms.date: 04/03/2019
ms.topic: reference
author: dotpaul
ms.author: paulming
...
ms.locfileid: "74809869"
---
# <a name="ca3010-review-code-for-xaml-injection-vulnerabilities"></a>Ca3010 : passez en revue le code pour les vulnérabilités d’injection XAML
...
Une entrée de requête HTTP potentiellement non approuvée atteint un <xref:System.Windows.Markup.XamlReader?displayProperty=nameWithType> méthode Load.
...
Cette règle tente de trouver l’entrée à partir des requêtes HTTP qui atteignent une méthode de chargement <xref:System.Windows.Markup.XamlReader?displayProperty=nameWithType>.
...
| --- | --- |
|`dotnet_code_quality.CA3010.excluded_symbol_names = MyType` | Correspond à tous les symboles nommés’MyType’dans la compilation
|`dotnet_code_quality.CA3010.excluded_symbol_names = MyType1|MyType2` | Correspond à tous les symboles nommés’MyType1 'ou’MyType2 'dans la compilation
|`dotnet_code_quality.CA3010.excluded_symbol_names = M:NS.MyType.MyMethod(ParamType)` | Correspond à la méthode spécifique’MyMethod’avec la signature complète donnée
|`dotnet_code_quality.CA3010.excluded_symbol_names = M:NS1.MyType1.MyMethod1(ParamType)|M:NS2.MyType2.MyMethod2(ParamType)` | Met en correspondance des méthodes spécifiques « MyMethod1 » et « MyMethod2 » avec la signature complète respective
