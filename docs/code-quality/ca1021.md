...
## <a name="example"></a>Exemple
L’exemple de bibliothèque suivant illustre la façon dont `ref` paramètres pour les types référence sont utilisés et montre une meilleure façon d’implémenter cette fonctionnalité.
...
Les méthodes qui implémentent le modèle **Try\<** , par exemple <xref:System.Int32.TryParse%2A?displayProperty=fullName>, ne déclenchent pas cette violation. L’exemple suivant illustre une structure (type valeur) qui implémente la méthode <xref:System.Int32.TryParse%2A?displayProperty=fullName>.
