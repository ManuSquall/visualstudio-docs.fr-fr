...
- Pour les types non scellés, l’implémentation de `Finalize` n’appelle pas l’un ou les deux `Dispose(bool)` ou le finaliseur de la classe de base.
...
Tous les types de <xref:System.IDisposable> doivent implémenter le [modèle dispose](/dotnet/standard/design-guidelines/dispose-pattern) correctement.
