...
|Valeur de `enum` non valide|<xref:System.ComponentModel.InvalidEnumArgumentException?displayProperty=fullName>|
...
Lorsqu’une opération n’est pas prise en charge (par exemple, dans un **flux substitué. écrire** dans un flux ouvert pour la lecture) lève <xref:System.NotSupportedException?displayProperty=fullName>
...
Pour toutes les autres situations, pensez à créer votre propre type qui dérive de <xref:System.Exception> et à le lever.
