...
- Limitez les types désérialisés. Implémentez un personnalisé <xref:System.Runtime.Serialization.SerializationBinder?displayProperty=nameWithType> . Avant de désérialiser avec <xref:System.Runtime.Serialization.NetDataContractSerializer> , affectez <xref:System.Runtime.Serialization.NetDataContractSerializer.Binder> à la propriété une instance de votre personnalisé <xref:System.Runtime.Serialization.SerializationBinder> . Dans la méthode substituée <xref:System.Runtime.Serialization.SerializationBinder.BindToType%2A> , si le type est inattendu, levez une exception pour arrêter la désérialisation.
  - Vérifiez que la propriété est définie pour tous les chemins de code <xref:System.Runtime.Serialization.NetDataContractSerializer.Binder> .
