...
Dans les versions 1,0 et 1,1 de la .NET Framework, une méthode publique ou protégée contient un `try`/`catch`/bloc `finally`. Le bloc `finally` semble réinitialiser l’état de sécurité et n’est pas inclus dans un bloc de `finally`.
...
Placez le `try`non encapsulé /`finally` dans un bloc try externe. Consultez le deuxième exemple qui suit. Cela oblige l' `finally` à s’exécuter avant le code de filtre.
