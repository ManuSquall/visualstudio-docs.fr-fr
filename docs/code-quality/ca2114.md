...
Examinez votre code pour vous assurer que les deux actions sont requises. Si les deux actions sont requises, assurez-vous que l’action au niveau de la méthode comprend la sécurité spécifiée au niveau du type. Par exemple, si votre type demande `X`d’autorisation, et que sa méthode doit également demander des autorisations `Y`, la méthode doit demander explicitement `X` et `Y`.
