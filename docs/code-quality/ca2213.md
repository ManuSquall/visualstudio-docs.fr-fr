...
L’extrait de code suivant montre un type `TypeB` qui enfreint la règle CA2213 en déclarant un champ `aFieldOfADisposableType` comme type jetable (`TypeA`) et n’appelant pas <xref:System.IDisposable.Dispose%2A> sur le champ.
