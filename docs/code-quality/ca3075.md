...
- la propriété <xref:System.Xml.XmlReaderSettings.DtdProcessing%2A> a la valeur parse.

- L’entrée non fiable est traitée à l’aide de <xref:System.Xml.XmlResolver> au lieu de <xref:System.Xml.XmlSecureResolver>.

- La méthode <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> est appelée avec une instance <xref:System.Xml.XmlReaderSettings> non sécurisée ou sans instance.
...
- Utilisez la <xref:System.Xml.XmlSecureResolver> pour limiter les ressources auxquelles XmlTextReader peut accéder.
...
- Évitez d’activer DtdProcessing si vous utilisez des sources non fiables en affectant à la propriété <xref:System.Xml.XmlReaderSettings.DtdProcessing%2A?displayProperty=nameWithType> la valeur **interdire** ou **Ignorer**.
