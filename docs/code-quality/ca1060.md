...
Les méthodes d’appel de code non managé, telles que celles qui sont marquées à l’aide de l' <xref:System.Runtime.InteropServices.DllImportAttribute?displayProperty=fullName> attribut, ou les méthodes définies à l’aide du `Declare` mot clé dans [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] , accèdent au code non managé. Ces méthodes doivent être dans l’une des classes suivantes :
