...
Pour corriger une violation de cette règle, remplacez le paramètre par un type de <xref:System.Uri> ; Il s’agit d’une modification avec rupture. Vous pouvez également fournir une surcharge de la méthode qui prend un paramètre <xref:System.Uri> ; Il s’agit d’une modification sans rupture.
