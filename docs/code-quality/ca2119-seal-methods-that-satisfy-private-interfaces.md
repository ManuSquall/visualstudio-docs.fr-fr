...
Un type public pouvant être hérité fournit une implémentation de méthode substituable `internal` d'`Friend` une interface (en Visual Basic).
...
- Implémentez la méthode sans utiliser `virtual` le modificateur.
