...
Cette règle recherche <xref:System.Web.Script.Serialization.JavaScriptSerializer?displayProperty=nameWithType> appels de méthode de désérialisation ou des références, après l’initialisation du <xref:System.Web.Script.Serialization.JavaScriptSerializer> avec un <xref:System.Web.Script.Serialization.SimpleTypeResolver?displayProperty=nameWithType>.
...
- Si votre code doit lire les données sérialisées à l’aide d’un <xref:System.Web.Script.Serialization.SimpleTypeResolver>, limitez les types désérialisés à une liste attendue en implémentant un <xref:System.Web.Script.Serialization.JavaScriptTypeResolver>personnalisé.
