...
Pour corriger une violation de cette règle, rendez le champ `private` ou `internal` et exposez-le à l’aide d’une propriété visible de l’extérieur.
...
- Lorsque la plateforme cible utilise des `ref` pour modifier des champs, tels que des frameworks MVVM (Model-View-ViewModel) pour WPF et UWP
