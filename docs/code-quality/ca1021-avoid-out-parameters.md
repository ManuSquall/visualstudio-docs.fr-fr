...
Les méthodes qui implémentent le modèle de **> try @ no__t-1Something** , comme <xref:System.Int32.TryParse%2A?displayProperty=fullName>, ne déclenchent pas cette violation. L’exemple suivant illustre une structure (type valeur) qui implémente la méthode <xref:System.Int32.TryParse%2A?displayProperty=fullName>.
