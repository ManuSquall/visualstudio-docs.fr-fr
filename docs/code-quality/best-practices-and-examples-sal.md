...
Si la fonction est supposée écrire dans l’élément, utilisez `_Inout_` à la `_In_`place de. Cela s’avère particulièrement utile en cas de conversion automatisée de macros plus anciennes en SAL. Avant SAL, de nombreux programmeurs utilisaient des macros comme commentaires, c’est-à-dire des macros `OUT`nommées `IN` `IN_OUT`,,, ou des variantes de ces noms. Bien que nous vous déconseillons de convertir ces macros en SAL, nous vous invitons également à être vigilants lorsque vous les convertissez, car le code peut avoir changé depuis l’écriture du prototype d’origine et l’ancienne macro peut ne plus refléter ce que fait le code. Soyez particulièrement vigilant avec la `OPTIONAL` macro comment, car elle est souvent placée de manière incorrecte, par exemple, du côté incorrect d’une virgule.
...
Si l’appelant n’est pas autorisé à passer un pointeur null, utilisez `_In_` ou `_Out_` au lieu `_In_opt_` de `_Out_opt_`ou. Cela s’applique même à une fonction qui vérifie ses paramètres et retourne une erreur si elle est NULL alors qu’elle ne doit pas l’être. Bien qu’une fonction vérifie son paramètre pour une valeur NULL inattendue et que le retour approprié soit une bonne pratique de codage défensive, cela ne signifie pas que l’annotation de paramètre`_*Xxx*_opt_`peut être d’un type facultatif ().
...
L’exemple suivant illustre une mauvaise utilisation courante `_Out_writes_`de.
...
Une annotation comme `_In_ PCSTR` est courante et utile. Il pointe vers une chaîne d’entrée qui a une terminaison NULL, car `_In_` la condition préalable permet la reconnaissance d’une chaîne terminée par le caractère null.
...
## <a name="_out_range_"></a>\_Hors\_limites\_

Si le paramètre est un pointeur et que vous souhaitez exprimer la plage de la valeur de l’élément vers lequel pointe le pointeur, utilisez `_Deref_out_range_` à la place de. `_Out_range_` Dans l’exemple suivant, la plage de * pcbFilled est exprimée, et non pcbFilled.
...
`_Deref_out_range_(0, cbSize)`n’est pas strictement requis pour certains outils, car il peut être déduit `_Out_writes_to_(cbSize,*pcbFilled)`à partir de, mais il est présenté ici à des fins d’exhaustivité.

## <a name="wrong-context-in-_when_"></a>Contexte incorrect \_dans quand\_
...
## <a name="true-in-_success_"></a>TRUE en \_cas de réussite\_

Si la fonction réussit lorsque la valeur de retour est différente de zéro `return != 0` , utilisez comme condition de réussite `return == TRUE`au lieu de. Une valeur différente de zéro ne signifie pas nécessairement l’équivalence de la valeur réelle que le `TRUE`compilateur fournit pour. Le paramètre de `_Success_` est une expression, et les expressions suivantes sont évaluées comme équivalentes `return != false`: `return != FALSE` `return != 0`,, `return` et sans paramètres ni comparaisons.
...
Pour une variable de référence, la version précédente de SAL utilisait le pointeur implicite comme cible d’annotation et nécessitait `__deref` l’ajout d’un aux annotations attachées à une variable de référence. Cette version utilise l’objet lui-même et ne nécessite pas `_Deref_`le complément.
