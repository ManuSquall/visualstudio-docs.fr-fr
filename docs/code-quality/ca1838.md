...
Les exemples précédents utilisent des caractères larges () de 2 octets `CharSet.Unicode` . Si la fonction native utilise des caractères de 1 octet ( `CharSet.Ansi` ), une `byte` mémoire tampon peut être utilisée à la place d’une `char` mémoire tampon. Exemple :
