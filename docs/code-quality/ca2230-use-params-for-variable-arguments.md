...
Dans C#, la `VarArgs` Convention d’appel est utilisée lorsque la liste de paramètres d’une méthode `__arglist` se termine par le mot clé. Visual Basic ne prend pas en `VarArgs` charge la Convention d’appel C++ , et visuel n’autorise son utilisation que dans du code non managé `...` qui utilise la notation d’ellipse.

## <a name="how-to-fix-violations"></a>Comment corriger les violations
Pour corriger une violation de cette règle dans C#, utilisez le mot clé [params](/dotnet/csharp/language-reference/keywords/params) à la `__arglist`place de.
