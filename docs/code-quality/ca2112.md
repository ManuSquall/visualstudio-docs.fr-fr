...
L’exemple suivant est composé d’un type de bibliothèque (`SecuredTypeWithFields`) avec des champs non sécurisés, un type (`Distributor`) qui peut créer des instances du type de bibliothèque et qui passe des instances à des types ne sont pas autorisés à les créer, et le code d’application qui peut lire les champs d’une instance, même s’il n’a pas l’autorisation qui sécurise le type.
