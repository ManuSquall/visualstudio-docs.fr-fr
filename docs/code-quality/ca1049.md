...
Un type fait référence à un champ <xref:System.IntPtr?displayProperty=fullName>, un champ <xref:System.UIntPtr?displayProperty=fullName> ou un champ <xref:System.Runtime.InteropServices.HandleRef?displayProperty=fullName>, mais n’implémente pas <xref:System.IDisposable?displayProperty=fullName>.
...
Il est possible de supprimer sans risque un avertissement de cette règle si le type ne fait pas référence à une ressource non managée. Dans le cas contraire, ne supprimez pas un avertissement de cette règle, car l’échec de l’implémentation de <xref:System.IDisposable> risque de rendre les ressources non managées indisponibles ou sous-utilisées.
