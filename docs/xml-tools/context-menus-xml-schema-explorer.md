...
|**Afficher toutes les références sortantes**|Pour chaque fichier de l'espace de noms sélectionné, recherche et met en surbrillance les éléments suivants :<br /><br /> -Tous les espaces de noms référencés dans les instructions `schemaLocation` Import sans attribut.<br />-Tous les fichiers dans les espaces de noms autres que celui sélectionné qui sont spécifiés dans l’attribut dans les `schemaLocation` instructions import et include.|
...
|**Afficher toutes les références sortantes**|Recherche et met en surbrillance les éléments suivants :<br /><br /> -Tous les espaces de noms spécifiés dans les attributs d’espace de noms de toutes les `schemaLocation` instructions Import qui n’ont pas l’attribut.<br />-Tous les fichiers spécifiés `schemaLocation` dans les attributs de toutes les instructions import et include.|
...
|**Accéder à l’élément d’origine**|Pour les références d'élément, navigue vers la définition réelle de l'élément.|
...
|**Afficher les têtes de groupe de substitution**|Pour les éléments globaux qui sont membres d'un groupe de substitution, recherche et met en surbrillance tous les en-têtes directs et indirects de l'élément sélectionné, notamment :<br /><br /> : Un en-tête de groupe de substitution spécifié sur l’élément sélectionné.<br />: En-tête de groupe de substitution spécifié sur son élément head.|
...
|**Accéder à l’attribut d’origine**|Pour les références d'attribut, navigue vers la définition réelle de l'attribut.|
