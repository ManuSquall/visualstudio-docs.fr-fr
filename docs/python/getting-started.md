...
Cette procédure pas à pas vous guide tout au long de la création d’une application Python vide, du choix d’un environnement de travail Python, puis de l’écriture de code pour découvrir la fonctionnalité IntelliSense en action. Vous allez ensuite utiliser brièvement la fenêtre REPL interactive pour créer davantage de code, puis terminer le programme et l’exécuter de lui-même et dans le débogueur.
...
La prise en charge de Python dans Visual Studio comprend un certain nombre de [modèles de projet](python-projects.md) pour vous aider à démarrer avec différents types de projets, notamment des applications web utilisant les frameworks Bottle, Flask et Django, ainsi qu’Azure Cloud Services. Dans le cadre de cette procédure pas à pas, nous allons cependant commencer avec un projet vide.
...
1. Les modèles Python se trouvent sous le menu **Modèles > Autres langages > Python** à gauche. Vous pouvez également y accéder en effectuant une recherche sur « Python » :
...
1. Si vous développez le nœud **Environnements Python**, vous voyez l’interpréteur Python qui est défini par défaut pour ce projet. Si vous développez également le nœud de cet interpréteur, vous obtenez une liste des bibliothèques disponibles dans cet environnement :
...
1. Commencez à saisir `print("Hello world")` et observez la manière dont Visual Studio IntelliSense affiche les options de saisie semi-automatique au cours du processus. L’option indiquée dans la liste déroulante correspond au mode de saisie semi-automatique par défaut qui est utilisé lorsque vous appuyez sur la touche de tabulation. Les saisies semi-automatiques sont le plus utiles dans le cas d’instructions ou d’identificateurs plus longs.

    ![Menu contextuel de la saisie semi-automatique IntelliSense](media/getting-started-coding-1.png)

1. IntelliSense affiche différentes informations selon l’instruction que vous utilisez, la fonction que vous appelez, et ainsi de suite. Avec la fonction `print`, tapez le caractère `(` pour que l’appel affiche des informations d’utilisation complètes sur cette fonction et indique en gras l’argument actuel que vous devez fournir (**value** comme indiqué ici) :
...
1. Pour exécuter le code, appuyez sur F5 ou sélectionnez l’élément de menu **Déboguer > Démarrer le débogage**.
...
1. Visual Studio exécute le code à l’aide de l’environnement par défaut dans le projet et affiche les résultats dans une fenêtre Commande. Appuyez sur une touche pour fermer cette fenêtre et mettre fin à la session de débogage.

    ![Bouton Démarrer dans la barre d’outils de débogage](media/getting-started-coding-4.png)

1. Outre les instructions et fonctions, la fonctionnalité IntelliSense fournit une saisie semi-automatique pour les instructions `import`. Les saisies semi-automatiques pour les instructions import vous permettent de détecter facilement les modules qui sont disponibles dans votre environnement, de même que les membres disponibles dans ce module. Dans l’éditeur, supprimez la ligne `print` et commencez à taper `import`. Une liste de modules s’affiche :
...
1. Autre fonctionnalité utile : la possibilité d’envoyer rapidement plusieurs lignes de code d’une fenêtre d’éditeur vers la fenêtre interactive, dans laquelle vous pouvez l’utiliser dans l’environnement REPL rapide plutôt que d’avoir à écrire un autre code à exécuter dans le débogueur. Pour voir les fonctionnalités, commencez par ajouter le code suivant à votre fichier hello.py ouvert dans l’éditeur :
...
1. L’utilisation de Ctrl+Entrée avec le signe insertion sur une seule ligne dans l’éditeur est également un moyen simple de parcourir votre code. Ctrl+Entrée permet d’exécuter la ligne active dans la fenêtre interactive, puis de définir automatiquement le signe insertion sur la ligne suivante. En appuyant sur Ctrl+Entrée à plusieurs reprises, vous pouvez exécuter tout le code dans un fichier.

1. Dans Visual Studio 2017, vous pouvez coller plusieurs lignes directement dans la fenêtre interactive (dans les versions précédentes, vous deviez coller ces lignes dans une fenêtre d’éditeur, les sélectionner, puis utiliser **Envoyer vers Interactive**). Une fois le collage terminé, la fenêtre interactive exécute ce code, comme vous pouvez le voir ci-dessous :
...
1. Étant donné que la définition de fonction se trouve dans l’historique REPL en tant qu’unité unique, vous pouvez facilement revenir en arrière et apporter les modifications souhaitées, puis tester à nouveau la fonction.

1. Quand vous êtes satisfait du code que vous avez écrit dans la fenêtre interactive, vous pouvez le sélectionner, cliquer dessus avec le bouton droit et sélectionner **Copier le code**, puis le coller dans l’éditeur. Cette commande **Copier le code** omet automatiquement toute sortie ainsi que le texte d’invite >>> et .... Par exemple, si vous utilisez la commande avec la sélection ci-dessous :

  ![Commande Copier le code dans la fenêtre interactive](media/getting-started-interactive-6.png)

  vous collez uniquement ce qui suit :
...
1. Enfin, la fenêtre interactive fournit un certain nombre de métacommandes pour charger des fichiers, réinitialiser l’environnement sans perdre l’historique et insérer des commentaires à mesure que vous écrivez du code. Pour plus d’informations, consultez la page [Fenêtres interactives - Métacommandes](interactive-repl.md#meta-commands).
...
1. Vérifiez que le code fonctionne correctement en appuyant sur F5 ou en sélectionnant la commande de menu **Déboguer > Démarrer le débogage**. Cette commande exécute le code dans le débogueur mais, étant donné que vous n’avez pas défini les points d’arrêt, elle imprime simplement un motif de vague pour quelques itérations. Appuyez sur n’importe quelle touche pour fermer la fenêtre de sortie à ce stade.
    ...
    > ```
    > 
    > Sinon, dans les cas où la fenêtre de sortie se ferme automatiquement alors qu’elle ne le devrait pas, cliquez avec le bouton droit sur le projet, sélectionnez **Propriétés**, sélectionnez l’onglet **Déboguer**, puis ajoutez `-i` au champ **Arguments de l’interpréteur**. Avec cet argument, l’interpréteur passe en mode interactif à la fin d’un programme en maintenant de cette façon la fenêtre ouverte jusqu’à ce que vous entriez Ctrl+Z, Entrée pour quitter.
...
1. Redémarrez le débogueur ; vous voyez maintenant que l’exécution du code s’arrête sur la ligne sur laquelle est défini ce point d’arrêt. Vous pouvez ici examiner la pile des appels et les variables locales dans la fenêtre Variables locales :

    ![Expérience de l’interface utilisateur des points d’arrêt pour Python](media/getting-started-debugging-2.png)

1. Parcourez quelques itérations de la boucle `for` ligne par ligne à l’aide de la touche F10, de la commande **Déboguer > Pas à pas principal** ou du bouton de la barre d’outils Pas à pas principal. Pas à pas principal signifie que le débogueur exécute chaque appel à `make_dot_string`, mais qu’il ne s’arrête pas à l’intérieur de cette fonction (sauf si vous définissez un point d’arrêt).
...
1. Il peut être assez fastidieux de parcourir des centaines d’itérations d’une boucle. Aussi, vous avez la possibilité d’ajouter une condition au point d’arrêt défini précédemment pour que le code s’arrête uniquement lorsque la valeur de `i` dépasse une certaine valeur, par exemple 1600. Pour définir une condition, cliquez avec le bouton droit sur le point rouge représentant le point d’arrêt et sélectionnez **Condition...**. Dans la fenêtre Paramètres de point d’arrêt qui s’affiche, entrez `i > 1600` comme expression et sélectionnez **Fermer**. Appuyez maintenant sur F5 pour continuer. Vous remarquez que le programme s’exécute pendant un certain temps avant de s’arrêter de nouveau. 
