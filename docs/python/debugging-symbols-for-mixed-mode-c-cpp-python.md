...
Pour offrir une expérience de débogage complète, le [débogueur Python en mode mixte](debugging-mixed-mode-c-cpp-python-in-visual-studio.md) dans Visual Studio nécessite des symboles de débogage pour l’interpréteur Python utilisé afin d’analyser de nombreuses structures de données internes. Pour *python27.dll*, par exemple, le fichier symbole correspondant est *python27.pdb*; pour *python36.dll*, le fichier symbole est *python36.pdb*. Chaque version de l’interpréteur fournit également des fichiers de symboles pour différents modules.

Dans Visual Studio 2017 et ultérieur, les interpréteurs Python 3 et Anaconda 3 installent automatiquement leurs symboles respectifs, qui sont ensuite automatiquement détectés par Visual Studio. Pour Visual Studio 2015 et plus tôt, ou lorsque vous utilisez d’autres interprètes, vous devez télécharger des symboles séparément, puis pointer Visual Studio vers eux à travers le dialogue **Tools** > **Options** dans **l’onglet Symboles Debugging.** > **Symbols** Ces étapes sont détaillées dans les sections suivantes.
...
- Python 3.4.x et versions antérieures : les symboles sont fournis dans des fichiers *.zip* que vous téléchargez à partir des [distributions officielles](#official-distributions) ou [d’Enthought Canopy](#enthought-canopy). Après avoir téléchargé les fichiers, extrayez-les dans un dossier local pour continuer, par exemple un dossier *Symbols* dans le dossier Python.
    ...
    > [!Important]
    > Les symboles ne sont pas les mêmes pour les différentes versions mineures de Python et entre les versions 32 bits et 64 bits. Vous devez donc télécharger les versions correspondantes exactes. Pour vérifier l’utilisation de l’interprète, élargissez le *nœud* **Python Environments** dans le cadre de votre projet dans **Solution Explorer** et notez le nom de l’environnement. Ensuite, passez à la *fenêtre* **Python Environments** et notez l’emplacement d’installation. Ouvrez une fenêtre Commande à cet emplacement, puis lancez *python.exe*, qui indique la version exacte et s’il s’agit d’une version 32 bits ou 64 bits.
...
1. Sélectionnez le menu **Tools** > **Options** et naviguez vers **Debugging** > **Symbols**.
