...
Dans Visual Studio sous Windows, la fenêtre [**Environnements Python**](#the-python-environments-window), décrite dans cet article, permet de gérer ces environnements et d’en sélectionner un par défaut pour les nouveaux projets. Pour un projet donné, vous pouvez aussi [sélectionner un environnement en particulier](selecting-a-python-environment-for-a-project.md) au lieu d’utiliser l’environnement par défaut.

>[!NOTE]
>Si vous utilisez Python dans Visual Studio, consultez les articles suivants pour obtenir les informations nécessaires :
>
...
> [!Note]
> Les captures d’écran présentées dans cette section représentent Visual Studio 15.8. L’interface utilisateur peut être différente selon les versions de Visual Studio.
