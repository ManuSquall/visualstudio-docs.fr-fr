...
1. Dans l’implémentation de <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage.SetSite%2A> votre VSPackage, `QueryService` appelez <xref:Microsoft.VisualStudio.Shell.Interop.SVsAppCommandLine> sur pour accéder <xref:Microsoft.VisualStudio.Shell.Interop.IVsAppCommandLine> à l’interface.
