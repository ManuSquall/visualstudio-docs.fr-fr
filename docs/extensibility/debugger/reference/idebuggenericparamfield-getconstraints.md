...
Récupère les contraintes qui sont associées à ce paramètre générique.  
...
#### Paramètres  
 `cConstraints`  
 \[in\]  numéro de contraintes.  
  
 `ppConstraints`  
 \[out\]  Retourne un tableau qui contient les contraintes associées à ce champ.  
  
 `pcConstraints`  
 \[in, out\]  Numéro de contraintes dans le tableau d' `ppConstraints` .  
