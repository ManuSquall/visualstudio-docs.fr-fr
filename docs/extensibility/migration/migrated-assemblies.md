---
title: Packages NuGet SDK VS
description: En savoir plus sur le package Visual Studio SDK et d’autres packages NuGet dont vous pouvez avoir besoin lors de la migration d’une extension Visual Studio vers Visual Studio 2022 preview.
ms.date: 06/08/2021
ms.topic: conceptual
...
ms.locfileid: "112308723"
---
# <a name="sdk-reference-packages"></a>Packages de référence SDK

[!INCLUDE [preview-note](../includes/preview-note.md)]

Le moyen le plus simple de créer des extensions Visual Studio consiste à utiliser une référence au [ `Microsoft.VisualStudio.Sdk` package NuGet](https://www.nuget.org/packages/microsoft.visualstudio.sdk).
Ce package est disponible pour cibler Visual Studio 2017 (15,0), Visual Studio 2019 (16,0, 16,9) et maintenant Visual Studio 2022.

En fonction de votre extension, il peut être nécessaire d’ajouter des packages Visual Studio SDK supplémentaires qui ne sont pas inclus dans le méta-package ci-dessus.
Lors du référencement d’autres packages du kit de développement logiciel (SDK), ces packages peuvent varier entre les versions majeures de VS.

Notez que de nombreux assemblys d’interopérabilité étaient incorporables avant Visual Studio 2022. À compter de Visual Studio 2022, l’incorporation n’est plus nécessaire ou prise en charge.
Veuillez *référencer* nos assemblys d’interopérabilité au lieu de les lier.

Le tableau suivant fournit un mappage à partir d’assemblys ou de packages votre extension pré-Visual Studio 2022 peut déjà référencer le nouvel ID de package à référencer lors du ciblage de Visual Studio 2022. Dans certains cas, les assemblys sont désormais disponibles dans les packages NuGet qui étaient auparavant uniquement disponibles à partir d’une installation locale de Visual Studio.
...
`Microsoft.VisualStudio.SDK.EmbedInteropTypes` | Périmé. Supprimer la référence.)
...
Notez le nombre d’assemblys d’interopérabilité à présent disponibles à partir d’un seul assembly d’interopérabilité fusionné.
Lorsqu’un package n’apparaît pas dans le tableau ci-dessus, il peut être le même dans les deux versions.
