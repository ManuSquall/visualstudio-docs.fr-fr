...
3. Ouvrez *TestUniversalProject.cs* et ajoutez les directives de `using` suivantes :
...
6. Supprimez le code existant de la méthode `ShowMessageBox` :
...
8. Recherchez le projet partagé. Le projet partagé est un conteneur pur ; elle ne génère pas ou ne produit pas de sorties. La méthode suivante recherche le premier projet partagé dans la solution en recherchant l’objet <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy> qui a la fonctionnalité de projet partagé.
...
9. Dans la méthode `ShowMessageBox`, génère la légende (nom du projet qui apparaît dans le **Explorateur de solutions**) du projet partagé.
...
11. Dans la méthode `ShowMessageBox`, génère la légende du projet de plateforme actif.
...
12. Itérez au sein des projets de plateforme. La méthode suivante obtient tous les projets d’importation (plateforme) à partir du projet partagé.
    ...
    > [!IMPORTANT]
    > Si l’utilisateur a ouvert un C++ projet d’application Windows universelle dans l’instance expérimentale, le code ci-dessus lève une exception. Il s'agit d'un problème connu. Pour éviter cette exception, remplacez le bloc `foreach` ci-dessus par ce qui suit :
...
13. Dans la méthode `ShowMessageBox`, génèrent la légende de chaque projet de plateforme. Insérez le code suivant après la ligne qui génère la légende du projet de plateforme actif. Seuls les projets de plateforme chargés s’affichent dans cette liste.
...
15. Dans la méthode `ShowMessageBox`, modifiez le projet de plateforme active. Insérez ce code dans le bloc `foreach`.
...
16. Essayez maintenant. Appuyez sur F5 pour lancer l’instance expérimentale. Créez un C# projet d’application de concentrateur universel dans l’instance expérimentale (dans la boîte de dialogue **nouveau projet** , **Visual C#**   > **Windows**  > **Windows 8**  > **application Hub**0**universelle** ). Une fois la solution chargée, accédez au menu **Outils** et cliquez sur **appeler TestUniversalProject**, puis vérifiez le texte dans le volet **sortie** . Vous devriez voir quelque chose qui ressemble à ce qui suit :
...
2. Dans la méthode `ShowMessageBox`, ajoutez le code suivant pour parcourir les éléments de la hiérarchie du projet de plateforme. Insérez-le dans le bloc `foreach`.
...
4. Essayez maintenant. Appuyez sur **F5** pour lancer l’instance expérimentale. Créer un C# projet d’application de concentrateur universel dans l’instance expérimentale (dans la boîte de dialogue **nouveau projet** , **Visual C#**   > **Windows**  > **Windows 8**  > **application Hub**0**universelle** ) Accédez au menu **Outils** et cliquez sur **appeler TestUniversalProject**, puis vérifiez le texte dans le volet **sortie** . Vous devriez voir quelque chose qui ressemble à ce qui suit :
...
4. Faire en sorte que la classe `HierarchyEventListener` implémente <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents> :
...
8. Inscrivez-vous à l' <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed> d’événements d’élément de projet DTE. Ajoutez le code suivant après avoir raccordé le deuxième écouteur.
...
10. Appelez cette méthode après avoir tout l’autre code dans `ShowMessageBox` pour modifier le nom de fichier de l’élément dans le projet partagé. Insérez ceci après le code qui obtient le chemin d’accès complet de l’élément dans le projet partagé.
...
11. Générez et exécutez le projet. Créez une C# application de concentrateur universel dans l’instance expérimentale, accédez au menu **Outils** et cliquez sur **appeler TestUniversalProject**et vérifiez le texte dans le volet de sortie général. Le nom du premier élément dans le projet partagé (nous pensons qu’il s’agit du fichier *app. Xaml* ) doit être modifié. vous devez voir que l’événement <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed> s’est déclenché. Dans ce cas, étant donné que l’attribution d’un nouveau nom à *app. Xaml* entraîne également le renommage de *app.Xaml.cs* , vous devez voir quatre événements (deux pour chaque projet de plateforme). (Les événements DTE n’effectuent pas le suivi des éléments dans le projet partagé.) Vous devez voir deux événements <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A> (un pour chacun des projets de plateforme), mais aucun événement <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A>.
...
13. Générez et exécutez le projet. Créez un C# projet universel dans l’instance expérimentale, accédez au menu **Outils** et cliquez sur **appeler TestUniversalProject**et vérifiez le texte dans le volet de sortie général. Une fois que le fichier du projet de plateforme a été renommé, vous devez voir un événement <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A> et un événement <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A>. Étant donné que la modification du fichier ne provoquait pas la modification d’autres fichiers, et étant donné que les modifications apportées aux éléments d’un projet de plateforme ne sont pas propagées en tout lieu, il n’y a qu’un seul de ces événements.
