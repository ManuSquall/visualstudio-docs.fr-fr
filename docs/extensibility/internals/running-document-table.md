...
|Propriétaire de la hiérarchie|Objet de hiérarchie qui possède le document, tel qu’il est représenté par une interface <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy>.|
...
|Objet de données de document|Au minimum, il s’agit d’un `IUnknown`<br /><br /> . L’IDE ne requiert pas d’interface particulière au-delà de l’interface de `IUnknown` pour un objet de données de document de l’éditeur personnalisé. Toutefois, dans le cas d’un éditeur standard, l’implémentation de l’interface <xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2> de l’éditeur est requise pour gérer les appels de persistance du fichier à partir du projet. Pour plus d’informations, consultez [enregistrement d’un document standard](../../extensibility/internals/saving-a-standard-document.md).|
...
|Détenteur du verrou|Instance d’une interface <xref:Microsoft.VisualStudio.Shell.Interop.IVsDocumentLockHolder>. Le détenteur de verrou est implémenté par des fonctionnalités telles que des assistants qui ouvrent et modifient des documents en dehors d’un éditeur. Un détenteur de verrou permet à la fonctionnalité d’ajouter un verrou de modification au document pour empêcher la fermeture du document pendant qu’il est toujours en cours de modification. En règle générale, les verrous de modification sont ajoutés uniquement par les fenêtres de document (c’est-à-dire les éditeurs).|
