...
Les sections suivantes fournissent des listes d’entrées de Registre pour les différentes options de service de langage disponibles dans [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)].
 ...
 Dans la liste suivante d’entrées de Registre, *vs reg root* est égal à HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VisualStudio \\*x. y*, où *x. y* est le numéro de version [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)].
...
|Prise en charge CF_HTML|REG_DWORD|0-1|Spécifie si l’éditeur permet la copie et le collage de données HTML.|
...
## <a name="registry-entries-for-debugger-languages-options"></a>Entrées de Registre pour les options des langues du débogueur
 Les services \Languages\Language *racine vs Reg*\\*Language Name \Debugger languages*\\*GUID*\ Key peuvent inclure les valeurs suivantes.

|Name|Tapez|Range|Description|
|----------|----------|-----------|-----------------|
|(Default)|REG_SZ|ASCII|La valeur par défaut peut être utilisée pour documenter le nom de la langue. Le nom de cette clé est un GUID d’un évaluateur d’expression qui a une entrée correspondante dans *\<VS racine Reg >* évaluateur \AD7Metrics\Expression.|
...
|Name|Tapez|Range|Description|
|----------|----------|-----------|-----------------|
|(Default)|REG_SZ|ResID|Nom complet localisé de cette page d’options. Le nom peut être un texte littéral ou # `nnn`, où `nnn` est un ID de ressource de chaîne dans la DLL satellite du VSPackage spécifié.|
|Package|REG_SZ|*GUID*|GUID du VSPackage qui implémente cette page d’options.|
|Page|REG_SZ|*GUID*|GUID de la page de propriétés à demander à partir du VSPackage en appelant la méthode <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage.GetPropertyPage%2A>. Si cette entrée de Registre n’est pas présente, la clé de registre décrit un nœud, pas une page.|
...
|Name|Tapez|Range|Description|
|----------|----------|-----------|-----------------|
|(Default)|REG_SZ|*GUID*|GUID du service pour le service de langage par défaut pour ce type d’extension de nom de fichier.|
...
|Name|Tapez|Range|Description|
|----------|----------|-----------|-----------------|
|(Default)|REG_SZ|""|Inutilisé vous pouvez placer votre nom ici pour la documentation.|
...
## <a name="registry-entries-for-logical-view-options"></a>Entrées de Registre pour les options de vue logique
 L'*interface utilisateur graphique*de l’éditeur de \Editors \\ *racine vs reg*> clé \LogicalViews peut contenir les valeurs suivantes.
...
## <a name="registry-entries-for-editor-extension-options"></a>Entrées de Registre pour les options d’extension de l’éditeur
 La clé \Extensions du GUID de l'*éditeur*de \Editors \\ *racine vs reg*peut contenir les valeurs suivantes. L’extension de nom de fichier n’inclut pas le point de début.
...
## <a name="registry-entries-for-managed-package-framework-language-service-options"></a>Entrées de Registre pour les options du service de langage du package managé
 ...
 Ces entrées de Registre sont accessibles via la classe <xref:Microsoft.VisualStudio.Package.LanguagePreferences>.
