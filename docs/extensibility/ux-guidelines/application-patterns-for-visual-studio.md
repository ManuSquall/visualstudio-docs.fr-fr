...
| Aucun modificateur | Source | Référence de suppressions à l’élément d’origine | Référence de suppressions à l’élément d’origine | | Aucun modificateur | Résultat | `DROPEFFECT_MOVE` est retourné en tant qu’action à partir de `::Drop` et de l’élément reste dans l’emplacement d’origine dans le stockage | `DROPEFFECT_MOVE` est retourné en tant qu’action à partir de `::Drop` et de l’élément reste dans l’emplacement d’origine dans le stockage |  
