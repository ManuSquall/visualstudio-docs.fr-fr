...
1.  Fournir la <xref:Microsoft.VisualStudio.Shell.ProvideEditorExtensionAttribute> classe avec les paramètres appropriés pour votre éditeur de la classe de votre VSPackage.  
    ...
    ```  
     ...
     Le `projectGuid` est le GUID pour les types de fichier divers, définis dans <xref:Microsoft.VisualStudio.VSConstants.CLSID_MiscellaneousFilesProject>. Le type de fichier divers est fourni, afin que le fichier résultant ne va pas faire partie du processus de génération.  
