...
> [!WARNING]
> Si vous attachez au processus de travail [!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)] pour le débogage et que vous atteignez un point d’arrêt, tout le code managé dans le processus de travail s’arrête. Un arrêt de tout le code managé dans le processus de travail peut provoquer un arrêt de traitement pour tous les utilisateurs sur le serveur. Avant d'effectuer un débogage sur un serveur de production, considérez l'impact potentiel sur le travail de production.
