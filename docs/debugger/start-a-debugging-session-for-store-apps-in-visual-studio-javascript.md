---
title: "Démarrer une session de débogage pour les applications UWP dans Visual Studio (JavaScript) | Documents Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
2.  Si la solution contient plusieurs projets, assurez-vous que le projet que vous souhaitez déboguer est le projet de démarrage. Dans l’Explorateur de solutions, sélectionnez le projet, puis choisissez **définir comme projet de démarrage** dans le menu contextuel.  
...
##  <a name="BKMK_Configure_the_debugging_session"></a> Configurer la session de débogage  
 Comme le script n'est pas compilé, les paramètres de plateforme et de configuration de build ne s'appliquent pas. Si vous déboguez un composant C++ ou composant managé, définissez la **Configuration** à **déboguer** et sélectionnez votre plateforme cible à partir de la **Configuration** boîte de dialogue.  
...
2.  Dans la liste **Plateforme** , sélectionnez la plateforme cible à générer. Dans la plupart des cas, **Any CPU** constitue le meilleur choix.  
...
###  <a name="BKMK_Choose_the_deployment_target"></a> Sélectionner la cible de déploiement  
 ...
 Pour une application UWP, choisissez une des options suivantes dans le **appareil cible** liste :  
...
-   Entrez le nom ou l’adresse IP de l’ordinateur distant dans le **nom de l’ordinateur** boîte.  
  
-   Cliquez sur la flèche bas dans la **nom de l’ordinateur** et sélectionnez  **\<rechercher... >**. Choisissez ensuite l’ordinateur distant à partir de **sélectionner une connexion du débogueur distant** boîte de dialogue.  
  
     ![Sélectionnez une connexion du débogueur distant](../debugger/media/vsrun_pro_selectremotedebuggerdlg.png "VSRUN_PRO_SelectRemoteDebuggerDlg")  
 ...
 Pour une application Windows Phone, choisissez **périphérique** ou l’un des émulateurs de la **appareil cible** liste.  
...
###  <a name="BKMK__Optional__Disable_network_loopbacks"></a> Désactiver les bouclages de réseau (facultatif)  
 ...
 Pour supprimer l’exemption du bouclage de réseau, choisissez **non** à partir de la **autoriser le bouclage de réseau** liste sur le **débogage** page de propriétés.  
...
###  <a name="BKMK_Start_debugging__F5_"></a> Démarrer le débogage (F5)  
 Lorsque vous choisissez **démarrer le débogage** sur la **déboguer** menu (clavier : F5), Visual Studio lance l’application avec le débogueur attaché. L'exécution se poursuit jusqu'à ce qu'un point d'arrêt soit atteint, que vous suspendiez manuellement l'exécution, qu'une exception non gérée se produise ou que l'application se termine.  
...
2.  Choisissez **démarrer le débogage** sur la **déboguer** menu (clavier : F5).  
...
1.  Sur le **déboguer** menu, choisissez **déboguer le Package d’application installé**.  
...
1.  Installez les outils de contrôle à distance Visual Studio sur le périphérique où l'application est installée. Consultez [installation des outils distants](http://msdn.microsoft.com/library/windows/apps/hh441469.aspx#BKMK_Installing_the_Remote_Tools).  
