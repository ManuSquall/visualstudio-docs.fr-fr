...
**Utiliser la nouvelle assistance d’exception** :  
...
- **Avertir s'il n'y a pas de code utilisateur au lancement (Managé uniquement)**  :  
...
**Transfert des propriétés et des opérateurs (Managé uniquement)**  :  
Empêche le débogueur d'effectuer un pas à pas détaillé des propriétés et des opérateurs en code managé.

**Activer l'évaluation de la propriété et d'autres appels de fonction implicites** :  
...
Indique au débogueur Visual Studio qu'il faut obtenir les fichiers sources à partir des serveurs sources qui implémentent le protocole SrcSrv (`srcsrv.dll`). Team Foundation Server et les outils de débogage pour Windows sont deux serveurs sources qui implémentent le protocole. Pour plus d’informations sur le programme d’installation de SrcSrv, consultez la [SrcSrv](/windows-hardware/drivers/debugger/srcsrv) documentation. En outre, consultez [spécifier les symboles (.pdb) et les fichiers sources](../debugger/specify-symbol-dot-pdb-and-source-files-in-the-visual-studio-debugger.md).
...
- **Toujours exécuter les commandes de serveur source non fiables sans demander de confirmation** :  
...
**Activer la prise en charge du lien source** :  
...
- **Revenir à l'authentification par Git Credential Manager pour toutes les requêtes de lien source** :  
...
**Activer le débogage JavaScript pour ASP.NET (Chrome, Edge et Internet Explorer)**  :  
...
**Activer les outils de développement Edge pour les applications JavaScript UWP (expérimental)**  :  
Active les outils de développement pour applications UWP JavaScript dans Edge.

**Activer le débogueur JavaScript hérité de Chrome pour ASP.NET** :  
...
**Lors de l'exécution de Visual Studio comme administrateur, lancer de manière expérimentale le débogage de JavaScript dans Chrome** :  
...
- **Activer Modifier et Continuer (XAML)**  :  
...
La fenêtre **Outils de diagnostic** s’affiche pendant que vous déboguez.

**Afficher le PerfTip de la durée calendaire durant le débogage** :  
