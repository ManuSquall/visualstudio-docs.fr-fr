...
<a name="iis"></a>
- Sélectionnez le **temps de développement pris en charge par IIS** lors de l’installation de Visual Studio. (Si nécessaire, réexécutez l’Visual Studio Installer, sélectionnez **modifier**, puis ajoutez ce composant.)
- Exécutez Visual Studio en tant qu’administrateur.
- Installez et configurez correctement IIS avec la ou les versions appropriées de ASP.NET et/ou de ASP.NET Core. Pour plus d’informations et pour obtenir des instructions, consultez [iis 8,0 avec ASP.NET 3,5 et ASP.NET 4,5](/iis/get-started/whats-new-in-iis-8/iis-80-using-aspnet-35-and-aspnet-45) ou [ASP.net Core hôte sur Windows avec IIS](/aspnet/core/host-and-deploy/iis/index).
