...
## <a name="BKMK_Set_a_breakpoint_from_debugger_windows"></a>Définir des points d’arrêt à partir des fenêtres du débogueur
...
1. Dans la boîte de dialogue **nouveau point d’arrêt sur fonction** , entrez le nom de la fonction dans la zone nom de la **fonction** .
   ...
   - Utilisez l’opérateur de contexte en C++mode natif.
...
## <a name="BKMK_set_a_data_breakpoint_managed"></a>Définir des points d’arrêt sur variable (.NET Core 3,0 ou version ultérieure)
...
## <a name="BKMK_set_a_data_breakpoint_native_cplusplus"></a>Définir des points d’arrêt C++ sur variable (natif uniquement)
...
### <a name="using-object-ids-in-conditional-expressions-c-and-f-only"></a>Utilisation d’ID d’objet dans lesC# expressions F# conditionnelles (et uniquement)
...
2. Démarrez le débogage et, lorsque l’exécution s’interrompt au point d’arrêt, sélectionnez **Déboguer** > **Windows** > **variables locales** ou **ALT**+**4** pour ouvrir la fenêtre **variables locales** .
...
**Pour définir un trace :**

1. Cliquez avec le bouton droit sur un point d’arrêt et sélectionnez **actions**. Ou, dans la fenêtre **paramètres de point d’arrêt** , pointez sur le point d’arrêt, sélectionnez l’icône des **paramètres** , puis sélectionnez **actions**.

1. Entrez un message dans le champ **consigner un message dans la fenêtre Sortie** . Le message peut inclure des chaînes de texte génériques, des valeurs de variables ou d’expressions placées entre accolades, et des[C#](../debugger/format-specifiers-in-csharp.md) spécificateurs de format (et [C++](../debugger/format-specifiers-in-cpp.md)) pour les valeurs.

   Vous pouvez également utiliser les mots clés spéciaux suivants dans le message :

   - **$Address** -instruction en cours
   - Nom de la fonction d’appel de **$Caller**
   - **$CallStack** -pile des appels
   - **$Function** -nom de la fonction actuelle
   - ID de processus de **$PID**
   - Nom du processus de **$PNAME**
   - ID de thread **$TID**
   - Nom du thread **$tname**
   - **$Tick** -nombre de cycles (à partir de Windows `GetTickCount`)

1. Pour imprimer le message dans la fenêtre **sortie** sans interruption, activez la case à cocher **continuer l’exécution** . Pour imprimer le message et arrêter l’exécution sur le point de trace, désactivez la case à cocher.

Les points de trace apparaissent sous forme de losanges rouges dans la marge de gauche du code source et les fenêtres de **points d’arrêt** .
