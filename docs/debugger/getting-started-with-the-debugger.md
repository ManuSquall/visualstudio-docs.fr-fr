---
title: Apprendre à déboguer avec le débogueur Visual Studio
...
* Vous devez disposer de Visual Studio 2017 avec la charge de travail **Développement .NET Desktop** ou **Développement Desktop en C++**.
...
2. Sous **Visual C#** ou **Visual C++**, choisissez **Windows Desktop** puis, dans le volet central, choisissez **Application console** (**Application console Windows** en C++).
...
3. Tapez un nom comme **get-started-debugging**, puis cliquez sur **OK**.
    ...
    > [!NOTE]
    > Pour passer de l’exemple de code C# à l’exemple de code C++ dans cet article, utilisez le filtre de langage en haut à droite de cette page.

4. Dans *Program.cs* (C#) ou *get-started-debugging.cpp* (C++), remplacez le code suivant
...
1. Dans la boucle `foreach` de la fonction `Main` (la boucle `for` de la fonction `main` en C++), définissez un point d’arrêt en cliquant sur la marge gauche de la ligne de code suivante :

    `shape.Draw()` (ou `shape->Draw()` en C++)
...
1. Avec l’exécution mise en pause dans l’appel de la méthode `shape.Draw` dans la méthode `Main` (`shape->Draw` en C++), appuyez sur **F11** (ou choisissez **Déboguer > Pas à pas détaillé**) pour avancer dans le code de la classe `Rectangle`.
...
2. Appuyez sur **F10** (ou choisissez **Déboguer > Pas à pas principal**) plusieurs fois, jusqu’à ce que le débogueur s’arrête sur l’appel de la méthode `base.Draw` (`Shape::Draw` en C++), puis appuyez sur **F10** encore une fois.
     ...
     Notez que cette fois, le débogueur n’effectue pas de pas à pas détaillé dans la méthode `Draw` de la classe de base (`Shape`). **F10** fait avancer le débogueur sans effectuer de pas à pas détaillé dans les fonctions ou les méthodes du code de votre application (le code s’exécute néanmoins). En appuyant sur F10 sur l’appel de la méthode `base.Draw` (ou `Shape::Draw`) (au lieu de **F11**), nous avons ignoré le code d’implémentation pour `base.Draw` (qui ne nous intéresse peut-être pas pour l’instant).
...
2. Cliquez sur le bouton **Exécuter jusqu’au clic** ![Exécuter jusqu’au clic](../debugger/media/dbg-tour-run-to-click.png "RunToClick").
    ...
    Le débogueur avance jusqu’à l’implémentation de la méthode `Console.WriteLine` pour la classe `Triangle` (`std::cout` en C++).
...
1. Appuyez sur **Maj** + **F11** (ou **Déboguer > Pas à pas sortant**).
     ...
     Vous êtes normalement revenu dans la boucle `foreach` de la méthode `Main` (la boucle `for` en C++).
...
Le débogueur s’arrête à nouveau sur le point d’arrêt que vous définissez, sur la méthode `shape.Draw()` (`shape->Draw()` en C++).
...
1. Alors que l’application est mise en pause sur la méthode `shape.Draw()` (`shape->Draw()` en C++), placez le curseur sur l’objet `shapes` : vous voyez alors la valeur de sa propriété par défaut, la propriété `Count`.

1. Développez l’objet `shapes` pour voir toutes ses propriétés, comme le premier index du tableau `[0]`, qui a une valeur de `Rectangle` (C#) ou une adresse mémoire (C++).
...
2. Ensuite, examinons la fenêtre **Variables locales**, sous un onglet à côté de la fenêtre **Automatique**.
    ...
    La fenêtre **Variables locales** vous montre les variables qui se trouvent dans l’[étendue](https://www.wikipedia.org/wiki/Scope_(computer_science)) actuelle.
...
1. Alors que l’application est mise en pause dans la boucle `foreach` (la boucle `for` en C++), cliquez sur la fenêtre **Pile des appels**, qui est ouverte par défaut dans le volet inférieur droit.

2. Appuyez sur **F11** quelques fois, jusqu’à ce que le débogueur place l’application en pause sur la méthode `Circle.Draw` dans l’éditeur de code. Regardez la fenêtre **Pile des appels**.

    ![Examiner la pile des appels](../debugger/media/get-started-call-stack.png "ExamineCallStack")

    La fenêtre **Pile des appels** montre l’ordre dans lequel les méthodes et les fonctions sont appelées. La ligne du haut montre la fonction active (la méthode `Circle.Draw` ou `Circle::Draw` de cette application). La deuxième ligne montre que `Circle.Draw` a été appelée depuis la méthode `Main` (`main` en C++), etc.
...
1. Avec le débogueur mis en pause dans l’appel de la méthode `Circle.Draw`, appuyez sur **F11** plusieurs fois jusqu’à ce que le débogueur se mette en pause sur l’appel de la méthode `base.Draw` (`Shape::Draw` en C++).

1. Utilisez la souris pour sélectionner la flèche jaune (le pointeur d’exécution) sur la gauche, puis déplacez la flèche jaune d’une ligne vers le haut, où est fait l’appel de la méthode `Console.WriteLine` (`std::cout` en C++).

1. Appuyez une fois de plus sur **F11**.

    Le débogueur réexécute la méthode `Console.WriteLine` (`std::cout` en C++).
