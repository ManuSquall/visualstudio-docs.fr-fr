---
title: Détails du tas de débogage CRT | Documents Microsoft
ms.custom: ''
ms.date: 11/04/2016
...
##  <a name="BKMK_Contents"></a> Sommaire  
 ...
 [Types de blocs sur le tas de débogage](#BKMK_Types_of_blocks_on_the_debug_heap)  
  
 [Recherchez les fuites de l’intégrité et de la mémoire du segment de mémoire](#BKMK_Check_for_heap_integrity_and_memory_leaks)  
  
 [Configurer le tas de débogage](#BKMK_Configure_the_debug_heap)  
  
 [nouveau, supprimer et _CLIENT_BLOCKs dans C++ tas de débogage](#BKMK_new__delete__and__CLIENT_BLOCKs_in_the_C___debug_heap)  
...
##  <a name="BKMK_Types_of_blocks_on_the_debug_heap"></a> Types de blocs sur le tas de débogage  
 Chaque bloc de mémoire dans le tas de débogage est assigné à l'un des cinq types d'allocations. Ces types sont suivis et reportés différemment pour la détection des fuites et la création de rapports d'état. Vous pouvez spécifier un type de bloc en l’allouant avec un appel direct à une des fonctions d’allocation de tas de débogage telles que [_malloc_dbg](/cpp/c-runtime-library/reference/malloc-dbg). Les cinq types de blocs de mémoire dans le tas de débogage (défini dans le **nBlockUse** membre de la **_CrtMemBlockHeader** structure) sont les suivantes :  
  
 **_NORMAL_BLOCK**  
 Un appel à [malloc](/cpp/c-runtime-library/reference/malloc) ou [calloc](/cpp/c-runtime-library/reference/calloc) crée un bloc Normal. Si vous avez l’intention d’utiliser uniquement des blocs Normal et n’ont pas besoin de blocs Client, vous souhaitez peut-être définir [_CRTDBG_MAP_ALLOC](/cpp/c-runtime-library/crtdbg-map-alloc), ce qui entraîne l’allocation de tas de tous les appels à être mappés à leurs équivalents de débogage dans les versions Debug. Cela permettra de stocker le nom de fichier et le numéro de ligne relatifs à chaque appel d'allocation dans l'en-tête du bloc correspondant.  
...
```  
 ...
 Une fonction de raccordement fournie au client pour faire un dump des objets stockés dans les blocs Client peut être installée à l’aide de [_CrtSetDumpClient](/cpp/c-runtime-library/reference/crtsetdumpclient)et sera ensuite appelée chaque fois qu’un bloc Client est exportée par une fonction de débogage. En outre, [_CrtDoForAllClientObjects](/cpp/c-runtime-library/reference/crtdoforallclientobjects) peut être utilisé pour appeler une fonction donnée fournie par l’application pour chaque bloc Client dans le tas de débogage.  
...
##  <a name="BKMK_Check_for_heap_integrity_and_memory_leaks"></a> Recherchez les fuites de l’intégrité et de la mémoire du segment de mémoire  
...
|**_CRTDBG_LEAK_CHECK_DF**|Off|Provoque une vérification des fuites à la sortie du programme via un appel à **_CrtDumpMemoryLeaks**. Un rapport d'erreurs est généré si l'application n'a pas pu libérer toute la mémoire qu'elle a allouée.|  
...
2.  Activez les bits par `OR`- effectue une opération (au niveau du bit &#124; symbole) la variable temporaire avec les masques de bits correspondants (représentés dans le code de l’application par des constantes manifestes).  
...
##  <a name="BKMK_new__delete__and__CLIENT_BLOCKs_in_the_C___debug_heap"></a> nouveau, supprimer et _CLIENT_BLOCKs dans C++ tas de débogage  
...
|--------------|-----------------|  
|[_CrtMemCheckpoint](/cpp/c-runtime-library/reference/crtmemcheckpoint)|Enregistre un instantané du tas dans un **_CrtMemState** structure fournie par l’application.|  
...
|[_CrtMemDumpStatistics](/cpp/c-runtime-library/reference/crtmemdumpstatistics)|Exporte une donnée **_CrtMemState** structure. La structure peut contenir un instantané de l'état du tas de débogage à un moment donné ou la différence entre deux instantanés.|  
|[_CrtMemDumpAllObjectsSince](/cpp/c-runtime-library/reference/crtmemdumpallobjectssince)|Fait un dump des informations sur tous les objets alloués depuis la capture d'un instantané donné du tas ou le début de l'exécution. Chaque fois qu’elle exporte un **_CLIENT_BLOCK** bloc, il appelle une fonction de raccordement fournie par l’application, si une telle fonction a été installée à l’aide de **_CrtSetDumpClient**.|  
