...
ms.locfileid: "72745413"
---
# <a name="compilanddetails"></a>CompilandDetails
Les informations de compiland sont réparties entre les symboles avec une balise de `SymTagCompiland` (détails bas) et une balise de `SymTagCompilandDetails` (détails élevés). `SymTagCompilandDetails` nécessite le chargement de symboles supplémentaires. Toutefois, il fournit une multitude d’informations sur le module (compiland) qui n’est pas disponible avec un symbole de `SymTagCompiland`.
...
|[IDiaSymbol::get_editAndContinueEnabled](../../debugger/debug-interface-access/idiasymbol-get-editandcontinueenabled.md)|`BOOL`|`TRUE` si Modifier & Continuer ont été activés au moment de la compilation.|
...
|[IDiaSymbol::get_hasDebugInfo](../../debugger/debug-interface-access/idiasymbol-get-hasdebuginfo.md)|`BOOL`|`TRUE` si ce module de module (compiland) contient des informations de débogage (uniquement dans DIA SDK V 8.0 ou version ultérieure).|
|[IDiaSymbol::get_hasManagedCode](../../debugger/debug-interface-access/idiasymbol-get-hasmanagedcode.md)|`BOOL`|`TRUE` si ce compiland contient du code managé (uniquement dans DIA SDK v 8.0 ou ultérieur).|
...
|[IDiaSymbol::get_platform](../../debugger/debug-interface-access/idiasymbol-get-platform.md)|`DWORD`|Plateforme sur laquelle le compiland a été compilé (l’une des valeurs d' [énumération CV_CPU_TYPE_e](../../debugger/debug-interface-access/cv-cpu-type-e.md) ).|
