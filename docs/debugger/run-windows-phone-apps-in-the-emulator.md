---
title: "Exécuter des applications Windows Phone 8.1 dans l’émulateur | Documents Microsoft"
ms.custom: 
ms.date: 07/18/2017
ms.reviewer: 
ms.suite: 
...
Pour plus d’informations sur l’exécution de Windows 10 Mobile dans l’émulateur, consultez [Test avec Microsoft Emulator](/windows/uwp/debug-test-perf/test-with-the-emulator).
  
Pour plus d’informations sur les fonctionnalités que vous pouvez tester dans l’émulateur Windows Phone 8.1, consultez [tester les fonctionnalités de l’application dans l’émulateur de Windows Phone](http://msdn.microsoft.com/en-us/c1b2b0ec-b8cc-4a98-84c1-701428e45cb1).  
...
> [!TIP]
>  Lorsque vous testez votre application sur l’émulateur, laissez l’émulateur ouvert entre les sessions de débogage pour pouvoir réexécuter votre application rapidement.  
...
2.  Sur le **Standard** barre d’outils, sélectionnez une des options d’émulateur.  
     ...
     ![Liste des images d’émulateur de Windows Phone](../debugger/media/wp_emulator_list.png "WP_Emulator_list")  
...
3.  Pour déployer et exécuter votre application avec débogage, dans le **déboguer** menu, cliquez sur **démarrer le débogage**, ou appuyez sur F5.  
     ...
     Pour déployer votre application sans l’exécuter, dans le **générer** menu, cliquez sur **déployer la Solution**.  
...
-   Pour arrêter une application en cours d'exécution, effectuez l'une des opérations suivantes :  
    ...
    -   Dans l’émulateur, appuyez sur la **retour** pour quitter l’application. Si la page active de l’application n’a pas de page de démarrage de l’application, vous devrez peut-être appuyer sur la **précédent** bouton plusieurs fois.  
...
2.  Dans **Concepteur de projets**, dans le **déboguer** page, laissez le **Tombstone sur désactivation lors du débogage** case est désactivée si vous voulez que l’application passe à un dormant lorsque l’état désactivé. Cochez la case si vous voulez que l'application soit complètement désactivée.  
...
-   Dans l’émulateur, appuyez sur la **précédent** bouton pour revenir à l’application. Si vous avez navigué dans d’autres pages ou ouvert une autre application, vous devrez peut-être appuyer sur la **précédent** bouton plusieurs fois pour réactiver l’application.  
...
###  <a name="BKMK_depltool"></a>Exécuter une application avec l’outil de déploiement d’Application  
 ...
 Pour plus d’informations, consultez [des applications de déploiement de Windows Phone 8.1 avec l’outil de déploiement de l’Application](http://msdn.microsoft.com/Library/23700f82-1399-44d9-bc0c-714be4a48ee6).  
...
|---------------------|---------------------------|  
|![Options de barre d’outils Windows Phone Emulator d’entrée](../debugger/media/wp_emulator_.png "WP_Emulator_")|**Configurer la saisie à un seul point ou multipoint**<br /><br /> Quand vous activez la saisie multipoint, vous pouvez cliquer avec le bouton droit pour déplacer les points tactiles sans toucher l'écran. Vous pouvez ensuite cliquer avec le bouton gauche pour déplacer les deux points tactiles simultanément.|  
|![Orientation de la barre d’outils Windows Phone Emulator](../debugger/media/wp_emulator_rotation.png "WP_Emulator_rotation")|**Configurer l’orientation de l’émulateur**<br /><br /> Vous pouvez choisir parmi trois orientations possibles pour l'émulateur Windows Phone : portrait, paysage gauche ou paysage droite. La taille de l'émulateur n'est pas modifiée quand vous changez l'orientation.<br /><br /> Pour modifier l’orientation, cliquez sur le **faire pivoter à gauche** bouton ou le **faire pivoter à droite** bouton.|  
|![Taille des options sur la barre d’outils Windows Phone Emulator](../debugger/media/wp_emulator_size.png "WP_Emulator_size")|**Configurer la taille de l’émulateur**<br /><br /> Vous pouvez modifier la taille de l'émulateur sur l'écran de l'ordinateur hôte. La taille en points par pouce (PPP) de l'émulateur est basée sur celle du moniteur hôte, indépendamment de la valeur du zoom.<br /><br /> -Pour ajuster l’émulateur à l’écran, cliquez sur le **ajuster à l’écran** bouton.<br />-Pour modifier le paramètre du zoom, cliquez sur le **Zoom** bouton. Le **Zoom** boîte de dialogue s’ouvre. Dans le **Zoom** boîte de dialogue, entrez une valeur de zoom entre 33 et 100.|  
...
-   Cliquez sur le **Power** bouton pour simuler l’affichage activation et la désactivation. Cliquez et maintenez le bouton enfoncé pour simuler la désactivation du téléphone.  
  
-   Cliquez sur le **monter le Volume** ou **baisser le Volume** pour simuler la modification du volume des haut-parleurs du téléphone pour les notifications et les appels téléphoniques.  
...
2.  L’image du milieu affiche l’émulateur après avoir appuyé sur le **Power** bouton pour éteindre l’affichage.  
...
3.  L’image de droite affiche l’écran de l’émulateur après avoir appuyé sur le **monter le Volume** bouton pour augmenter le volume.  
 ...
 ![Boutons sur l’émulateur Windows Phone](../debugger/media/wp_emulator_buttons.png "WP_Emulator_buttons")  
...
##  <a name="BKMK_tasks_kbd"></a>Utiliser le clavier de l’ordinateur avec l’émulateur  
 ...
 Pour utiliser le clavier de votre ordinateur dans l’émulateur, appuyez sur la touche Pg préc ou la PAUSE/ATTN (émulateur Windows 8/8.1) ou F4 (émulateur Windows 10).  
  
 Pour cesser d’utiliser le clavier de votre ordinateur dans l’émulateur, appuyez sur la touche Pg suiv ou la PAUSE/ATTN (émulateur Windows 8/8.1) ou F4 (émulateur Windows 10).  
...
|F3|Rechercher||  
|F4|Dans l’émulateur Windows 10, tour à l’aide du clavier de l’ordinateur local et non à l’aide de clavier de l’ordinateur local.|Non applicable dans l'émulateur Windows 8/8.1.|  
|F5|Non applicable.||  
...
##  <a name="BKMK_checkpoints"></a>Enregistrer et charger des points de contrôle personnalisés  
 Enregistrer un instantané de l’état de l’émulateur Windows à l’aide de la **des points de contrôle** onglet de l’émulateur Windows **outils supplémentaires**. Cette fonctionnalité est utile si vous testez fréquemment votre application avec les mêmes données et paramètres.  
...
-   Cliquez sur **nouveau point de contrôle** pour capturer un nouvel instantané de l’état de l’émulateur avec les données et les paramètres requis pour tester votre application plus tard. Le nouveau point de contrôle est ajouté à la **des points de contrôle** liste.  
  
     Vous ne pouvez pas capturer un point de vérification si le débogueur est attaché à l'émulateur.  
  
-   Sélectionnez un point de contrôle dans le **des points de contrôle** liste pour afficher les informations sur le point de contrôle.  
  
-   Sélectionnez la case d’option dans le **par défaut** colonne afin qu’un point de contrôle enregistré le point de contrôle par défaut pour l’émulateur actif.  
...
-   Cliquez sur **supprimer** pour supprimer un instantané dont vous n’avez plus besoin.  
 ...
 L’image d’origine de l’émulateur apparaît toujours comme le premier élément dans le **des points de contrôle** liste et ne peut pas être modifiées ou supprimées. Toutefois, vous pouvez sélectionner un autre instantané que l'image par défaut de l'émulateur.  
  
 ![Onglet points de contrôle de l’émulateur Windows Phone](../debugger/media/wp_emulator_checkpoints.png "WP_Emulator_checkpoints")  
...
##  <a name="BKMK_tasks_shot"></a>Captures d’écran dans l’émulateur  
 ...
 ![Captures d’écran à partir du Windows Phone Emulator](../debugger/media/wp_emulator_screenshots.png "WP_Emulator_screenshots")  
...
3.  Dans la barre d’outils de l’émulateur, cliquez sur le bouton développer pour ouvrir la **outils supplémentaires** fenêtre.  
  
4.  Cliquez sur le **capture d’écran de** onglet.  
  
5.  Lorsque votre application est prête, cliquez sur le **Capture** bouton.  
...
-   Spécifiez une version Release dans Visual Studio. Après avoir spécifié une version Release, lancez votre application en sélectionnant le **déployer *[nom de l’application]***  lien sur le **générer** menu.  
