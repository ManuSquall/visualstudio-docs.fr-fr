---
title: "Procédure pas à pas de LeaderboardScene Visual Studio Tools pour Unity Azure| Microsoft Docs"
ms.custom: 
ms.date: 10/19/2017
ms.reviewer: crdun
ms.suite: 
...
LeaderboardScene est composé uniquement d’interface utilisateur. Dans la hiérarchie de scène, utilisez **Alt+clic** sur la **flèche de développement** en regard du GameObject Canvas pour le développer avec ses GameObjects enfants. Le GameObject Leaderboard avec le script Leaderboard attaché est imbriqué sous Canvas et Panel.

![GameObject Leaderboard](media/vstu_azure-leaderboard-explanation-image1.png)

## <a name="leaderboard-script"></a>Script Leaderboard

La classe `Leaderboard` utilise une fonction `async Start`, qui est toujours appelée quand le script est activé, tout comme une fonction `Start` Unity classique.

`DownloadHighScoresAsync` utilise les fonctions `OrderBy` et `Take` de <a href="https://msdn.microsoft.com/en-us/library/azure/jj554257(v=azure.10).aspx">`IMobileServiceTable`</a> pour trier les scores élevés dans la table facile Azure et accepter uniquement les premières entrées selon la constante `SizeOfHighScoreList`, qui sont stockées dans la liste `highScores`.

Une instance de l’interface utilisateur préfabriquée des lignes des scores élevés est ensuite instanciée pour chaque entrée de la liste.
