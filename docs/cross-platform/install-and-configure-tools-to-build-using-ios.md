...
> [!NOTE]
>  Pour plus d’informations sur l’utilisation des services Mac hébergés dans le cloud plutôt que sur un Mac, consultez [Générer et simuler iOS dans le cloud](https://taco.visualstudio.com/en-us/docs/build_ios_cloud/). Les instructions portent sur la génération à l’aide de Visual Studio Tools pour Apache Cordova. Pour suivre les instructions de génération à l’aide de Visual C++ pour le développement mobile multiplateforme, remplacez vcremote par vs-mda-remote.  
 ...
 [Générer un nouveau code PIN de sécurité](#GeneratePIN)  
  
 [Générer un nouveau certificat de serveur](#GenerateCert)  
  
 [Configurer l’agent distant sur le Mac](#ConfigureMac)  
...
-   Un [identifiant Apple](https://appleid.apple.com/)  
...
-   Une identité de signature iOS configurée dans Xcode  
     ...
     Pour plus d’informations sur l’obtention d’une identité de signature iOS, consultez le document relatif à la [gestion des identités et des certificats de signature](https://developer.apple.com/library/ios/documentation/IDEs/Conceptual/AppDistributionGuide/MaintainingCertificates/MaintainingCertificates.html) dans la bibliothèque du développeur iOS. Pour afficher ou définir votre identité de signature dans Xcode, ouvrez le menu **Xcode** et choisissez **Préférences**. Sélectionnez **Comptes**, choisissez votre identifiant Apple, puis cliquez sur le bouton **Afficher les détails**.  
...
-   Si vous utilisez un appareil iOS pour le développement, un profil de mise en service (« Provisioning Profile ») configuré dans Xcode pour votre appareil  
     ...
     Pour plus d’informations sur la création de profils d’approvisionnement, consultez le document relatif à la [création de profils d’approvisionnement via le centre des membres](https://developer.apple.com/library/ios/documentation/IDEs/Conceptual/AppDistributionGuide/MaintainingProfiles/MaintainingProfiles.html#//apple_ref/doc/uid/TP40012582-CH30-SW24) dans la bibliothèque du développeur iOS.  
...
##  <a name="Install"></a> Installer l’agent distant pour iOS  
 Quand vous installez Visual C++ pour le développement mobile multiplateforme, Visual Studio peut communiquer avec [vcremote](http://go.microsoft.com/fwlink/p/?LinkId=534988). Cet agent distant qui s’exécute sur votre Mac permet de transférer des fichiers, de générer et d’exécuter votre application iOS, et d’envoyer des commandes de débogage.  
  
 Avant d’installer l’agent distant, vérifiez que vous disposez bien des [prérequis](#Prerequisites) et que vous avez installé [Visual C++ pour le développement mobile multiplateforme](../cross-platform/install-visual-cpp-for-cross-platform-mobile-development.md#InstallTheTools).  
...
-   Dans l’application Terminal de votre Mac, entrez :  
     ...
     Pendant l’installation, vcremote est installé et le mode développeur est activé sur votre Mac. [Homebrew](http://brew.sh/) et deux packages npm, vcremote-lib et vcremote-utils, sont également installés.  
...
-   Dans l’application Terminal de votre Mac, entrez :  
     ...
     L’agent distant démarre avec un répertoire de build par défaut ~/vcremote. Pour obtenir des options de configuration supplémentaires, consultez [Configurer l’agent distant sur le Mac](#ConfigureMac).  
 ...
 Le code confidentiel généré est à usage unique et n’est valable que pour une durée limitée. Si vous ne couplez pas Visual Studio à l’agent distant avant l’expiration du délai, vous devrez générer un nouveau code confidentiel. Pour plus d’informations, consultez [Générer un nouveau code PIN de sécurité](#GeneratePIN).  
...
7.  Choisissez **Coupler** pour activer le jumelage.  
     ...
     Le couplage persiste tant que vous ne changez pas de nom d’hôte ou de port. Si vous changez de nom d’hôte ou de port dans la boîte de dialogue **Options**, pour annuler la modification et rétablir le jumelage précédent, choisissez le bouton **Rétablir**.  
  
     Si le jumelage ne fonctionne pas, vérifiez que l’agent distant s’exécute selon les étapes décrites dans [Démarrer l’agent distant](#Start). Si trop de temps s’est écoulé après la génération du code PIN de l’agent distant, suivez les étapes décrites dans [Générer un nouveau code PIN de sécurité](#GeneratePIN) sur le Mac, puis réessayez. Si vous utilisez le nom d’hôte de votre Mac, essayez plutôt d’utiliser l’adresse IP qui figure dans le champ **Nom d’hôte**.  
...
9. Choisissez **OK** pour enregistrer les paramètres de connexion de jumelage à distance.  
...
##  <a name="GeneratePIN"></a> Générer un nouveau code PIN de sécurité  
...
2.  Entrez cette commande dans l’application Terminal :  
     ...
     L’agent distant génère un nouveau code confidentiel temporaire. Pour jumeler Visual Studio en utilisant le nouveau code PIN, répétez les étapes décrites dans [Configurer l’agent distant dans Visual Studio](#ConfigureVS).  
...
5.  Pour jumeler Visual Studio en utilisant le nouveau code PIN, répétez les étapes décrites dans [Configurer l’agent distant dans Visual Studio](#ConfigureVS).  
  
##  <a name="ConfigureMac"></a> Configurer l’agent distant sur le Mac  
...
-   Pour spécifier l’emplacement des fichiers de l’agent distant, entrez :  
     ...
     où *chemin_répertoire* est l’emplacement sur votre Mac des fichiers journaux, des builds et des certificats de serveur. Par défaut, cet emplacement correspond à /Users/*nom_utilisateur*/vcremote. Les builds sont organisées par numéro de build à cet emplacement.  
...
-   Pour exécuter l’agent en utilisant un fichier de configuration au lieu de paramètres de ligne de commande, entrez :  
     ...
     où *chemin_fichier_config* est le chemin d’un fichier config au format JSON. Les options de démarrage et leurs valeurs ne doivent pas inclure de tirets.  
