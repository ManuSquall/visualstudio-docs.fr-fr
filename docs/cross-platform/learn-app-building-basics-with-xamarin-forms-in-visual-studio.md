...
1.  Remplacez le contenu de **WeatherPage.xaml** par le balisage ci-dessous. Les éléments nommés à l’aide de l’attribut `x:Name`, comme décrit précédemment, peuvent être référencés à partir du code. Xamarin.Forms fournit également plusieurs [options de disposition](/xamarin/xamarin-forms/controls/layouts/). Ici, WeatherPage utilise [Grid](http://developer.xamarin.com/api/type/Xamarin.Forms.Grid/) et [StackLayout](http://developer.xamarin.com/api/type/Xamarin.Forms.StackLayout/).  
