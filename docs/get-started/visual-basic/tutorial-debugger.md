...
3. Dans la boîte de dialogue **Nouveau projet**, dans le volet gauche, développez **Visual Basic**, puis choisissez **.NET Core**. Dans le volet central, choisissez **Application console (.NET Core)**. Ensuite, nommez le projet *get-started-debugging*.
...
1. Dans la fenêtre **Créer un projet**, entrez ou tapez *console* dans la zone de recherche. Ensuite, choisissez **Visual Basic** dans la liste des langages, puis choisissez **Windows** dans la liste des plateformes. 
   ...
   ![Choisissez le modèle de base visuelle pour l’application Console (.NET Core)](../visual-basic/media/vs-2019/get-started-create-console-project.png)
...
1. Appuyez sur **F11** une `SendMessage` fois de plus pour entrer dans la méthode.
     ...
     ![Utilisez F11 pour entrer dans le code](../visual-basic/media/get-started-f11-vb.png "F10 Entrez")
...
1. En pause à l’appel de méthode, appuyez sur **F10** (ou choisissez **Debug > Step Over**) une fois.
     ...
     ![Utilisez F10 pour passer au-dessus du code](../visual-basic/media/get-started-step-over-vb.png "F10 Step Over")
...
1. Dans l’éditeur de code, `Console.WriteLine` faites défiler vers le bas et planez au-dessus de la méthode dans la `SendMessage` méthode jusqu’à ce que le bouton vert Run to **Click** Run to ![Click](../../debugger/media/dbg-tour-run-to-click.png "RunToClick") apparaît sur la gauche. L’info-bulle du bouton indique « Lancer l’exécution jusqu’ici ».
     ...
     ![Utilisez la fonction Run to Click](../visual-basic/media/get-started-run-to-click-vb.png "Exécuter jusqu’au clic")
...
1. Bien qu’en `name += letters[i]` pause sur `letters` l’instruction, planer au-dessus de la variable et `"f"c`vous voyez que c’est la valeur par défaut, la valeur du premier élément dans le tableau, .
...
1. Appuyez sur **F5** (ou **Debug** > **Continuer**) à `For` quelques reprises pour itérer à plusieurs reprises à `name` travers la boucle, s’arrêtant à nouveau au point d’arrêt, et planant sur la variable à chaque fois pour vérifier sa valeur.
     ...
     ![Afficher une astuce de données](../visual-basic/media/get-started-data-tip-vb.png "Afficher un conseil de données")
...
1. Élargissez `letters` la variable pour montrer les éléments qu’elle contient.
     ...
     ![Inspecter les variables dans la fenêtre locale](../visual-basic/media/get-started-locals-window-vb.png "Variables locales (fenêtre)")
...
2. Cliquez **sur F11** à quelques reprises jusqu’à `SendMessage` ce que vous voyez la pause de débbugger dans la méthode. Regardez la fenêtre **Pile des appels**.
    ...
    ![Examiner la pile des appels](../visual-basic/media/get-started-call-stack-vb.png "ExaminerCallStack")
